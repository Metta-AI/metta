...
class Trainer:
    ...
    def __init__(
        ...
        self._metta_agent = self._distributed_helper.wrap_policy(self._metta_agent, self._device)
        self._metta_agent.train()
        features = self._env.observation_features
        self._metta_agent.initialize_to_environment(
            features, self._env.action_names, self._env.max_action_args, self._device
        )

        losses = self._cfg.losses.init_losses(self._metta_agent, self._cfg, self._env, self._device)
        ...
        experience = Experience.from_losses(
            ...
            self._cfg.minibatch_size,
            self._metta_agent.get_agent_experience_spec(),
            self._env.single_action_space,
            losses,
            self._device,
        )

        self.optimizer = create_optimizer(self._cfg.optimizer, self._metta_agent)

        self.core_loop = CoreTrainingLoop(
            policy=self._metta_agent,
