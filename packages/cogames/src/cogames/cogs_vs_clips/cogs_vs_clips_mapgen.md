...
- `--seed` controls simulator/policy RNG only; it does not affect map layout.
...
- `play`/`eval`: with `--map-seed` (or a mission seed), the map layout follows a deterministic per-episode seed
  sequence; without `--map-seed` (and no mission seed), the map layout is random.
...
- Recipe `maps_cache_size` controls how many distinct procedural maps are cached when the MapGen seed is unset (no
  `--map-seed` and mission seed is `None`). When a MapGen seed is set, the simulator advances the seed each episode;
  if `maps_cache_size` is set, the seed sequence repeats every `maps_cache_size` episodes, otherwise it increases
  monotonically. Set it to `None` to disable caching (more diversity, slower) or `1` for a single cached map.
