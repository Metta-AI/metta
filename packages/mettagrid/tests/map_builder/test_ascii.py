...
class TestAsciiMapBuilderConfig:
    def test_create(self):
        ...
        try:
            config = mettagrid.map_builder.ascii.AsciiMapBuilder.Config.from_uri(temp_file)
            builder = config.create()
            assert isinstance(builder, mettagrid.map_builder.ascii.AsciiMapBuilder)
        finally:
            os.unlink(temp_file)
...
class TestAsciiMapBuilder:
    ...
    def test_build_complex_map(self):
        ...
        try:
            config = mettagrid.map_builder.ascii.AsciiMapBuilder.Config.from_uri(temp_file)
            builder = mettagrid.map_builder.ascii.AsciiMapBuilder(config)
            game_map = builder.build()
    ...
    def test_build_single_line_map(self):
        ...
        try:
            ...
            expected = np.array([["wall", "agent.agent", "wall"]], dtype=mettagrid.mapgen.types.map_grid_dtype)
            assert np.array_equal(game_map.grid, expected)
        finally:
            os.unlink(temp_file)

    def test_config_accepts_string_map_data(self):
        config = mettagrid.map_builder.ascii.AsciiMapBuilder.Config(
    ...
    def test_build_empty_map(self):
        ...
        try:
            ...
            expected = np.array([["empty"]], dtype=mettagrid.mapgen.types.map_grid_dtype)
            assert np.array_equal(game_map.grid, expected)
        finally:
            os.unlink(temp_file)
    ...
    def test_file_not_found(self):
        with pytest.raises(FileNotFoundError):
            config = mettagrid.map_builder.ascii.AsciiMapBuilder.Config.from_uri("nonexistent_file.txt")
    ...
    def test_global_defaults_merge_into_legend(self):
        ...
        config = mettagrid.map_builder.ascii.AsciiMapBuilder.Config.from_str(yaml_content)
        assert config.char_to_name_map["#"] == "wall"
        assert config.char_to_name_map["."] == "empty"
        assert config.char_to_name_map["@"] == "agent.agent"
    ...
    def test_global_defaults_cannot_be_overridden(self):
        ...
        with pytest.raises(pydantic.ValidationError, match=r"Cannot override global default mapping for '#'.*"):
            mettagrid.map_builder.ascii.AsciiMapBuilder.Config.from_str(yaml_content)
    ...
    def test_irregular_shaped_map(self):
        ...
        try:
            with pytest.raises(ValueError, match="All lines in ASCII map must have the same length"):
                mettagrid.map_builder.ascii.AsciiMapBuilder.Config.from_uri(temp_file)
        finally:
            os.unlink(temp_file)
