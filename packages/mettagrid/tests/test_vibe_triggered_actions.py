    ...
    def test_transfer_fails_without_enough_resources(self):
        ...
        config = GameConfig(
            ...
            agents=[
                AgentConfig(
                    team_id=0,
                    inventory=InventoryConfig(initial={"energy": 10}),  # Only 10 energy, not enough
                ),
                AgentConfig(
                    team_id=1,
    ...
    def test_attack_takes_priority_over_transfer(self):
        ...
        config = GameConfig(
            ...
            actions=ActionsConfig(
                ...
                attack=AttackActionConfig(
                    enabled=False,
                    vibes=["charger"],  # Attack triggers on charger vibe
                    success=AttackOutcome(freeze=5),
                ),
                transfer=TransferActionConfig(
                    enabled=True,
                    vibe_transfers=[
                        VibeTransfer(
                            vibe="charger",  # Transfer also triggers on charger vibe
                            target={"energy": 5},
                            actor={"energy": -5},
                        ),
                    ],
