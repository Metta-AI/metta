...
from .types import BaselineHyperparameters, CellType, ParsedObservation, SimpleAgentState
from .utils import (
    change_vibe_action,
    is_station,
    is_wall,
...
class DemoPolicyImpl(StatefulPolicyImpl[SimpleAgentState]):
    ...
    def _random_step(self, s, parsed):
        ...
        blocked = {
            ...
            if self._adjacent(s,(r,c)) and
               (is_wall(obj.name) or "extractor" in obj.name
    ...
    def _step_towards(self, s, target, parsed):
        ...
        if abs(tr-r) >= abs(tc-c):
            if tr<r: cand.append("north")
            elif tr>r: cand.append("south")
            if tc<c: cand.append("west")
            elif tc>c: cand.append("east")
        else:
            if tc<c: cand.append("west")
            elif tc>c: cand.append("east")
            if tr<r: cand.append("north")
    ...
    def _rtype(self, name):
        ...
        if "_extractor" not in name: return None
    ...
    def step_with_state(self, obs:AgentObservation, s:SimpleAgentState):
        ...
        parsed = parse_observation(
            s, obs,
            obs_hr=self._obs_hr, obs_wr=self._obs_wr,
        ...
        # Assemble
        if (
            s.carbon>=s.heart_recipe.get("carbon",0) and
            s.oxygen>=s.heart_recipe.get("oxygen",0) and
            s.germanium>=s.heart_recipe.get("germanium",0) and
            s.silicon>=s.heart_recipe.get("silicon",0)
        ):
            assembler = self._closest(s, parsed, lambda o: is_station(o.name.lower(),"assembler"))
            if assembler:
                if s.current_glyph!="heart_a":
        ...
        deficits = {
            r: s.heart_recipe.get(r,0) - getattr(s,r,0)
