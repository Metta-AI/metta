...
class MambaBackboneComponent(nn.Module):
    """Simplified Mamba backbone that mirrors the Transformer policy flow."""
    ...
    def forward(self, td: TensorDict) -> TensorDict:
        ...
        tokens = tokens.reshape(batch * tt, seq_len, tokens.size(-1))
        positions = torch.arange(seq_len, device=device, dtype=torch.long)
        pos_enc = _sinusoidal_positional_encoding(positions, tokens.size(-1))
        tokens = tokens + pos_enc.unsqueeze(0)

        hidden = self._apply_layers(tokens)
        pooled = self._pool_output(hidden)
        flat = pooled.reshape(batch * tt, -1)

        td[self.out_key] = flat
        return td
