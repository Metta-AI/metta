...
def sample_checkpoint_path():
    ...
    if not model_files:
        pytest.skip("No trained tribal model found. Run training first: "
...
class TestTribalNeuralNetworkLoading:
    ...
    def test_neural_network_inference(self, sample_checkpoint_path, tribal_environment):
        ...
        if '_action_' in output_td:
            ...
            assert action_logits.shape[0] == batch_size * num_agents, f"Expected {batch_size * num_agents} agents, got {action_logits.shape[0]}"
            ...
            print(f"✅ Action logits shape: {action_logits.shape}, range: [{action_logits.min():.3f}, {action_logits.max():.3f}]")
        
        if 'values' in output_td:
            values = output_td['values']
            assert isinstance(values, torch.Tensor), "Values should be tensor"
            assert values.shape[0] == batch_size * num_agents, f"Expected {batch_size * num_agents} agents, got {values.shape[0]}"
        ...
        if 'actions' in output_td:
            ...
            assert actions.shape[0] == batch_size * num_agents, f"Expected {batch_size * num_agents} agents, got {actions.shape[0]}"
    ...
    def test_neural_network_architecture(self, sample_checkpoint_path):
        ...
        expected_components = [
            '_obs_', 'obs_normalizer', 'cnn1', 'cnn2', 'obs_flattener',
            'fc1', 'encoded_obs', '_core_', 'critic_1', '_value_',
            'actor_1', '_action_embeds_', 'actor_query', '_action_'
        ]
    ...
    def test_end_to_end_inference_consistency(self, sample_checkpoint_path, tribal_environment):
        ...
        if '_action_' in output1 and '_action_' in output2:
            torch.testing.assert_close(output1['_action_'], output2['_action_'], 
                                     msg="Action logits should be deterministic")
        
        if 'values' in output1 and 'values' in output2:
            torch.testing.assert_close(output1['values'], output2['values'],
                                     msg="Value estimates should be deterministic")
