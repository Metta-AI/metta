...
class AgalitePure(nn.Module):
    ...
    def __init__(
        ...
        self.cnn_encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            init_layer(nn.Linear(128, d_model // 2)),
            nn.ReLU(),
        )

        self.self_encoder = nn.Sequential(
            init_layer(nn.Linear(self.num_layers, d_model // 2)),
            nn.ReLU(),
        )
        ...
        if hasattr(self.action_space, "nvec"):
            ...
            self.actor = nn.ModuleList([init_layer(nn.Linear(d_model, n), std=0.01) for n in self.action_space.nvec])
        else:
            ...
            self.actor = init_layer(nn.Linear(d_model, num_actions), std=0.01)

        self.critic = init_layer(nn.Linear(d_model, 1), std=1)
