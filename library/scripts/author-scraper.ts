...
export async function fetchPapersByAuthor(
  ...
  try {
    ...
    console.log(
      `🔍 Fetching papers for author: "${cleanAuthorName}" (max: ${maxResults})...`
    );
    ...
    const totalResults = parseInt(
      parsedData.feed["opensearch:totalResults"]?._ || "0",
      10
    );
    ...
    const papers: ArxivPaperData[] = entries.map((entry) => {
      ...
      const primaryCategory =
        entry["arxiv:primary_category"]?.term || categories[0];
    ...
    console.log(
      `✅ Successfully fetched ${papers.length} papers for "${cleanAuthorName}" (total available: ${totalResults})`
    );
    ...
    console.error(
      `❌ Error fetching papers for author: ${error instanceof Error ? error.message : "Unknown error"}`
    );
    throw error;
  }
}
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log("Usage:");
    console.log(
      '  tsx scripts/author-scraper.ts "Author Name" [--max-results <number>]'
    );
    console.log("");
  ...
  if (maxResultsIndex !== -1 && maxResultsIndex + 1 < args.length) {
    ...
    if (
      !isNaN(maxResultsValue) &&
      maxResultsValue > 0 &&
      maxResultsValue <= 2000
    ) {
      maxResults = maxResultsValue;
    } else {
      console.error(
        "❌ Invalid max-results value. Must be between 1 and 2000."
      );
      process.exit(1);
    }
  }
  ...
  try {
    ...
    console.error(
      `❌ Failed to fetch papers: ${error instanceof Error ? error.message : "Unknown error"}`
    );
    process.exit(1);
  }
}
