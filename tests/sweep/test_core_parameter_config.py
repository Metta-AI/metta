...
def test_parameter_config_creation():
    pc = metta.sweep.core.ParameterConfig(min=0.001, max=0.1, distribution="log_normal", mean=0.01, scale="auto")
    assert pc.min == 0.001
    assert pc.max == 0.1
...
def test_sweep_parameters_builder_logit_sanitization():
    ...
    param = metta.sweep.core.SweepParameters.param(
        name="trainer.losses.loss_configs.ppo.gae_lambda",
        distribution=metta.sweep.core.Distribution.LOGIT_NORMAL,
        min=0.0,
        max=1.0,
        search_center=None,
        scale="auto",
