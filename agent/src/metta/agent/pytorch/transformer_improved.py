"""
Improved transformer agent - starting from exact copy of working transformer.

This implementation starts as an exact copy of the working transformer and
adds only minimal, careful improvements one at a time.
Current changes from base transformer:
1. Slightly larger hidden dimensions (128 -> 192) for more capacity
2. Everything else identical to ensure stability
"""
...
class ImprovedPolicy(nn.Module):
    """Policy network - nearly identical to working transformer."""

    def __init__(
        self,
        env,
        input_size: int = 192,  # Slightly larger than 128
        hidden_size: int = 192,  # Slightly larger than 128
        n_heads: int = 8,  # Same as original
        n_layers: int = 6,  # Same as original
        d_ff: int = 768,  # Slightly larger than 512
        max_seq_len: int = 256,  # Same as original
        dropout: float = 0.1,
        use_causal_mask: bool = True,
        use_gating: bool = True,
    ):
        """Initialize the policy network with minimal changes from working transformer.
        ...
        # Linear layers to match Fast (but with our input_size)
        ...
        # Transformer module (replacing LSTM)
        logger.info(
        ...
        # Critic branch with conservative initialization for transformers
        self.critic_1 = init_layer(nn.Linear(hidden_size, 1024), std=1.0)  # Same as original
        self.value_head = init_layer(nn.Linear(1024, 1), std=0.1)  # Small for value head

        # Actor branch with conservative initialization
        self.actor_1 = init_layer(nn.Linear(hidden_size, 512), std=0.5)  # Same as original
...
class TransformerImproved(PyTorchAgentMixin, TransformerWrapper):
    """Improved transformer agent - nearly identical to working transformer.

    This implementation starts from the working transformer and adds only
    minimal improvements to ensure stability while getting slightly better
    performance.
    """
    ...
    def __init__(
        ...
        input_size: int = 192,  # Slightly larger
        hidden_size: int = 192,  # Slightly larger
        n_heads: int = 8,  # Same as original
        n_layers: int = 6,  # Same as original
        d_ff: int = 768,  # Slightly larger
        max_seq_len: int = 256,  # Same as original
        dropout: float = 0.1,
        use_causal_mask: bool = True,
        use_gating: bool = True,
    ...
    def forward(self, td: TensorDict, state=None, action=None):
        """Forward pass through the agent - exact copy from working transformer."""
