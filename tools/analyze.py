...
def main():
    ...
    parser.add_argument("--eval-db-uri", type=str, default=None, help="Override evaluation database URI")
    parser.add_argument("--device", type=str, default=None, help="Override device (cuda/cpu)")

    args = parser.parse_args()

    # Load configuration from file
    cfg = AnalyzeConfig.from_file(args.config)
    ...
    try:
        ...
        policy_pr = policy_store.policy_record(
            cfg.analysis.policy_uri, cfg.analysis.policy_selector.type, metric=cfg.analysis.policy_selector.metric
        )
