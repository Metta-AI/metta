...
class JobManager:
    ...
    def _get_total_timesteps(self, job_config: JobConfig) -> int | None:
        ...
        Supports both metta training (trainer.total_timesteps) and cogames (steps).
        """
        # Check metta/arena style
        if "trainer.total_timesteps" in job_config.args:
            try:
                return int(job_config.args["trainer.total_timesteps"])
            except (ValueError, TypeError):
                pass

        # Check cogames style
        if "steps" in job_config.args:
            try:
                return int(job_config.args["steps"])
            except (ValueError, TypeError):
                pass
