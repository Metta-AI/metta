...
class TestTrainingJobConfig:
    ...
    def test_training_job_with_config(self):
        ...
        job = TrainingJob(
            wandb_run_name="test.run.1", skypilot_job_id="sky-test-123", config=config, notes="Test job with config"
        )
    ...
    def test_training_job_serialization_with_config(self):
        ...
        config = TrainingJobConfig(
            curriculum="test/curriculum", gpus=8, additional_args=["trainer.optimizer.learning_rate=0.001"]
        )

        original_job = TrainingJob(
            wandb_run_name="test.serialize.1", skypilot_job_id="sky-serialize-123", config=config
        )
