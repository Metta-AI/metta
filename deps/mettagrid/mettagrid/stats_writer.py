...
class StatsDB:
    ...
    def create_episode(self, seed: int, map_w: int, map_h: int, metadata: Optional[Dict] = None) -> str:
        if self.read_only:
        ...
        self.con.execute(
            ...
            (id, seed, map_w, map_h, started_at, metadata)
            VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, ?)
            """,
            (episode_id, seed, map_w, map_h, metadata_json),
        )
...
class StatsWriter:
    ...
    def start_episode(self, seed: int, map_w: int, map_h: int, meta: Optional[Dict] = None) -> str:
        """
        Start a new episode.
        ...
        Returns:
            Episode ID
        """
        self.current_episode_id = self.db.create_episode(seed, map_w, map_h, meta)
        self.metrics = {}
