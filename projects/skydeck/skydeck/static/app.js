...
function updateExperimentsTable(experiments) {
    if (!experiments || experiments.length === 0) {
        document.getElementById('experiments-tbody').innerHTML =
            '<tr><td colspan="100" class="empty-state">No experiments found. Click "New Experiment" to get started.</td></tr>';
        return;
    }
    ...
    experiments.forEach(exp => {
        if (editingRows.has(exp.id)) {
            ...
            // If this row was already expanded, reload its job history and checkpoints
...
function updateTableHeader(flagColumns) {
    ...
    if (flagColumns.length === 0) {
        ...
        thead.innerHTML = `
            <tr>
                ...
                <th class="col-checkbox"><input type="checkbox" id="select-all" onchange="toggleSelectAll(this.checked)"></th>
    ...
    html += `<th class="col-checkbox" rowspan="${totalRows}"><input type="checkbox" id="select-all" onchange="toggleSelectAll(this.checked)"></th>`;
    html += `<th class="col-expand" rowspan="${totalRows}"></th>`;
    html += `<th class="col-id" rowspan="${totalRows}">ID</th>`;
...
function createMainRow(exp, flagColumns) {
    ...
    row.innerHTML = `
        ...
        <td class="col-checkbox">
            <input type="checkbox" ${isSelected ? 'checked' : ''} onchange="toggleExperimentSelection('${exp.id}', this.checked); event.stopPropagation();" onclick="event.stopPropagation();">
        </td>
        <td class="col-expand">
...
function createExpandedRow(exp, numFlagColumns) {
    ...
    const totalColumns = 6 + numFlagColumns;  // drag, checkbox, expand, id, state, resources + flags
                ...
                <div style="display: flex; gap: 20px; align-items: flex-start; margin-bottom: 20px;">
                    <div class="detail-section" style="width: auto; min-width: 400px;">
                        ...
                        <div class="detail-grid" id="config-grid-${exp.id}">
                            ...
                            <span class="detail-value config-field" data-field="tool_path">${exp.tool_path || '-'}</span>

                            <span class="detail-label">Nodes x GPUs:</span>
                            <span class="detail-value">
                                <span class="config-field" data-field="nodes">${exp.nodes}</span> x <span class="config-field" data-field="gpus">${exp.gpus}</span>
                            </span>

                            <span class="detail-label">Git Branch:</span>
                            <span class="detail-value config-field" data-field="git_branch">${exp.git_branch || '-'}</span>
                        </div>
                    ...
                    <div class="detail-section" style="width: auto; flex-shrink: 0;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            ...
                            <div style="display: flex; gap: 5px;">
                                <button class="btn btn-small" id="start-btn-${exp.id}" onclick="startExperiment('${exp.id}', event)" style="padding: 3px 8px; font-size: 11px;">Start</button>
                                <button class="btn btn-small btn-danger" id="stop-btn-${exp.id}" onclick="stopExperiment('${exp.id}', event)" style="padding: 3px 8px; font-size: 11px;">Stop</button>
                            </div>
                        </div>
                        <div id="jobs-${exp.id}" class="jobs-list">
...
async function toggleStar(experimentId, newStarredState) {
    try {
        // Update server
...
async function loadJobHistory(experimentId) {
    const container = document.getElementById(`jobs-${experimentId}`);
...
async function loadCheckpoints(experimentId) {
    ...
    try {
        ...
        const rows = checkpoints.map(cp => {
            ...
            const mettaUrl = `metta://policies/${experimentId}:v${cp.epoch}`;

            const s3Pill = s3Url
                ? `<span class="storage-pill" style="${pillStyle} background-color: #E8F5E9; color: #2E7D32;" onclick="event.stopPropagation(); copyToClipboard('${s3Url.replace(/'/g, "\\'")}', 'S3 URL copied!'); return false;" title="Copy S3 URL">s3</span>`
                : '';

            const mettaPill = `<span class="storage-pill" style="${pillStyle} background-color: #E3F2FD; color: #1565C0;" onclick="event.stopPropagation(); copyToClipboard('${mettaUrl.replace(/'/g, "\\'")}', 'Metta URL copied!'); return false;" title="Copy Metta URL">metta</span>`;

            const obsUrl = cp.observatory_url || '';
            const obsPill = obsUrl
                ? `<span class="storage-pill" style="${pillStyle} background-color: #F3E5F5; color: #6A1B9A;" onclick="event.stopPropagation(); copyToClipboard('${obsUrl.replace(/'/g, "\\'")}', 'Observatory URL copied!'); return false;" title="Copy Observatory URL">obs</span>`
                : '';
...
async function deleteExperiment(experimentId) {
    // For now, delete immediately without confirmation
    // TODO: Add a non-modal confirmation UI
    try {
        await apiCall(`/experiments/${experimentId}`, {
            method: 'DELETE',
        });
        expandedRows.delete(experimentId);
        showNotification(`Deleted experiment ${experimentId}`, 'success');
        await loadData();
    } catch (error) {
...
async function bulkDelete() {
    ...
    try {
        ...
        await Promise.all(promises);
        selectedExperiments.clear();
        showNotification(`Deleted ${count} experiments`, 'success');
        await loadData();
    } catch (error) {
