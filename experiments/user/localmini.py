from experiments.recipes import arena, navigation
...
# Can run this locally with `./tools/run.py experiments.user.localmini.train`
# Can't run this on skypilot because your experiments user file is not pushed to git
def train(run: str = "local.localmini.1") -> TrainTool:
    env = navigation.make_env()
    env.game.max_steps = 100
    cfg = arena.train(curriculum=navigation.make_curriculum(env))
    return cfg
...
def evaluate(run: str = "local.localmini.1") -> SimTool:
