...
uv run ./tools/run.py sim navigation policy_uri=file://./train_dir/test_$TEST_ID/checkpoints

# Using wandb artifact
uv run ./tools/run.py sim arena policy_uri=wandb://run/test_$TEST_ID
```
...
uv run ./tools/run.py train arena run=cursor_$TEST_ID trainer.total_timesteps=100000
uv run ./tools/run.py sim arena policy_uri=file://./train_dir/cursor_$TEST_ID/checkpoints
uv run ./tools/run.py analyze arena eval_db_uri=./train_dir/eval_$TEST_ID/stats.db
```
...
uv run ./tools/run.py sim navigation policy_uri=POLICY_URI
...
The new system uses recipe-based tool invocation:

- **Training**: `train arena` or `train navigation`
- **Evaluation**: `sim arena` or `sim navigation`
