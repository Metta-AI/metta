...
behaviour. When protocol_details_obs=True, converters show their protocol inputs
...
class TestConverterObservations:
    """Test converter observations with and without protocol_details_obs."""

    def create_converter_env(self, protocol_details_obs=False):
    ...
    def test_converter_obs_without_protocol_details(self):
        """Test that converters don't show protocol details when protocol_details_obs=False."""
        env = self.create_converter_env(protocol_details_obs=False)
        obs, _ = env.reset()
        agent_obs = obs[0]
        ...
        assert len(inventory_features) == 0, (
            "No inventory/protocol features should be present when protocol_details_obs=False"
        )

    def test_converter_obs_with_protocol_details(self):
        """Test that converters show protocol details when protocol_details_obs=True."""
        env = self.create_converter_env(protocol_details_obs=True)
        obs, _ = env.reset()
        agent_obs = obs[0]
        ...
        assert ore_red_input_id in feature_map, f"Should have ore_red protocol input at offset {ore_red_input_id}"
        assert ore_blue_input_id in feature_map, f"Should have ore_blue protocol input at offset {ore_blue_input_id}"
        ...
        # Protocol outputs are shown at dynamic offsets
        assert battery_red_output_id in feature_map, (
            f"Should have battery_red protocol output at offset {battery_red_output_id}"
        )
        assert feature_map[battery_red_output_id] == 1, (
            f"battery_red output should be 1, got {feature_map[battery_red_output_id]}"
        )

    def test_multiple_converters_different_protocol_details(self):
        """Test that different converters show their own protocol details correctly."""
        env = self.create_converter_env(protocol_details_obs=True)
        obs, _ = env.reset()
        agent_obs = obs[0]
        ...
        # Extract protocol features for generator
        gen_feature_map = {token[1]: token[2] for token in generator_tokens}

        # Extract protocol features for altar
        ...
        # Verify the protocol differences
