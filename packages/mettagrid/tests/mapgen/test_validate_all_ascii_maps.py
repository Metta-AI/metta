...
class TestAsciiMap:
    ...
    def test_uses_known_symbols(self, content, map_file, char_to_name):
        ...
        map_chars = {char for line in map_lines for char in line}
        allowed = set(char_to_name.keys()) | set(legend_map.keys())
        unknown_chars = map_chars - allowed
    ...
    def test_has_consistent_line_lengths(self, content, map_file):
        ...
        map_lines, _ = parse_ascii_map(content)

        line_lengths = [len(line) for line in map_lines]
    ...
    def test_loads_as_numpy_array(self, content, map_file, char_to_name):
        ...
        map_lines, legend_map = parse_ascii_map(content)
        combined_map = char_to_name | legend_map

        level_array = np.array([list(line) for line in map_lines], dtype="U6")
        _mapped_array = np.vectorize(combined_map.get)(level_array)

        # Basic structure validation
        assert level_array.ndim == 2, "Should be 2D array"
        assert level_array.shape[0] == len(map_lines), "Should have correct row count"
