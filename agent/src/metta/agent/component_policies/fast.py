...
class Fast(nn.Module):
    """
    Fast component-based policy built directly from YAML config, aligned with ComponentPolicy.
    """
    ...
    def __init__(
        ...
        # Directly instantiate PyTorch components with their parameters
        components = {
            ...
            "_action_embeds_": ActionEmbedding(
                ...
                num_embeddings=100,
                nn_params=DictConfig({"embedding_dim": 16, "num_embeddings": 100}),
                sources=None,
            ),
        ...
        # Setup components using ComponentPolicy's approach
        ...
        for name, component in self.components.items():
            if not getattr(component, "ready", False):
                raise RuntimeError(f"Component {name} in Fast policy was never setup.")
        ...
        logger.info(f"Fast policy components: {self.components}")

        # Initialize action conversion tensors (will be set by MettaAgent)
        self.cum_action_max_params = None
        self.action_index_tensor = None

        logger.info(f"Fast policy initialized with components: {list(self.components.keys())}")

    def _setup_components(self, component):
        """Setup component connections - matching ComponentPolicy logic."""
    ...
    def forward(self, td: TensorDict, state=None, action: Optional[torch.Tensor] = None) -> TensorDict:
        """Forward pass - matching ComponentPolicy structure."""

        # Handle BPTT reshaping like ComponentPolicy
        if td.batch_dims > 1:
            B = td.batch_size[0]
            TT = td.batch_size[1]
        ...
        # Run the value and action components (like ComponentPolicy)
    ...
    def forward_inference(self, td: TensorDict) -> TensorDict:
        """Sample actions for inference - matching ComponentPolicy."""
        value = td["_value_"]
        logits = td["_action_"]
    ...
    def forward_training(self, td: TensorDict, action: torch.Tensor) -> TensorDict:
        """Evaluate actions for training - matching ComponentPolicy."""
        value = td["_value_"]
        logits = td["_action_"]
    ...
    # ============================================================================
    # Memory-related Methods (matching ComponentPolicy)
    # ============================================================================
    ...
    # ============================================================================
    # Weight/Training Utility Methods (matching ComponentPolicy)
    # ============================================================================
    ...
    # ============================================================================
    # Helper Methods (matching ComponentPolicy)
    # ============================================================================
