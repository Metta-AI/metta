"""
Improved transformer agent with experimental enhancements.
...
4. Learnable positional embeddings option
5. GELU activation and other modern transformer improvements
6. Bilinear action decoding with embeddings
"""
...
from metta.agent.modules.transformer_module_improved import TransformerModuleImproved
...
class ImprovedPolicy(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Creating TransformerModuleImproved with hidden_size={hidden_size}, n_heads={n_heads}, n_layers={n_layers}"
        )
        self._transformer = TransformerModuleImproved(
            d_model=hidden_size,
            n_heads=n_heads,
            n_layers=n_layers,
            d_ff=d_ff,
        ...
        logger.info("TransformerModuleImproved created successfully")
