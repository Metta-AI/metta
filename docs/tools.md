...
metrics tracking. **Key Features**:

- Distributed training across multiple GPUs/nodes
- Wandb integration for experiment tracking
- Checkpoint saving and policy versioning
- Configurable evaluation during training **Usage**:
...
Initialize hyperparameter sweep experiments using Wandb sweeps and Metta Protein optimization. **Usage**:
...
Evaluate policies generated during hyperparameter sweeps and update Protein observations. **Usage**:
...
Run comprehensive policy evaluation with simulation suites and statistics export. **Usage**:
...
Generate replay files for detailed post-hoc analysis in MettaScope. **Usage**:
...
Interactive gameplay interface allowing humans to control Metta agents. **Usage**:
...
Real-time visualization of agent behavior with ASCII or Miniscope rendering. **Usage**:
...
Generate dashboard data for web-based visualization of training runs and evaluations. **Usage**:
...
View stored maps from various sources (local files, S3, etc.). **Usage**:
...
character-to-name mappings. **Usage**:
...
Normalize ASCII map characters to ensure consistency across different encodings. **Usage**:
...
Load and validate Hydra configuration files for debugging config issues. **Usage**:
...
Auto-tune vectorization parameters for optimal performance using pufferlib. **Usage**:
