...
def forward(self, td: TensorDict, action: torch.Tensor = None) -> TensorDict:
    """Forward pass for the QContrastive policy."""
    self.network(td)
...
class QContrastiveConfig(PolicyArchitecture):
    ...
    components: List[ComponentConfig] = [
        ...
        ActionEmbeddingConfig(out_key="action_embedding", embedding_dim=_actor_embed_dim),
        ActorQueryConfig(in_key="core", out_key="actor_query", hidden_size=_lstm_latent, embed_dim=_actor_embed_dim),
        ActorKeyConfig(
            query_key="actor_query",
            embedding_key="action_embedding",
            out_key="logits",
            embed_dim=_actor_embed_dim,
        ),
    ]
    ...
    def make_policy(self, env_metadata: EnvironmentMetaData) -> Policy:
        ...
        pred_input_dim = self._lstm_latent + self._actor_embed_dim
