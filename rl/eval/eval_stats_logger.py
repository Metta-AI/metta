...
class EvalStatsLogger:
    def __init__(self, cfg, wandb_run, save_path: str=None):
        self._cfg = cfg
    ...
    def _add_additional_fields(self, eval_stats, game_cfg, eval_name = "eval"):
        additional_fields = {}
        ...
        flattened_env = flatten_config(game_cfg)
    ...
    def log(self, eval_stats, game_cfg):
        ...
        if isinstance(eval_stats, dict):
            ...
            for eval_name, stats in eval_stats.items():
                self._add_additional_fields(stats, game_cfg, eval_name = eval_name)
                eval_suite_stats.extend(stats)
            eval_stats = eval_suite_stats
        else:
            self._add_additional_fields(eval_stats, game_cfg)
