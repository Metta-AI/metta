"""Simple job queue and dispatcher for parallel job execution.

Internal utility used by the release system and other devops scripts to run
multiple jobs (local or remote) in parallel with status tracking.
...
Example usage:
    ...
    # Create dispatcher
    dispatcher = JobDispatcher(name="validation_batch")

    # Add jobs
    for validation in validations:
        job = RemoteJob(name=validation.name, module=validation.module, args=validation.args)
