...
static void BM_GridLocationCreation(const benchmark::State& state) {
  for (auto _ : state) {
...
static void BM_GridObjectInitWithLocation(const benchmark::State& state) {
  for (auto _ : state) {
    TestGridObject obj;
    GridLocation loc(10, 20, 1);
    obj.init(5, loc);
...
static void BM_GridObjectInitWithCoordinates(const benchmark::State& state) {
  for (auto _ : state) {
    TestGridObject obj;
    obj.init(5, 10, 20);
    benchmark::DoNotOptimize(&obj);
...
static void BM_GridObjectInitWithCoordinatesAndLayer(const benchmark::State& state) {
  for (auto _ : state) {
    TestGridObject obj;
    obj.init(5, 10, 20, 1);
    benchmark::DoNotOptimize(&obj);
...
static void BM_GridObjectObs(const benchmark::State& state) {
...
static void BM_CreateManyObjects(const benchmark::State& state) {
...
static void BM_GridLookup(const benchmark::State& state) {
