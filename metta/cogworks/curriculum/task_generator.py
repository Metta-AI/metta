...
class TaskGenerator(ABC):
    ...
    def __init__(self, config: TaskGeneratorConfig):
        ...
        self._reference_task: MettaGridConfig | None = None

    def get_task(self, task_id: int) -> MettaGridConfig:
        """Generate a task (MettaGridConfig) using task_id as seed."""
        rng = random.Random()
        rng.seed(task_id)
        mg_config = self._apply_overrides(self._generate_task(task_id, rng), self._config.overrides)
    ...
    def _validate_task_invariants(self, mg_config: MettaGridConfig, task_id: int) -> None:
        ...
        Args:
            mg_config: The generated task configuration to validate
        ...
        current_action_count = sum(
            1 for action in mg_config.game.actions.model_dump().values() if action.get("enabled", True)
        )
        ref_action_count = sum(1 for action in ref.game.actions.model_dump().values() if action.get("enabled", True))
