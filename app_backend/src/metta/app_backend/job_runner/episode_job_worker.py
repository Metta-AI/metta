...
def main():
    ...
    try:
        ...
        stats_client.update_job(job_id, JobRequestUpdate(status=JobStatus.running, worker=pod_id))
        logger.info(f"Started job {job_id} on pod {pod_id}")
        ...
        for policy_uri in job.policy_uris:
            ...
            if parsed.scheme != "metta":
                ...
                policy_version = MettaSchemeResolver().get_policy_version(policy_uri)
