...
def test_puffer_env():
    ...
    for step in range(20):
        ...
        actions = np.random.randint(0, min(3, env.single_action_space.nvec.max()), 
                                   size=(env.num_agents, 2), dtype=np.int32)
...
def test_gym_env():
    ...
    for step in range(20):
        ...
        action = np.random.randint(0, min(3, env.action_space.nvec.max()), 
...
def test_pettingzoo_env():
    ...
    for step in range(20):
        ...
        for agent in env.agents:
            actions[agent] = np.random.randint(0, min(3, env.action_space.nvec.max()), 
                                             size=2, dtype=np.int32)
