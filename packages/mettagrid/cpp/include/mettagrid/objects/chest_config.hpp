...
struct ChestConfig : public GridObjectConfig {
  ...
  std::string chest_id;  // Unique identifier for per-chest stat tracking
};
...
inline void bind_chest_config(py::module& m) {
  py::class_<ChestConfig, GridObjectConfig, std::shared_ptr<ChestConfig>>(m, "ChestConfig")
      .def(py::init<TypeId, const std::string&, InventoryItem, const std::set<int>&, const std::set<int>&, const std::vector<int>&, const std::string&>(),
           py::arg("type_id"),
           py::arg("type_name"),
      ...
      .def_readwrite("chest_id", &ChestConfig::chest_id);
}
