# Test runner for C++ observations tests
import subprocess
import sys
...
def run_test():
    ...
    test_bin = mettagrid_dir / "bazel-bin" / "test_observations_bin"
    if not test_bin.exists():
        # Try without _bin suffix
        test_bin = mettagrid_dir / "bazel-bin" / "test_observations"
    if not test_bin.exists():
        # Try in current directory as fallback
        test_bin = Path("test_observations_bin")
