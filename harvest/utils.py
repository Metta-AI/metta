...
def find_direction_to_object_from_obs(
    obs: AgentObservation,
    ...
    *,
    ...
    """Find direction to an adjacent object matching the resource pattern.

    Checks the 4 cardinal directions from the agent's position in the observation
    for an object whose tag name contains the resource_pattern (e.g., "germanium").

    Returns: "north", "south", "east", or "west" if found, None otherwise.
    """
    # Direction offsets from center (obs_hr, obs_wr)
    directions = {
        "north": (-1, 0),
        "south": (1, 0),
        "east": (0, 1),
    ...
    # Build a map of positions to tag names from observation tokens
    position_tags: dict[tuple[int, int], list[str]] = {}
    for tok in obs.tokens:
        if tok.feature.name == "tag":
            pos = tok.location
            tag_name = tag_names.get(tok.value, "").lower()
            if pos not in position_tags:
                position_tags[pos] = []
            position_tags[pos].append(tag_name)

    # Check each adjacent position for matching object
    for direction, (dr, dc) in directions.items():
        adj_pos = (obs_hr + dr, obs_wr + dc)
        tags = position_tags.get(adj_pos, [])
        for tag in tags:
            if resource_pattern.lower() in tag and "extractor" in tag:
                return direction
