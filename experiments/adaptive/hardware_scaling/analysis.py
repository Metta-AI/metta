...
class HardwareScalingAnalyzer:
    ...
    def fit_scaling_laws(self, df: pd.DataFrame) -> Dict[str, Tuple[float, float]]:
        ...
        if "samples_to_target" in df.columns:
            ...
            try:
                ...
                logger.info(f"Samples scaling: S = {params[0]:.2e} * GPUs^{params[1]:.3f}")
    ...
    def calculate_fom(
        ...
        df["sample_efficiency_norm"] = df["sample_efficiency"] / df["sample_efficiency"].max()
    ...
    def generate_report(self, df: pd.DataFrame) -> str:
        ...
        report.append(f"Most sample-efficient: {df.loc[df['sample_efficiency'].idxmax()]['hardware_config']}")
        report.append(f"Fastest to target: {df.loc[df['time_efficiency'].idxmax()]['hardware_config']}")
        report.append(f"Most cost-effective: {df.loc[df['cost_efficiency'].idxmax()]['hardware_config']}")
        report.append("")
        ...
        for scenario, config in recommendations.items():
            ...
            report.append(f"  Hardware: {config['hardware']} ({config['gpus']} GPUs on {config['nodes']} nodes)")
            report.append(f"  Expected score: {config['expected_score']:.4f}")
            report.append(f"  Time to target: {config['expected_time_hours']:.2f} hours")
    ...
    def _extract_hardware_id(self, run_name: str) -> Optional[str]:
        ...
        for i, part in enumerate(parts):
            if part.startswith("g") and i + 1 < len(parts) and parts[i + 1].startswith("n"):
    ...
    def _analyze_hardware_config(
        ...
        for run in runs:
            if run.state == "finished" and run.summary:
                score = run.summary.get("sweep/score", run.summary.get("evaluator/eval_arena/score", 0))
                if score > best_score:
                    best_score = score
                    best_run = run
        ...
        best_hyperparams = {
            ...
            "learning_rate": config.get("trainer", {}).get("optimizer", {}).get("learning_rate"),
            "ppo_clip": config.get("trainer", {}).get("losses", {}).get("ppo", {}).get("clip_coef"),
            "value_clip": config.get("trainer", {}).get("losses", {}).get("value", {}).get("clip_coef"),
            "gae_lambda": config.get("trainer", {}).get("losses", {}).get("value", {}).get("gae_lambda"),
        }
        ...
        time_to_target = summary.get("time_to_target", best_run.runtime / 3600 if best_run.runtime else 1.0)
        total_cost = summary.get("sweep/cost", gpus * nodes * time_to_target * 1.5)  # $1.5 per GPU-hour
