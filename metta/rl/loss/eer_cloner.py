...
class EERClonerConfig(LossConfig):
    ...
    r_lambda: float = Field(default=0.002, ge=0)  # scale the teacher log likelihoods that are added to rewards

    # Probability floor for teacher actions not taken (to avoid log(0) = -inf). note that this gets mult by r_lambda too
    teacher_prob_floor: float = Field(default=0.1, ge=1e-6, le=1.0)
