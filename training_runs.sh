...
#RUNNING ON METTA1: georgedeane.devbox.nav_mem_pretrained
...
./devops/skypilot/launch.py train georgedeane.sky.nav_mem_pretrained trainer.curriculum=env/mettagrid/curriculum/navsequence/nav_mem trainer.initial_policy.uri=gd_backchain_mem_pretrained:v18
...
python -m devops.aws.batch.launch_task --cmd=train --run=b.georgdeane.nav_backchain_mem_pretrained  --git-branch=george-navsequence-experiments trainer.curriculum=env/mettagrid/curriculum/navsequence/nav_backchain_mem trainer.initial_policy.uri=wandb://run/gd_backchain_mem_pretrained:v18 --skip-validation \
