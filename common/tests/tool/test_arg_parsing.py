...
import metta.common.tool.run_tool
...
def test_parse_value_types():
    ...
    # Booleans
    assert metta.common.tool.run_tool.parse_value("true") is True
    assert metta.common.tool.run_tool.parse_value("FALSE") is False

    # Integers
    assert metta.common.tool.run_tool.parse_value("42") == 42
    assert metta.common.tool.run_tool.parse_value("-100") == -100

    # Floats
    assert metta.common.tool.run_tool.parse_value("3.14") == 3.14

    # Strings remain strings when not parseable
    assert metta.common.tool.run_tool.parse_value("hello") == "hello"
    assert metta.common.tool.run_tool.parse_value("42abc") == "42abc"
    assert metta.common.tool.run_tool.parse_value("") == ""


def test_parse_value_json_structures():
    """Verify JSON objects and arrays are parsed correctly."""
    assert metta.common.tool.run_tool.parse_value('{"k":1}') == {"k": 1}
    assert metta.common.tool.run_tool.parse_value("[1,2,3]") == [1, 2, 3]

    # Invalid JSON falls back to string
    assert metta.common.tool.run_tool.parse_value("{not json}") == "{not json}"

    # Oversized JSON-like content returns original string (security limit)
    big = "{" + "x" * 1_000_001 + "}"
    result = metta.common.tool.run_tool.parse_value(big)
...
def test_parse_cli_args_basic():
    ...
    result = metta.common.tool.run_tool.parse_cli_args(["name=test", "count=42", "enabled=true", "nested.field=value"])
...
def test_parse_cli_args_handles_special_cases():
    ...
    # Empty value
    result = metta.common.tool.run_tool.parse_cli_args(["key="])
    assert result["key"] == ""

    # Value containing equals signs
    result = metta.common.tool.run_tool.parse_cli_args(["url=http://example.com?a=1&b=2"])
...
def test_parse_cli_args_rejects_invalid_format():
    ...
    with pytest.raises(ValueError, match="Invalid argument format"):
        metta.common.tool.run_tool.parse_cli_args(["no_equals_sign"])

    with pytest.raises(ValueError, match="Invalid argument format"):
        metta.common.tool.run_tool.parse_cli_args([""])
...
def test_nestify_converts_flat_to_nested():
    ...
    nested = metta.common.tool.run_tool.nestify(flat)
...
def test_get_tool_fields_includes_parent_fields():
    ...
    fields = metta.common.tool.run_tool.get_tool_fields(SimpleTestTool)
...
def test_classify_remaining_args_separates_known_and_unknown():
    ...
    tool_fields = metta.common.tool.run_tool.get_tool_fields(SimpleTestTool)
    ...
    overrides, unknown = metta.common.tool.run_tool.classify_remaining_args(remaining, tool_fields)
