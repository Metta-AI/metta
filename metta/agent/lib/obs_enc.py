...
class ObsLatentAttnBuiltIn(LayerBase):
    ...
    def _make_net(self) -> None:
        ...
        if self._query_token_dim % self._num_heads != 0:
            raise ValueError(
                f"query_token_dim ({self._query_token_dim}) must be divisible by num_heads ({self._num_heads})"
            )

        self._q_token = nn.Parameter(torch.randn(1, self._num_query_tokens, self._query_token_dim))
        nn.init.trunc_normal_(self._q_token, std=0.02)

        # Input projection for keys and values
        self.norm_kv = nn.LayerNorm(self._feat_dim)
        if self._feat_dim != self._query_token_dim:
            self.kv_proj = nn.Linear(self._feat_dim, self._query_token_dim)
        else:
            ...
            layer = nn.ModuleDict(
                {
                    "multihead_attn": nn.MultiheadAttention(
                        embed_dim=self._query_token_dim, num_heads=self._num_heads, batch_first=True
                    ),
                    "norm1": nn.LayerNorm(self._query_token_dim),
                    "norm2": nn.LayerNorm(self._query_token_dim),
                    "mlp": nn.Sequential(
    ...
    def _forward(self, td: TensorDict) -> TensorDict:
        ...
        for layer in self.layers:
            ...
            # PyTorch's MultiheadAttention expects (query, key, value) all with batch_first=True
            # query: [B_TT, num_query_tokens, query_token_dim]
            # key, value: [B_TT, M, query_token_dim]
            attn_output, _ = layer["multihead_attn"](
                query=queries_norm, key=kv, value=kv, key_padding_mask=key_padding_mask, need_weights=False
            )
...
class ObsLatentAttnNested(LayerBase):
    """
    Performs multi-layer cross-attention between learnable query tokens and input features.
    ...
    def _forward(self, td: TensorDict) -> TensorDict:
        ...
        for layer in self.layers:
            ...
            # attn_scores = torch.einsum("bhqd,bhkd->bhqk", q_p, k_p) * self._scale
            attn_scores = torch.matmul(q_p, k_p.transpose(-2, -1)) * self._scale
...
class ObsLatentAttnNestedRaw(LayerBase):
    """A variant of :class:`ObsLatentAttnNested` whose attention computation follows the
    reference `mha_nested` implementation as closely as possible while retaining the
    original interface (nested input observations, learnable query tokens, optional
    cls-token output).
    """
    ...
    # ---------------------------------------------------------------------
    def _make_net(self) -> None:
        # expected input: [B, M, feat_dim] where M is jagged
        self._feat_dim = self._in_tensor_shapes[0][1]
        if self._query_token_dim is None:
            self._query_token_dim = self._feat_dim
        ...
        # learnable query tokens (shared across batch)
        self._q_token = nn.Parameter(torch.randn(1, self._num_query_tokens, self._query_token_dim))
        nn.init.trunc_normal_(self._q_token, std=0.02)

        E_total = self._query_token_dim  # total embedding across all heads

        # projection weights (follow `mha_nested` signature)
        self.W_q = nn.Parameter(torch.empty(E_total, self._query_token_dim))
        self.b_q = nn.Parameter(torch.zeros(E_total))

        self.W_k = nn.Parameter(torch.empty(E_total, self._feat_dim))
        self.b_k = nn.Parameter(torch.zeros(E_total))

        self.W_v = nn.Parameter(torch.empty(E_total, self._feat_dim))
        self.b_v = nn.Parameter(torch.zeros(E_total))

        self.W_out = nn.Parameter(torch.empty(self._out_dim, E_total))
        self.b_out = nn.Parameter(torch.zeros(self._out_dim))

        # initialise weights
        for param in [self.W_q, self.W_k, self.W_v, self.W_out]:
            nn.init.xavier_uniform_(param)

        # pre-compute scaling factor for attention logits
        self._scale = (E_total // self._num_heads) ** -0.5
    ...
    def _forward(self, td: TensorDict) -> TensorDict:
        ...
        # convert padded observation tensor into nested tensor so we can support jagged seq len per batch element
        obs_per_batch = torch.unbind(x_features, dim=0)
        nested_list = []
        for obs, flip_idx in zip(obs_per_batch, flip_pts, strict=False):
            nested_list.append(obs[:flip_idx])
        x_features = torch.nested.nested_tensor(
            nested_list, dtype=torch.float, device=x_features.device, layout=torch.jagged
        )

        # # prepare fixed-length query tokens for the entire batch
        queries = self._q_token.expand(B_TT, -1, -1)  # [B_TT, num_query_tokens, E_q]

        # queries = torch.nested.nested_tensor(queries.unbind(dim=0), dtype=torch.float, device=x_features.device,
        #                                      layout=torch.jagged)

        # linear projections (implemented via F.linear with pre-defined weights)
        q_proj = F.linear(queries, self.W_q, self.b_q)  # [B_TT, Q, E_total]
        k_proj = F.linear(x_features, self.W_k, self.b_k)  # [B_TT, M, E_total]
        v_proj = F.linear(x_features, self.W_v, self.b_v)  # [B_TT, M, E_total]

        # reshape for multi-head attention
        head_dim = self._query_token_dim // self._num_heads
        q_proj = einops.rearrange(q_proj, "b q (h d) -> b h q d", h=self._num_heads)  # [B, H, Q, D]
        k_proj = einops.rearrange(k_proj, "b m (h d) -> b h m d", h=self._num_heads)  # [B, H, M, D]
        v_proj = einops.rearrange(v_proj, "b m (h d) -> b h m d", h=self._num_heads)  # [B, H, M, D]

        # attention computation
        attn_scores = torch.matmul(q_proj, k_proj.transpose(-2, -1)) * self._scale
        # attn_scores = torch.matmul(q_proj, k_proj) * self._scale  # [B, H, Q, M]
        attn_weights = F.softmax(attn_scores)

        attn_weights = torch.nested.to_padded_tensor(attn_weights, padding=0)
        v_proj = torch.nested.to_padded_tensor(v_proj, padding=0)

        attn_output = torch.matmul(attn_weights, v_proj)  # [B, H, Q, D]

        # merge heads
        attn_output = einops.rearrange(attn_output, "b h q d -> b q (h d)")  # [B, Q, E_total]

        # output projection
        attn_output = F.linear(attn_output, self.W_out, self.b_out)  # [B, Q, out_dim]

        if self._use_cls_token:
            attn_output = attn_output[:, 0]

        td[self._name] = attn_output
        return td
