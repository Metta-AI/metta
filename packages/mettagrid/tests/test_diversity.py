...
def test_calculate_diversity_bonus_three_groups():
    ...
    m_g0, s_g0 = (
        np.mean(episode_rewards[agent_groups == 0]),
        np.std(episode_rewards[agent_groups == 0]) + 1e-6,
    )
    m_g1, s_g1 = (
        np.mean(episode_rewards[agent_groups == 1]),
        np.std(episode_rewards[agent_groups == 1]) + 1e-6,
    )
    m_g2, s_g2 = (
        np.mean(episode_rewards[agent_groups == 2]),
        np.std(episode_rewards[agent_groups == 2]) + 1e-6,
    )
    ...
    factors.append(
        1 + similarity_coef * sim_0 + diversity_coef * np.mean([div_0_g1, div_0_g2])
    )
    ...
    factors.append(
        1 + similarity_coef * sim_1 + diversity_coef * np.mean([div_1_g1, div_1_g2])
    )
    ...
    factors.append(
        1 + similarity_coef * sim_2 + diversity_coef * np.mean([div_2_g0, div_2_g2])
    )
    ...
    factors.append(
        1 + similarity_coef * sim_3 + diversity_coef * np.mean([div_3_g0, div_3_g2])
    )
    ...
    factors.append(
        1 + similarity_coef * sim_4 + diversity_coef * np.mean([div_4_g0, div_4_g1])
    )
    ...
    factors.append(
        1 + similarity_coef * sim_5 + diversity_coef * np.mean([div_5_g0, div_5_g1])
    )
...
def test_zero_total_diversity_score_when_agent_reward_equals_all_other_group_means():
    ...
    expected_factor_agent0 = (
        1 + similarity_coef * similarity_score_agent0
    )  # + diversity_coef * 0
