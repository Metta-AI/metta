...
from metta.rl.model_analysis import compute_dead_neuron_stats, compute_saturated_activation_stats
...
def test_compute_dead_neuron_stats_counts_layers() -> None:
    policy = _TinyPolicy()
    stats = compute_dead_neuron_stats(policy, threshold=1e-8)

    assert stats["weights/dead_neurons/total_neurons"] == 5.0
    assert stats["weights/dead_neurons/dead_neurons"] == 4.0
    assert stats["weights/dead_neurons/fraction"] == 0.8
    assert stats["weights/dead_neurons/active_fraction"] == pytest.approx(0.2)
    # Ensure layer-specific entries exist and use sanitized names
    assert stats["weights/dead_neurons/layer1"] == 1.0
    assert stats["weights/dead_neurons/layer2"] == 0.5
