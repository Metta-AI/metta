...
def main() -> None:
    print("=" * 80)
    print("Axons (streaming RTU, diagonal) Triton vs PyTorch Benchmark")
    print("=" * 80)
    print()
    ...
    print("Configuration format: (batch, seq_len, hidden, resets, p)")
    print()
    print(f"{'Config':<48} {'PyTorch (ms)':<15} {'Triton (ms)':<15} {'Speedup':<10} {'Max Diff':<12}")
    print("-" * 110)

    for b, t, h, use_resets, p in configs:
