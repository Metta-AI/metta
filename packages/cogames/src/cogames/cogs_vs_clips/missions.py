...
class NeutralFacedVariant(MissionVariant):
    ...
    def apply(self, mission: Mission) -> Mission:
        def modifier(cfg: MettaGridConfig) -> None:
            ...
            # Also allow heart crafting without specific emote requirements
            ...
            if isinstance(assembler_cfg, AssemblerConfig):
                ...
                for protocol in assembler_cfg.protocols:
                    # Keep exactly one heart protocol and make it default (no vibe requirement)
                    if protocol.output_resources.get("heart", 0) > 0:
                        if not kept:
                            protocol.vibes = []
                            filtered.append(protocol)
                            kept = True
                        # Drop additional heart protocols to avoid duplicate default vibe entries
                    # Drop non-heart protocols to ensure no duplicate default vibe combinations
                if kept:
...
class PerceptionShareVariant(MissionVariant):
    name: str = "perception_share"
    description: str = (
        "Reward based on perceived chest heart amounts; optionally share observed reward gains between agents."
    )

    def apply(self, mission: Mission) -> Mission:
        def modifier(cfg: MettaGridConfig) -> None:
            # Use the per-agent belief for group reward, which propagates via observations
            num = max(1, cfg.game.num_agents)
            cfg.game.agent.rewards.stats = {"belief.group_reward": 1.0 / num}
            # Enable a small social propagation effect by default; can be tuned per experiment
            if not hasattr(cfg.game, "social_reward_share_fraction"):
                cfg.game.social_reward_share_fraction = 0.0
...
class NoDungeonsVariant(MissionVariant):
    name: str = "no_dungeons"
    description: str = "Disable dungeon overlays."

    def apply(self, mission: Mission) -> Mission:
        # Ensure dungeon layers are not generated by zeroing all dungeon weights
        mission.procedural_overrides.update(
            {
...
class NoBiomesVariant(MissionVariant):
    name: str = "no_biomes"
    description: str = "Disable biome overlays."

    def apply(self, mission: Mission) -> Mission:
        mission.procedural_overrides.update(
            {
                "biome_weights": {"caves": 0.0, "desert": 0.0, "forest": 0.0, "city": 0.0},
                "max_biome_zone_fraction": 0.0,
            }
        )
        return mission


class DistributionExhibitionVariant(MissionVariant):
    name: str = "distribution_exhibition"
    description: str = "Showcase all building placement distributions on a minimal arena."

    def apply(self, mission: Mission) -> Mission:
        mission = NoBiomesVariant().apply(mission)
        mission = NoDungeonsVariant().apply(mission)
...
class BigCityVariant(MissionVariant):
    name: str = "big_city"
    description: str = "Make the city blocks bigger."

    def apply(self, mission: Mission) -> Mission:
        mission.procedural_overrides["max_biome_zone_fraction"] = 0.95
        return mission
...
VARIANTS = [
    ...
    ClipRateOnVariant,
    NoBiomesVariant,
    DistributionExhibitionVariant,
    PerceptionShareVariant,
    # HeartChorusVariant,
]
...
class AssembleMissionNew(Mission):
    name: str = "assemble_new"
    description: str = "Make HEARTs by using the assembler. Coordinate your team to maximize efficiency."
    site: Site = TRAINING_FACILITY

    class Variant(MissionVariant):
        def apply(self, mission: Mission) -> Mission:
            for v in [
                SmallMapVariant(),
                NoBiomesVariant(),
