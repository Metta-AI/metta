...
logger = logging.getLogger("cogames.policies.lstm_model_based_policy")


class LSTMModelBasedPolicyNet(torch.nn.Module):
    def __init__(self, env):
        super().__init__()
        # Public: Required by PufferLib for RNN state management
        self.hidden_size = 128

        self._flattened_obs_dim = int(np.prod(env.single_observation_space.shape))

        self._net = torch.nn.Sequential(
            pufferlib.pytorch.layer_init(torch.nn.Linear(self._flattened_obs_dim, self.hidden_size)),
            torch.nn.ReLU(),
        ...
        self._next_obs_head = torch.nn.Linear(self.hidden_size, self._flattened_obs_dim)
...
class LSTMModelBasedAgentPolicy(StatefulAgentPolicy[LSTMState]):
    """Per-agent policy that uses the shared LSTM network."""

    def __init__(self, net: LSTMModelBasedPolicyNet, device: torch.device, num_actions: int):
        self._net = net
        self._device = device
...
class LSTMModelBasedPolicy(TrainablePolicy):
    ...
    def __init__(self, env: MettaGridEnv, device: torch.device):
        super().__init__()
        self._net = LSTMModelBasedPolicyNet(env).to(device)
        self._device = device
        self._num_actions = int(env.single_action_space.n)
