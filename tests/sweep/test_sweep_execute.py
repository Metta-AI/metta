"""Test sweep execute orchestration."""
...
pytest.skip("Skipping sweep execute tests due to torch import issues", allow_module_level=True)
...
try:
    import sweep_execute
except ImportError:
    sweep_execute = None


class TestSweepExecute:
    """Test sweep execute main functions."""
    ...
    @patch("sweep_execute.run_once")
    @patch("sweep_execute.run_single_rollout")
    @patch("sweep_execute.initialize_sweep")
    def test_main_successful_rollouts(self, mock_initialize, mock_run_single, mock_run_once, mock_config):
        ...
        from sweep_execute import main
        ...
        # Run main
        with patch.object(sys, "argv", ["sweep_execute.py"]):
    ...
    @patch("sweep_execute.time.sleep")
    @patch("sweep_execute.run_once")
    @patch("sweep_execute.run_single_rollout")
    @patch("sweep_execute.initialize_sweep")
    ...
    @patch("sweep_execute.OmegaConf.load")
    @patch("sweep_execute.evaluate_sweep_rollout")
    @patch("sweep_execute.launch_training_subprocess")
    @patch("sweep_execute.prepare_sweep_run")
    @patch("sweep_execute.run_once")
    def test_run_single_rollout_success(
        ...
        from sweep_execute import run_single_rollout
    ...
    def test_launch_training_subprocess(self, mock_subprocess_run):
        ...
        from sweep_execute import launch_training_subprocess
