...
def help_configs() -> None:
    ...
    info('artifact = load_policy_artifact_from_uri("file://./train_dir/my_run/checkpoints/my_run:v12.mpt")')
    info('artifact = load_policy_artifact_from_uri("s3://bucket/path/my_run/checkpoints/my_run:v12.mpt")')
    info('policy = artifact.policy  # or artifact.instantiate(policy_env_info, torch.device("cpu"))')
