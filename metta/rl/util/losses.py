...
def compute_contrastive_loss(
    ...
    current_indices = (segment_indices.unsqueeze(1) * bptt_horizon +
                      torch.arange(bptt_horizon, device=device)).view(-1)
    ...
    # adjust delta for episode boundaries using dones
    dones_flat = minibatch["dones"].view(-1)
    timesteps_in_segment = torch.arange(bptt_horizon, device=device).repeat(len(segment_indices))
    max_delta = torch.zeros_like(delta)
    ...
    for i in range(batch_size):
        ...
        if len(done_indices) > 0:
            max_delta[i] = done_indices[0]
        else:
            max_delta[i] = bptt_horizon - 1 - time_idx

    delta = torch.min(delta, max_delta)
    positive_indices = current_indices + delta

    # sample negatives
    num_negatives = trainer_cfg.contrastive.num_negatives
    negative_indices = torch.randint(0, all_lstm_flat.shape[0],
    ...
    for i in range(batch_size):
        ...
        while mask.any():
            negative_indices[i, mask] = torch.randint(0, all_lstm_flat.shape[0], (mask.sum(),), device=device)
            mask = (negative_indices[i] == current_indices[i]) | (negative_indices[i] == positive_indices[i])
...
def process_minibatch_update(
    ...
    if trainer_cfg.contrastive.enabled and hasattr(experience, 'lstm_outputs'):
        ...
        contrastive_loss, contrastive_metrics = compute_contrastive_loss(
            minibatch=minibatch,
            lstm_hidden=minibatch_lstm,
            all_lstm_hidden=experience.lstm_outputs,
    ...
    loss = (
        ...
        + ks_action_loss
        + ks_value_loss
        + trainer_cfg.contrastive.weight * contrastive_loss  # <- add this
    )
