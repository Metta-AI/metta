...
def wait_for_run(cfg: DictConfig | ListConfig, path: str, logger: Logger) -> None:
    """Block until the master node has written a non-null *run* entry.

    The worker node needs the concrete ``run`` identifier generated by the
    master (e.g. ``my_sweep.r.0``).  A placeholder file is created before the
    sweep starts so we *cannot* rely on the file's mere existence; we have to
    poll until the value becomes truthy.

    To avoid an infinite hang we use an upper bound of *max_wait* seconds –
    considerably longer than the time the master typically spends in
    ...
    raise TimeoutError(f"Timed out after {max_wait}s waiting for master to populate 'run' in {path}")
