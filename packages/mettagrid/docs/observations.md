...
| `protocol_input:{resource_name}`  | Required input resource amount for current protocol                             | assembler, extractors     | Only available if `protocol_details_obs` is enabled. One feature per resource   |
| `protocol_output:{resource_name}` | Output resource amount for current protocol                                     | assembler, extractors     | Only available if `protocol_details_obs` is enabled. One feature per resource   |
