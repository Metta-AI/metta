...
class RobustObsEncoderRev2(nn_layer_library.LayerBase):
    ...
    def _forward(self, td: TensorDict):
        ...
        for b in range(N):
            ...
            if num_features > 0:
                # --- Gumbel-Softmax based Feature Selection ---
                gumbel_tau = 1.0  # Gumbel temperature (often 1.0)
                score_temp = self.gumbel_temperature  # Temperature to scale scores
                logits = attn_scores / score_temp  # Scale scores

                num_slots = self.hidden // self.key_dim  # Should be 8
                assert self.hidden % self.key_dim == 0

                # Generate num_slots independent Gumbel-Softmax distributions
                # We repeat the logits and apply Gumbel-Softmax row-wise
                # logits shape: [num_features]
                # repeated_logits shape: [num_slots, num_features]
                repeated_logits = logits.unsqueeze(0).repeat(num_slots, 1)

                # Apply Gumbel-Softmax
                # gumbel_weights shape: [num_slots, num_features]
                gumbel_weights = F.gumbel_softmax(repeated_logits, tau=gumbel_tau, hard=False, dim=-1)

                # Compute weighted averages for each slot using the corresponding weights
                # value[:, :self.key_dim] has shape [num_features, key_dim]
                # matmul: [num_slots, num_features] @ [num_features, key_dim] -> [num_slots, key_dim]
                slot_values = torch.matmul(gumbel_weights, value[:, : self.key_dim])

                # Reshape and fill hidden_vec
                # slot_values shape [num_slots, key_dim] -> [num_slots * key_dim]
                hidden_vec = slot_values.reshape(-1)  # Shape [8 * key_dim]
