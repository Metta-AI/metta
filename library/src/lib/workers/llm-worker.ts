...
export class LLMWorker {
  ...
  private async processJob(job: Job<BackgroundJobs['generate-llm-abstract']>): Promise<void> {
    const { paperId } = job.data;
    
    console.log(`🤖 [LLM Worker] Generating abstract for paper ${paperId}`);

    try {
      const abstract = await PaperAbstractService.generateAbstractForPaper(paperId);
      
      if (abstract) {
        console.log(`✅ [LLM Worker] Generated abstract for paper ${paperId}`);
      } else {
        console.log(`⚠️ [LLM Worker] No abstract generated for paper ${paperId}`);
      }
    } catch (error) {
      console.error(`❌ [LLM Worker] Failed to generate abstract for paper ${paperId}:`, error);
