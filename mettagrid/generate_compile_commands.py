...
def parse_aquery_output(aquery_output):
    ...
    for block in action_blocks:
        ...
        for arg in arguments:
            if arg.endswith((".cpp", ".cc", ".cxx", ".c")) and not arg.startswith("-"):
                ...
                if arg.startswith("src/"):
                    ...
                    possible_paths = [arg, f"src/metta/mettagrid/{arg}", f"tests/{arg}", f"benchmarks/{arg}"]
        ...
        if source_file and os.path.exists(source_file):
            ...
            default_cmd = f"clang++ -std=c++20 -Isrc/metta/mettagrid -Isrc/metta -c {source_file}"
            command = {
                "directory": os.getcwd(),
                "file": os.path.abspath(source_file),
                "command": " ".join(clean_args) if clean_args else default_cmd,
            }
...
def generate_fallback_commands():
    ...
    python_include = subprocess.run(
        [python_cmd, "-c", "import sysconfig; print(sysconfig.get_path('include'))"], capture_output=True, text=True
    ).stdout.strip()

    # Get pybind11 include path
    pybind11_include = subprocess.run(
        [python_cmd, "-c", "import pybind11; print(pybind11.get_include())"], capture_output=True, text=True
    ).stdout.strip()
