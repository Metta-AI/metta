...
- **Team reward sharing** – Agents may belong to groups that share a configurable portion of their rewards with teammates.
- **Building interaction** – Agents deposit required inputs (`put_items`) and retrieve produced outputs (`get_items`). Buildings currently cannot be damaged and have no hitpoints.
...
- **Mines** (`mine_red`, `mine_blue`, `mine_green`) output one unit of the corresponding ore.
- **Generators** transform ore into matching batteries.
- **Armory** turns red ore into armor.
- **Lasery** converts red ore and batteries into lasers.
- **Lab** consumes red ore and batteries to create blueprints.
- **Factory** uses a blueprint plus ore and batteries to mass-produce armor and lasers.
- **Altar** exchanges batteries for hearts (reward items).
...
## Historical notes and future plans

Earlier versions of MettaGrid described an energy system, shields, and building hitpoints; these mechanics have been removed in favor of the current inventory-based design. The project roadmap highlights planned features such as metabolism and energy costs, richer object interactions, population-level dynamics, and marker-based communication tools (see [`roadmap.md`](../roadmap.md)).
