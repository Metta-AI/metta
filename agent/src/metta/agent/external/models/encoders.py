...
class ObsLatentAttn(nn.Module):
    ...
    def forward(self, td: torch.Tensor, obs_mask: torch.Tensor, _BxTT_) -> torch.Tensor:
        x_features = td
        key_mask = None
        if self._use_mask:
            key_mask = obs_mask
...
class ObsSelfAttn(nn.Module):
    ...
    def forward(self, td: torch.Tensor, obs_mask: torch.Tensor) -> torch.Tensor:
        x_features = td
        if self._use_cls_token:
        ...
        key_mask = None
        if self._use_mask:
            key_mask = obs_mask  # True for elements to be masked
            if self._use_cls_token:
