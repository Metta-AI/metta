"""
Task Dependency Learning Simulator and sweep utilities.

This module bundles everything needed to study curriculum selectivity offline:

* A lightweight dynamical-system simulator that mirrors the production task
  dependency pipeline (waterfall progression, CurriculumEnv integration,
  WandB-friendly logging).
* Convenience configs and `Tool` wrappers so the simulator can be launched via
  `uv run ./tools/run.py ...` like any other recipe.
* Hyperparameter sweep helpers (z-score amplification, chain length, active
  pool size, etc.) used to reproduce the selectivity analyses shared with the
  curriculum team.

Keeping the simulator self-contained here lets us iterate on curriculum
algorithms quickly without touching MettaGrid environments or the full training
stack.
"""
