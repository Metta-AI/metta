...
class DualPolicyRollout:
    ...
    def _get_npc_actions(self, observations: Tensor) -> Tuple[Tensor, Tensor, Tensor, Optional[Dict[str, Tensor]]]:
        ...
        if self.config.npc_type == "scripted":
            ...
            from metta.rl.util.checkpoint_npc_inference import run_checkpoint_npc_inference

            actions, log_probs, values, lstm_state = run_checkpoint_npc_inference(
                self.npc_policy, observations, self.device
            )
