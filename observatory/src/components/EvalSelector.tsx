...
export const EvalSelector: React.FC<EvalSelectorProps> = ({
  ...
  const handleCategoryToggle = (category: string) => {
    ...
    if (isCurrentlySelected) {
      ...
      const newSelections = new Set([...selectedEvalNames, ...categories.get(category)!.map(envName => `${category}/${envName}`)]);
      ...
      <div className={styles.categoriesContainer}>
        ...
        {!loading && evalNames.size === 0 && (
          <div className={styles.emptyMessage}>
            No evaluations available. Select policies first.
          </div>
        )}

        {!loading && Array.from(categories.entries()).map(([category, envNames]) => (
                ...
                {envNames.map(envName => (
                  <div
                    ...
                    className={`${styles.environmentItem} ${
                      isEnvSelected(category, envName) ? styles.selected : ''
                    }`}
