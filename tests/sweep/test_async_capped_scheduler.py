...
class TestAsyncCappedOptimizingScheduler:
    ...
    def test_eval_unbounded_capacity(self):
        """max_concurrent_evals=None should allow scheduling all pending evals without errors."""
        config = AsyncCappedSchedulerConfig(
            max_trials=10,
            recipe_module="test.module",
            train_entrypoint="train",
        ...
        # A subsequent call should not reschedule the same evals
        jobs_again = scheduler.schedule(runs, available_training_slots=0)
        assert jobs_again == []
