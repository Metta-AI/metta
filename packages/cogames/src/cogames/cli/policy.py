...
policy_arg_w_proportion_example = DELIMITER.join(
    (
        "[blue]CLASS[/blue]",
        "[cyan]DATA[/cyan]",
...
policy_arg_example = DELIMITER.join(
    (
        "[blue]CLASS[/blue]",
        "[cyan]DATA[/cyan]",
    )
...
def _parse_policy_spec(spec: str) -> PolicySpec:
    ...
    if len(parts) > 3:
        raise ValueError(f"Policy specification must include at most two '{DELIMITER}' separated values.")
