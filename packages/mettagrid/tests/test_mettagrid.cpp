...
TEST_F(MettaGridCppTest, AttackAction) {
  ...
  // Create attack action handler targeting the agent directly in front (slot 5)
  AttackActionConfig attack_cfg({{TestItems::LASER, 1}}, {{TestItems::LASER, 1}}, {{TestItems::ARMOR, 3}});
  Attack attack(attack_cfg, &game_config, /*target_slot=*/5, "attack_front");
  std::mt19937 rng(42);
  attack.init(&grid, &rng);

  // Perform attack; target slot 5 maps to the agent directly in front
  bool success = attack.handle_action(*attacker);
  ...
  success = attack.handle_action(*attacker);
  EXPECT_TRUE(success);
...
TEST_F(MettaGridCppTest, PutRecipeItems) {
  ...
  bool success = put.handle_action(*agent);
  EXPECT_TRUE(success);
  ...
  success = put.handle_action(*agent);
...
TEST_F(MettaGridCppTest, GetOutput) {
  ...
  // Test getting output
  bool success = get.handle_action(*agent);
  EXPECT_TRUE(success);
...
TEST_F(MettaGridCppTest, ActionTracking) {
  ...
  noop.handle_action(*agent);  // count 1, max 1
  ...
  agent->location.r = 6;
  agent->location.c = 6;
  noop.handle_action(*agent);  // count 0, max 1
  ...
  noop.handle_action(*agent);  // count 1, max 1
  EXPECT_FLOAT_EQ(agent->stats.get("status.max_steps_without_motion"), 1.0f);
  noop.handle_action(*agent);  // count 2, max 2
  noop.handle_action(*agent);  // count 3, max 3
  EXPECT_FLOAT_EQ(agent->stats.get("status.max_steps_without_motion"), 3.0f);
  agent->location.r = 7;
  agent->location.c = 7;
  noop.handle_action(*agent);  // count 0, max 3
  ...
  noop.handle_action(*agent);  // count 1, max 3
  noop.handle_action(*agent);  // count 2, max 3
  EXPECT_FLOAT_EQ(agent->stats.get("status.max_steps_without_motion"), 3.0f);
  noop.handle_action(*agent);  // count 3, max 3
  noop.handle_action(*agent);  // count 4, max 4
...
TEST_F(MettaGridCppTest, FractionalConsumptionProbability) {
  ...
  bool success = noop.handle_action(*poor_agent);
...
TEST_F(MettaGridCppTest, FractionalConsumptionWithOverflow) {
  ...
  bool success = noop.handle_action(*agent);
...
TEST_F(MettaGridCppTest, FractionalConsumptionAttackAction) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    bool success = attack.handle_action(*attacker);  // Attack directly in front via slot 5
    if (success) {
      successful_attacks++;
...
TEST_F(MettaGridCppTest, FractionalConsumptionChangeGlyphAction) {
  ...
  ObservationType next_glyph = static_cast<ObservationType>((initial_glyph + 1) % glyph_cfg.number_of_glyphs);
  ChangeGlyph change_glyph(glyph_cfg, next_glyph, "change_glyph");
  std::mt19937 rng(42);
  ...
  while (agent->inventory.amount(TestItems::ORE) >= 2) {
    bool success = change_glyph.handle_action(*agent);
    if (!success) break;
    changes++;
...
TEST_F(MettaGridCppTest, FractionalConsumptionBoundaryValues) {
  ...
  bool first_success = noop.handle_action(*agent);
  EXPECT_TRUE(first_success);

  // Very high chance we consumed the resource (99%)
  bool second_success = noop.handle_action(*agent);
...
TEST_F(MettaGridCppTest, FractionalConsumptionDeterministicWithSameSeed) {
  ...
  for (int i = 0; i < 50; i++) {
    noop1.handle_action(*agent1);
    noop2.handle_action(*agent2);
  }
...
TEST_F(MettaGridCppTest, AssemblerClippingAndUnclipping) {
  ...
  bool success = assembler.onUse(*agent);
