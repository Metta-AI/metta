...
import gitta as git
from devops.skypilot.launch import (
    _validate_run_tool,
    _validate_sky_cluster_name,
    patch_task,
)
...
def _infer_gpus_per_node(task: sky.Task) -> int | None:
    """Attempt to read the number of GPUs per node from the SkyPilot task."""
    try:
    ...
    for res in resources:
        ...
        if isinstance(accelerators, dict) and accelerators:
            total = 0
            for value in accelerators.values():
                try:
                    total += int(value)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s metta.tools.ray_sweep.RaySweepTool sweep_config.sweep_id=my-sweep

  %(prog)s experiments.recipes.my_recipe.sweeps.ray --nodes 2 --gpus 4 --ray-head-port 7000 -- run=my_run
        """,
    )

    parser.add_argument(
        "module_path",
        help="Module path to run (e.g., metta.tools.ray_sweep.RaySweepTool). "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Dashboard port for the Ray head node (omit to use Ray defaults)",
    )
    parser.add_argument(
        "--run-ci-tests",
        action="store_true",
    ...
    env_updates: dict[str, Any] = dict(
        METTA_CLUSTER_MODE="ray_sweep",
        METTA_RUN_ID=run_id,
    ...
    inferred_gpus = args.gpus if args.gpus is not None else _infer_gpus_per_node(task)
    if inferred_gpus is not None:
        task = task.update_envs({"RAY_GPUS_PER_NODE": inferred_gpus})
    ...
    extra_details = {"cluster_mode": "ray_sweep"}
    if args.copies > 1:
    ...
    if args.copies == 1:
        ...
        for _ in range(1, args.copies + 1):
            ...
            copy_task = copy_task.update_envs(
                {
                    "METTA_RUN_ID": run_id,
                    "METTA_CLUSTER_MODE": "ray_sweep",
                }
