...
class PPO_NPC(BaseLoss):
    ...
    def __init__(
        ...
        # load NPC policy
        self.npc_policy: PolicyAgent = CheckpointManager.load_from_uri(self.loss_cfg.npc_uri, device)
        if hasattr(self.npc_policy, "initialize_to_environment"):
            features = self.vec_env.driver_env.get_observation_features()
            self.npc_policy.initialize_to_environment(
                features, self.vec_env.driver_env.action_names, self.vec_env.driver_env.max_action_args, self.device
            )

        # create indices for NPC and student agents
        target_batch_size, batch_size, num_envs = calculate_batch_sizes(
            self.trainer_cfg.forward_pass_minibatch_target_size,
            self.num_agents,
            self.trainer_cfg.rollout_workers,
            self.trainer_cfg.async_factor,
        )
        # force npc ratio to divide total num_agents using //
        npc_agents = self.num_agents // self.loss_cfg.npc_ratio
        student_agents = self.num_agents - npc_agents
        logger.info(f"Using {npc_agents} NPC agents and {student_agents} student agents.")
        # check that num_agents divids batch size
        assert self.num_agents % batch_size == 0, "num_agents must divide batch size"
        # set indices for NPC and student agents. It should repeat npc_agents times for NPC and student_agents times for
        #  student. ie if there num agents = 4 and npc ratio = 0.5, and batch size = 40 then npc indices should be
        # [0, 1, 4, 5, 8, 9, ...] and student indices should be [2, 3, 6, 7, 10, 11, ...]
        self.npc_batch_indices = torch.repeat_interleave(torch.arange(npc_agents), batch_size // npc_agents)
        self.student_batch_indices = torch.repeat_interleave(torch.arange(student_agents), batch_size // student_agents)
