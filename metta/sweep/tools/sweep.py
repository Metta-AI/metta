...
class SweepTool(Tool):
    ...
    def invoke(self, args: dict[str, str]) -> int | None:
        ...
        if self.dispatcher_type == DispatcherType.LOCAL:
            ...
            # Use sweep_name as the group for job segregation
            group = self.sweep_name if self.sweep_name else None
            dispatcher = RemoteQueueDispatcher(db_url=self.db_url, group=group)
            if group:
                logger.info(f"[SweepTool] Using RemoteQueueDispatcher for distributed execution (group: {group})")
            else:
                logger.info("[SweepTool] Using RemoteQueueDispatcher for distributed execution")
