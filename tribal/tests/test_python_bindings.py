...
class TestTribalBindings(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create config with 100 steps max
    ...
    def test_memory_management(self):
        ...
        envs = []
        for _i in range(5):
            game_config = tribal.TribalGameConfig(10, 5, 3, True, 0.01, 1, 1.0, 0.1, 0.8, 0.0, 0.0)
            config = tribal.TribalConfig(game_config, False)
    ...
    def test_rl_integration_scenario(self):
        ...
        game_config = tribal.TribalGameConfig(50, 5, 3, True, 0.01, 1, 1.0, 0.1, 0.8, 0.0, 0.0)
        config = tribal.TribalConfig(game_config, False)
        env = tribal.TribalEnv(config)
        env.reset_env()
...
class TestTribalBindingsIntegration(unittest.TestCase):
    ...
    def test_multiple_environments(self):
        ...
        for _i in range(3):
            game_config = tribal.TribalGameConfig(20, 5, 3, True, 0.01, 1, 1.0, 0.1, 0.8, 0.0, 0.0)
            config = tribal.TribalConfig(game_config, False)
            env = tribal.TribalEnv(config)
            env.reset_env()
            envs.append(env)
