...
def analyze_agent(agent, name):
    ...
    if hasattr(agent.policy, 'policy'):
        ...
        print(f"  Action embeddings shape: {policy.action_embeddings.weight.shape if hasattr(policy, 'action_embeddings') else 'N/A'}")
...
def main():
    ...
    yaml_agent.initialize_to_environment(
        features, 
        env.action_names, 
        env.max_action_args,
        torch.device('cpu')
    ...
    # Initialize to environment
    py_agent.initialize_to_environment(
        features,
        env.action_names,
        env.max_action_args,
        torch.device('cpu')
    ...
    test_obs = {
        'env_obs': torch.randint(0, 255, (4, 200, 3), dtype=torch.uint8)
    }
    
    try:
        yaml_out = yaml_agent(test_obs)
        print(f"YAML forward pass successful")
    ...
    try:
        ...
        print(f"py_agent forward pass successful")
