...
def make_rich_monitor_table(runs: list["RunInfo"], score_metric: str = "env_agent/heart.created") -> Table:
...
def create_run_banner(
    ...
    score_metric: str = "env_agent/heart.created",
    ...
    def generate_test_runs():
        ...
        for i in range(min(8, display_limit + 2)):  # Generate a few more than display limit
            ...
            if i < 2:
                ...
                summary = {"env_agent/heart.created": 0.85 + i * 0.05}
                current_steps = 1000000000
                total_timesteps = 1000000000
            elif i < 4:
                status = JobStatus.IN_TRAINING
                summary = {"env_agent/heart.created": 0.75 + i * 0.05}
