...
class SampleCogPolicyImpl(StatefulPolicyImpl[SampleCogState]):
    def __init__(
        self,
    ...
    def _parse_observation(self, obs: AgentObservation, state: SampleCogState) -> SampleCogState:
    ...
    def _go_to(self, state: SampleCogState, target: tuple[int, int]) -> Tuple[Action, SampleCogState]:
    ...
    def step_with_state(self, obs: AgentObservation, state: SampleCogState) -> Tuple[Action, SampleCogState]:
    ...
    def initial_agent_state(self) -> SampleCogState:
        """Get the initial state for a new agent."""
        return SampleCogState()
...
class SamplePolicy(MultiAgentPolicy):
    short_names = ["sample", "cvc_sample"]

    def __init__(self, policy_env_info: PolicyEnvInterface):
        super().__init__(policy_env_info)
        self._agent_policies: dict[int, StatefulAgentPolicy[SampleCogState]] = {}

    def agent_policy(self, agent_id: int) -> StatefulAgentPolicy[SampleCogState]:
        if agent_id not in self._agent_policies:
            self._agent_policies[agent_id] = StatefulAgentPolicy(
                SampleCogPolicyImpl(self._policy_env_info, agent_id),
