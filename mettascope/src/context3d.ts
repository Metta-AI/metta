...
export class Context3d {
  ...
  drawTextWorld(
    ...
    for (const ch of text) {
      ...
      // Kerning adjustment in font pixel units.
      if (prevLabel) {
        const row = font.kerning[prevLabel]
        if (row) {
          const adjust = row[label]
      ...
      // Draw relative to local origin at (0,0); world transforms are applied via currentTransform.
      const drawX = penX + glyph.bearingX - glyphInnerPadding - mBase
      const drawY = penY + glyph.bearingY - glyphInnerPadding - mBase
      const drawW = sw + 2 * mBase
      const drawH = sh + 2 * mBase
