...
./devops/skypilot/launch.py train run=daphne.sequence.bucketed.06-20  trainer.curriculum=env/mettagrid/curriculum/sequence --no-spot --gpus=4 --skip-git-check \
...
#NAV-MEMORY-SEQUENCE
./devops/skypilot/launch.py train run=daphne.nav_memory_sequence.bucketed.06-20  trainer.curriculum=env/mettagrid/curriculum/nav_memory_sequence --no-spot --gpus=4 --skip-git-check \
