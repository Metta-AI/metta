...
def convert_to_cpp_game_config(mettagrid_config: dict | "mettagrid.config.mettagrid_config.GameConfig"):
    ...
    supported_vibes = mettagrid.config.vibes.VIBES[:num_vibes]
    ...
    for team_id, team_agents in team_groups.items():
        ...
        inventory_config = mettagrid.mettagrid_c.InventoryConfig(limits=limits_list)

        cpp_agent_config = mettagrid.mettagrid_c.AgentConfig(
            type_id=0,
            type_name="agent",
            group_id=team_id,
    ...
    for object_type, object_config in game_config.objects.items():
        if isinstance(object_config, mettagrid.config.mettagrid_config.WallConfig):
            # Convert tag names to IDs
            tag_ids = [tag_name_to_id[tag] for tag in object_config.tags]

            cpp_wall_config = mettagrid.mettagrid_c.WallConfig(
        ...
        elif isinstance(object_config, mettagrid.config.mettagrid_config.AssemblerConfig):
            protocols = []
            seen_vibes = []
            ...
            for protocol_config in reversed(object_config.protocols):
                ...
                cpp_protocol = mettagrid.mettagrid_c.Protocol()
            ...
            cpp_assembler_config = mettagrid.mettagrid_c.AssemblerConfig(
                type_id=object_config.type_id, type_name=object_type, initial_vibe=object_config.vibe
            )
        ...
        elif isinstance(object_config, mettagrid.config.mettagrid_config.ChestConfig):
            # Convert tag names to IDs
            tag_ids = [tag_name_to_id[tag] for tag in object_config.tags]
            ...
            cpp_chest_config = mettagrid.mettagrid_c.ChestConfig(
                type_id=object_config.type_id, type_name=object_type, initial_vibe=object_config.vibe
            )
    ...
    global_obs_cpp = mettagrid.mettagrid_c.GlobalObsConfig(
    ...
    actions_cpp_params["noop"] = mettagrid.mettagrid_c.ActionConfig(**action_params)
    ...
    actions_cpp_params["move"] = mettagrid.mettagrid_c.MoveActionConfig(**action_params)
    ...
    actions_cpp_params["attack"] = mettagrid.mettagrid_c.AttackActionConfig(**action_params)
    ...
    actions_cpp_params["change_vibe"] = mettagrid.mettagrid_c.ChangeVibeActionConfig(**action_params)
    ...
    actions_cpp_params["resource_mod"] = mettagrid.mettagrid_c.ResourceModConfig(**action_params)
    ...
    if game_config.clipper is not None:
        clipper: mettagrid.config.mettagrid_config.ClipperConfig = game_config.clipper
        clipper_protocols = []
        ...
        game_cpp_params["clipper"] = mettagrid.mettagrid_c.ClipperConfig(
    ...
    return mettagrid.mettagrid_c.GameConfig(**game_cpp_params)
