...
def save_policy_with_metadata(
    ...
    This function provides compatibility with run.py by wrapping the functionality
    that's now in CheckpointManager.save_policy().
    ...
    Args:
        ...
        evals: Evaluation scores (optional)
    ...
    if hasattr(policy, "module"):  # DistributedDataParallel or DistributedMettaAgent
    ...
    if evals:
        if hasattr(evals, "avg_category_score"):
            ...
            category_scores = list(evals.category_scores.values()) if hasattr(evals, "category_scores") else []
            score = float(np.mean(category_scores)) if category_scores else 0.0
            if hasattr(evals, "category_scores") and hasattr(evals, "simulation_scores"):
                evals_dict = {
    ...
    total_time = 0
    total_train_time = 0
    if hasattr(timer, "get_elapsed"):
        total_time = timer.get_elapsed()
        if hasattr(timer, "get_all_elapsed"):
            all_elapsed = timer.get_all_elapsed()
            total_train_time = all_elapsed.get("_rollout", 0) + all_elapsed.get("_train", 0)
    elif hasattr(timer, "elapsed"):
    ...
    # Synchronize all ranks if distributed
    if torch.distributed.is_initialized():
        torch.distributed.barrier()
