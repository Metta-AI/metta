...
class TestSendWandbAlertWithTimeout:
    ...
    def test_send_wandb_alert_success(self, mock_logger, mock_wandb_api, mock_open):
        ...
        ipc_data = {"run_id": "test_run_123", "project": "test_project", "entity": "test_entity"}
        mock_file = Mock()
        ...
        mock_run.alert.assert_called_once_with(title="Test Alert", text="Test Alert", level="WARN")
    ...
    def test_send_wandb_alert_missing_required_fields(self, mock_logger, mock_open):
        ...
        # Missing run_id
        ipc_data = {"project": "test_project", "entity": "test_entity"}
        mock_file = Mock()
        mock_file.read.return_value = json.dumps(ipc_data)
        mock_open.return_value.__enter__ = Mock(return_value=mock_file)
        mock_open.return_value.__exit__ = Mock(return_value=None)
...
class TestMonitorHeartbeat:
    ...
    def test_monitor_heartbeat_timeout_triggers_kill(
        self, mock_alert, mock_killpg, mock_time, mock_getmtime, _mock_sleep
    ):
...
class TestMainFunction:
    ...
    def test_main_monitor_command_default_args(self, mock_monitor):
        ...
        mock_monitor.assert_called_once_with("/tmp/heartbeat.txt", pid=12345, timeout=600.0, check_interval=60.0)
    ...
    def test_main_monitor_command_custom_args(self, mock_monitor):
        ...
        mock_monitor.assert_called_once_with("/tmp/heartbeat.txt", pid=9999, timeout=300.0, check_interval=30.0)
