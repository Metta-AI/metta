...
import cogames.policy.scripted_agent.sample_policy as sample_policy
...
@app.command("make-policy", help="Create a new sample policy")
def make_policy(
    output: Path = typer.Option("my_policy.py", "--output", "-o", help="Output file path"),  # noqa: B008
) -> None:
    """Create a new sample policy in the current directory."""
    try:
        # Get the path to the sample_policy.py file
        sample_policy_path = Path(sample_policy.__file__)
        if not sample_policy_path.exists():
            console.print("[red]Error: Sample policy file not found[/red]")
            raise typer.Exit(1)
        ...
        shutil.copy2(sample_policy_path, dest_path)
        console.print(f"[green]Sample policy copied to: {dest_path}[/green]")
