...
class AdaptiveExperimentMetadata:
    """
    Metadata about an experiment stored in the Store.
    ...
    @classmethod
    def from_runs(cls, runs: list[RunInfo]) -> "AdaptiveExperimentMetadata":
        runs_created = len(runs)
        runs_pending = sum(1 for run in runs if run.status == JobStatus.PENDING)
        runs_in_progress = sum(1 for run in runs if run.status == JobStatus.IN_TRAINING)
        runs_completed = sum(1 for run in runs if run.status == JobStatus.COMPLETED)
        return cls(
            experiment_id=runs[0].group or "",
