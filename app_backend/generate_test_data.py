...
def update_training_run_metadata(base_url: str, token: str, run_id: uuid.UUID, description: str, tags: List[str]) -> None:
        ...
        with httpx.Client(base_url=base_url) as http_client:
            ...
            for i, epoch_config in enumerate(run_config["epochs"]):
                epoch = stats_client.create_epoch(
                    ...
                    attributes={
                        "learning_rate": epoch_config["lr"],
                        "performance_stage": epoch_config["performance"]
                    }
                )
                ...
                for task in suite["tasks"]:
                    for policy_data in policies:
                        ...
                        performance_multipliers = {
                            "early": 0.3, "random": 0.2, "baseline": 0.4,
                            "improving": 0.6, "learning": 0.5, "optimized": 0.8,
                            "converged": 0.9, "cooperative": 0.85, "easy_tasks": 0.7,
                            "medium_tasks": 0.8, "hard_tasks": 0.75, "safe_random": 0.4,
                            "constrained_learning": 0.7, "standard": 0.6, "robust": 0.8
                        }
                        base_performance = performance_multipliers.get(stage, 0.5)
                        
                        # Add some task-specific variation
                        task_difficulty = {
                            "maze_easy": 1.2, "maze_hard": 0.7, "obstacle_course": 0.8,
                            "pick_and_place": 0.9, "door_opening": 0.6, "object_stacking": 0.5,
                            "team_navigation": 0.8, "resource_sharing": 0.7,
                            "safe_navigation": 0.9, "emergency_stop": 0.95,
                            "perturbed_env": 0.6, "domain_transfer": 0.4
                        }
