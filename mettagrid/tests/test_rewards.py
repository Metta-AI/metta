...
class TestRewards:
    ...
    def test_reward_buffer_memory_layout(self):
        """Test that reward buffers have correct memory layout and are properly updated."""
        num_agents = 2
        env = create_reward_test_env(num_agents=num_agents)
        ...
        # Verify buffer properties before setting
        assert rewards.flags.c_contiguous, "Rewards buffer should be C-contiguous"
        assert rewards.dtype == np_rewards_type, f"Rewards buffer should have dtype {np_rewards_type}"

        env.set_buffers(observations, terminals, truncations, rewards)
        env.reset()

        # Manually modify reward buffer to test memory sharing
        rewards[0] = 99.5
        rewards[1] = -42.3
        ...
        # Verify that step overwrote our manual values
        assert not np.array_equal(rewards, [99.5, -42.3]), "Step should have overwritten manual reward values"

        # Verify that returned rewards match buffer
        np.testing.assert_array_equal(step_rewards, rewards, "Returned step rewards should match buffer")

        print(f"✅ Reward buffer memory layout correct: {step_rewards}")
...
if __name__ == "__main__":
    ...
    print("=== Testing Step Rewards Initialization ===")
    test.test_step_rewards_initialization()

    print("\n=== Testing Heart Collection Rewards ===")
    test.test_heart_collection_rewards()

    print("\n=== Testing Multiple Heart Collections ===")
    test.test_multiple_heart_collections()

    print("\n=== Testing Episode Rewards Accumulation ===")
    test.test_episode_rewards_accumulation()

    print("\n=== Testing Gym Mode Rewards ===")
    test.test_gym_mode_rewards()

    print("\n=== Testing Reward Buffer Memory Layout ===")
    test.test_reward_buffer_memory_layout()

    print("\n🎉 All reward tests completed successfully!")
