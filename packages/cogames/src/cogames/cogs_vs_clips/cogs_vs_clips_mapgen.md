...
- `--map-seed`: base MapGen seed for procedural maps. `play`/`eval` use a fixed map; `train` derives per-env maps.
- `--seed`: controls simulator/policy RNG.
- If `--map-seed` is **unset** and the mission does not set a MapGen seed, `train` derives per-env map seeds from
  `--seed` (reproducible but varied maps) while `play`/`eval` leave the MapGen seed untouched; if it is `None`, maps
  vary.
- For fully reproducible play/eval runs, set **both** `--seed` and `--map-seed`.
- Recipe `maps_cache_size` controls how many distinct procedural maps are cached when the MapGen seed is unset; set it
  to `None` to disable caching (more diversity, slower) or `1` for a single cached map.
