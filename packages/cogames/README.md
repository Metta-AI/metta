...
- **Scripted policies**: Hand-coded logic (run `cogames make-policy --scripted` for a starter template)
- **Trainable policies**: Neural networks that learn from experience (run `cogames make-policy --trainable` for a template)
...
```bash
cogames make-policy --trainable -o my_policy.py
```
...
```bash
cogames train -m easy_hearts -p class=my_policy.MyTrainablePolicy
```
