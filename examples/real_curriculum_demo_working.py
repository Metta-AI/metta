#!/usr/bin/env python3
"""
Working Real Curriculum Analysis Demo
...
def analyze_curriculum(cfg: DictConfig) -> Dict:
    """Analyze a single curriculum using the working integration test approach."""
    logger.info("Analyzing curriculum...")
    ...
    try:
        ...
        trainer_cfg.analysis_output_dir = "real_curriculum_analysis"
...
def main():
    ...
    for curriculum_name, _config in curriculum_configs.items():
        logger.info(f"Processing {curriculum_name}...")

        try:
            # Use the working integration test approach
            results = analyze_curriculum(cfg=None)  # Will be provided by Hydra
