...
class BrewInstaller(
    ...
    def _get_changes_to_apply(
        self, packages: list[metta.setup.components.system_packages.types.BrewPackageConfig]
    ) -> tuple[
        list[metta.setup.components.system_packages.types.BrewPackageConfig],
        list[metta.setup.components.system_packages.types.BrewPackageConfig],
        list[str],
    ]:
        installed, pinned, tapped = self._get_installed_state()
        to_install: list[metta.setup.components.system_packages.types.BrewPackageConfig] = []

        def _package_in_output_list(
            p: metta.setup.components.system_packages.types.BrewPackageConfig, output_list: list[str]
        ) -> bool:
    ...
    def check_installed(self, packages: list[metta.setup.components.system_packages.types.BrewPackageConfig]) -> bool:
        ...
        for label, packages in [("To install", to_install), ("To pin", to_pin), ("To tap", to_tap)]:
            if packages:
                metta.setup.utils.info(f"{label}: {', '.join([str(p) for p in packages])}")
        return all_installed

    def install(self, packages: list[metta.setup.components.system_packages.types.BrewPackageConfig]) -> None:
        to_install, to_pin, to_tap = self._get_changes_to_apply(packages)
        for tap_name in to_tap:
            metta.setup.utils.info(f"Adding tap: {tap_name}")
            self._install_cmd(["brew", "tap", tap_name])

        if to_install:
            full_install_names = [p.fully_specified_name for p in packages]
            metta.setup.utils.info(f"Installing {full_install_names}...")
            self._install_cmd(["brew", "install", *full_install_names])

        if to_pin:
            pin_names = [p.name for p in to_pin]
            metta.setup.utils.info(f"Pinning {pin_names}...")
