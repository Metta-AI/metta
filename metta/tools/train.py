...
from metta.rl.stats import StatsTracker, process_stats
...
def handle_train(cfg: TrainTool, torch_dist_cfg: TorchDistributedConfig, wandb_run: WandbRun | None) -> None:
    ...
    class MyCallbacks(TrainingCallbacks):
        ...
        def evaluate_policy(
            self, epoch: int, stats_tracker: StatsTracker, env_cfg: MettaGridConfig, agent_step: int
        ) -> EvalResults | None:
            if cfg.trainer.evaluation:
                ...
                if wandb_run:
                    policy_uri = wandb_run.url  # Already a wandb:// URI
                else:
        ...
        def save_checkpoint(
            ...
            should_upload_wandb = wandb_run and (
                is_final or should_run(epoch, trainer_cfg.checkpoint.wandb_checkpoint_interval)
            )
