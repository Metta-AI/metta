...
def parse_architecture_spec(spec: str) -> tuple[str, dict[str, Any]]:
    import ast
    spec = spec.strip()
    if not spec:
...
def format_architecture_spec(class_path: str, payload: dict[str, Any]) -> str:
    class_path = class_path.strip()
    if not class_path:
        raise ValueError("architecture_spec cannot be empty")
    if not payload:
        return class_path
    sorted_payload = _sorted_structure(payload)
...
def architecture_from_spec(spec: str) -> Any:
    class_path, _ = parse_architecture_spec(spec)
