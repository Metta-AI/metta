...
class MettaAgent(nn.Module):
    ...
    def __setstate__(self, state):
        ...
        from metta.agent.utils import convert_old_checkpoint, is_old_checkpoint_format

        if is_old_checkpoint_format(state):
            # Convert old checkpoint format using utility
            agent_state, policy = convert_old_checkpoint(state)
            self.__dict__.update(agent_state)

            # Ensure _modules dict exists and set policy
