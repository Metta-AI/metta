...
class TestBulkEpisodeUpload:
    ...
    async def test_bulk_upload_route(
        ...
        with open(sample_duckdb, "rb") as f:
            ...
            response = test_client.post(
                "/stats/episodes/bulk_upload",
                files=files,
                params={"s3_bucket": "test-bucket", "s3_prefix": "test-prefix"},
        ...
        assert "s3://" in data["duckdb_s3_uri"]
        ...
        async with stats_repo.connect() as con:
            ...
            # Check episode_policies table (should have aggregated by policy)
            result = await con.execute("SELECT policy_version_id, num_agents FROM episode_policies")
            policies = await result.fetchall()
            assert len(policies) >= 1
            # All 4 agents should be counted for the single policy
            assert any(p[1] == 4 for p in policies)

            # Check episode_policy_metrics (should be aggregated)
            result = await con.execute(
                "SELECT metric_name, value FROM episode_policy_metrics WHERE metric_name = 'reward'"
            )
            metrics = await result.fetchall()
            # Should have reward metric aggregated across all 4 agents
            # 1.5 + 3.0 + 4.5 + 6.0 = 15.0
    ...
    async def test_bulk_upload_client(
        ...
        # Upload via client
        response = stats_client.bulk_upload_episodes(
            str(sample_duckdb), s3_bucket="test-bucket", s3_prefix="test-episodes"
        )
    ...
    async def test_bulk_upload_multiple_episodes(
        ...
        with open(db_path, "rb") as f:
            files = {"file": ("episodes.duckdb", f, "application/octet-stream")}
            response = test_client.post("/stats/episodes/bulk_upload", files=files, headers=test_user_headers)
