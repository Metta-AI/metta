...
class PolicyRecord:
    ...
    def policy(self) -> "PolicyAgent":
        ...
        if (self._cached_policy is not None and 
            'original_feature_mapping' in self.metadata and
            not hasattr(self._cached_policy, 'original_feature_mapping')):
            self._cached_policy.original_feature_mapping = self.metadata['original_feature_mapping'].copy()
            logger.info(f"Transferred original_feature_mapping from metadata to policy")
