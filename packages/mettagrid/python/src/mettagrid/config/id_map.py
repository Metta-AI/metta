...
class IdMap:
    ...
    def assign_type_ids(game_config: GameConfig) -> None:
        ...
        # Populate resolved_type_ids mapping
        if not hasattr(game_config, "_resolved_type_ids"):
            game_config._resolved_type_ids = {}
        game_config._resolved_type_ids = {name: obj.type_id for name, obj in objects.items() if obj.type_id is not None}
    ...
    def _compute_features(self) -> list[ObservationFeatureSpec]:
        ...
        features.append(ObservationFeatureSpec(id=feature_id, normalization=255.0, name="agent:glyph"))
        feature_id += 1
        ...
        # Recipe details features (if enabled)
        if game_cfg.recipe_details_obs:
            for resource_name in game_cfg.resource_names:
                features.append(
                    ObservationFeatureSpec(id=feature_id, normalization=100.0, name=f"recipe_input:{resource_name}")
                )
                feature_id += 1

            for resource_name in game_cfg.resource_names:
                features.append(
                    ObservationFeatureSpec(id=feature_id, normalization=100.0, name=f"recipe_output:{resource_name}")
                )
                feature_id += 1
