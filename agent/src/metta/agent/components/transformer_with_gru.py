...
from torchrl.data import Composite, UnboundedContinuousTensorSpec, UnboundedDiscrete
...
class SlidingTransformerWithGRU(nn.Module):
    ...
    def __init__(self, config: SlidingTransformerWithGRUConfig, env):
        ...
        if self.config.pool == "none":
            # This is a bit of a hack, but it's the easiest way to get the number of tokens
            # 1 (cls) + 1 (obs) + 1 (reward/reset) + 1 (action) = 4
            gru_input_dim = self.output_dim * 4
        else:
            gru_input_dim = self.output_dim
        ...
        # GRU hidden state buffer
        self.register_buffer("gru_hidden_state", torch.empty(0, self.config.gru_num_layers, self.config.gru_hidden_dim))
        ...
        if TT == 1:  # rollout
            ...
            if max_num_envs > self.position_counter.size(0):
                ...
                gru_filler = torch.zeros(
                    num_new_envs, *self.gru_hidden_state.shape[1:], device=self.gru_hidden_state.device
                )
                self.gru_hidden_state = torch.cat([self.gru_hidden_state, gru_filler])
            ...
            td.set(("recurrent_states", "gru_hidden"), self.gru_hidden_state[training_env_ids].detach())
            ...
            # GRU part for rollout
            gru_hidden = self.gru_hidden_state[training_env_ids].transpose(0, 1)  # [num_layers, B, H]
            gru_output, gru_hidden = self.gru(pooled_output.unsqueeze(1), gru_hidden)
            self.gru_hidden_state[training_env_ids] = gru_hidden.transpose(0, 1).detach()
            ...
            self.gru_hidden_state[training_env_ids] *= (~resets.bool()).long().unsqueeze(-1).unsqueeze(-1)
            ...
            # GRU part for training
            gru_hidden = td[("recurrent_states", "gru_hidden")]
            gru_hidden = gru_hidden.view(B, TT, self.config.gru_num_layers, self.config.gru_hidden_dim)[:, 0]
            gru_hidden = gru_hidden.transpose(0, 1).contiguous()
            gru_output, _ = self.gru(pooled_output, gru_hidden)

            pooled_output = rearrange(gru_output, "b tt ... -> (b tt) ...")
        ...
        return Composite(
            {
                ...
                ("recurrent_states", "gru_hidden"): UnboundedContinuousTensorSpec(
                    shape=torch.Size([self.config.gru_num_layers, self.config.gru_hidden_dim]),
                    dtype=torch.float32,
                ),
            }
        )
    ...
    def get_memory(self):
        return self.k_cache, self.v_cache, self.gru_hidden_state

    def set_memory(self, memory):
        """Cannot be called at the Policy level - use policy.<path_to_this_layer>.set_memory()"""
        self.k_cache, self.v_cache, self.gru_hidden_state = memory[0], memory[1], memory[2]
