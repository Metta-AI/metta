"""
Arena Basic Easy Shaped with AGaLiTe Agent Recipe

This recipe is based on arena_basic_easy_shaped but uses the AGaLiTe agent for training.
"""
...
def make_env(num_agents: int = 24) -> EnvConfig:
    ...
    env_cfg.label = "arena.bes_agalite"
...
def make_evals(env: Optional[EnvConfig] = None) -> List[SimulationConfig]:
    ...
    return [
        SimulationConfig(name="arena_bes_agalite/basic", env=basic_env),
        SimulationConfig(name="arena_bes_agalite/combat", env=combat_env),
    ]
...
def train() -> TrainTool:
    ...
    This is based on arena_basic_easy_shaped but uses the agalite agent.
    """
    env_cfg = make_env()
    ...
    return TrainTool(
        trainer=trainer_cfg,
        policy_architecture=agent_config,
    )
...
def play(
    ...
    return PlayTool(
        sim=SimulationConfig(env=eval_env, name="arena_bes_agalite"),
...
def replay(env: Optional[EnvConfig] = None) -> ReplayTool:
    ...
    return ReplayTool(sim=SimulationConfig(env=eval_env, name="arena_bes_agalite"))
...
if __name__ == "__main__":
    ...
    Usage:
        uv run experiments/recipes/arena_bes_agalite.py [port] [num_agents_or_policy] [policy]
        
    Examples:
        uv run experiments/recipes/arena_bes_agalite.py              # Default: port 8001, 24 agents
        uv run experiments/recipes/arena_bes_agalite.py 8002 6        # Port 8002, 6 agents
        uv run experiments/recipes/arena_bes_agalite.py 8003 wandb://run/my-run
    """
    import os
    import sys
