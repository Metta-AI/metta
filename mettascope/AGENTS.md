...
- ./mettascope/index.html do not edit index.html! It is auto generated by an engineer with a script from Figma. The code
  is plain TypeScript with no framework.
...
All commands are executed from the `mettascope/` directory. | Task | Command | Notes | | ------------- |
----------------------------------- | ----------------------------------------------------- | | Build once |
`pnpm run build` | | | Lint & format | `pnpm run lint` / `pnpm run format` | should be run on code changes | | Run tests
| `pnpm run test` | frontend tests require the backend is already running |
