...
class MettaActorBig3(LayerBase):
    """
    Has relu on hidden input """
    def __init__(self, mlp_hidden_dim=512, bilinear_output_dim=32, **cfg):
        super().__init__(**cfg)
        self.mlp_hidden_dim = mlp_hidden_dim # this is hardcoded for a two layer MLP
        self.bilinear_output_dim = bilinear_output_dim
    ...
    def _forward(self, td: TensorDict):
        ...
        # Apply activation
        activated_scores = self._relu(biased_scores) # Shape: [N, K + hidden]
...
class MettaActorBig4(LayerBase):
    """
    Implements a bilinear interaction layer followed by an MLP with a lot of reshaping.
    It replicates what could be achieved by piecing together a number of other layers which would be more flexible.
    """
    ...
    def _forward(self, td: TensorDict):
        ...
        # Split the bilinear operation into two stages with a non-linearity in between.
        # Stage 1: combine the hidden representation with the weight tensor, keeping the embedding dimension.
        # Resulting shape: [N, K, E]
        intermediate = torch.einsum('n h, k h e -> n k e', input_1_reshaped, self.W)

        # Stage 2: apply tanh activation to introduce non-linearity.
...
class MettaActorSingleHead2(LayerBase):
    """
    Implements a bilinear interaction layer followed by an MLP with a lot of reshaping.
    It replicates what could be achieved by piecing together a number of other layers which would be more flexible.
    """
