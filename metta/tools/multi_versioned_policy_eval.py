...
class MultiPolicyVersionEvalTool(ToolWithResult):
    ...
    def run_job(self) -> ToolResult:
        ...
        with ExitStack() as stack:
            ...
            post_result = self.after_rollout(rollout_results=rollout_results, policy_specs=policy_specs)
        return post_result or ToolResult(result="success")

    def after_rollout(
        self, *, rollout_results: list[SimulationRunResult], policy_specs: Sequence[PolicySpec]
    ) -> ToolResult | None:
        """
        Hook that allows subclasses to derive additional artifacts from the rollout data.

        Returning None keeps the default ToolResult(result="success").
        """
        return None
