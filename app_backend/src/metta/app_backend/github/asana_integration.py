...
from metta.app_backend.github.retry import RetryExhausted, retry_with_backoff
...
def _get_asana_client() -> asana.ApiClient:
    ...
    if not settings.ASANA_PAT:
        raise ValueError("ASANA_PAT not configured")

    config = asana.Configuration()
    config.access_token = settings.ASANA_PAT
...
def _get_github_to_asana_mapping(github_logins: set[str]) -> dict[str, str]:
    ...
    # Get roster project ID from environment (same as GitHub Actions uses)
    roster_project_gid = os.getenv("ASANA_ROSTER_PROJECT_GID", "1209948553419016")
    gh_login_field_gid = os.getenv("ASANA_GH_LOGIN_FIELD_GID", "1210594297567963")
    asana_email_field_gid = os.getenv("ASANA_EMAIL_FIELD_GID", "1209050603577235")
