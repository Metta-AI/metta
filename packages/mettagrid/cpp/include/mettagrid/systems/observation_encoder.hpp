...
public:
  explicit ObservationEncoder(size_t resource_count,
      ...
      : recipe_details_obs(recipe_details_obs), resource_count(resource_count) {
    // Build feature ID maps for recipe details if enabled
      ...
      if (resource_names && feature_ids && !resource_names->empty()) {
        ...
        for (size_t i = 0; i < resource_names->size(); ++i) {
          ...
          if (input_it != feature_ids->end()) {
            ...
            throw std::runtime_error("Recipe input feature 'input:" + resource_name + "' not found in feature_ids");
          }
          ...
          if (output_it != feature_ids->end()) {
            ...
            throw std::runtime_error("Recipe output feature 'output:" + resource_name + "' not found in feature_ids");
          }
        }
      } else {
        throw std::runtime_error("ObservationEncoder with recipe_details_obs requires resource_names and feature_ids");
      }
    }
  }
