...
class AssemblerConfig(GridObjectConfig):
    ...
    chest_radius: int = Field(
        default=0,
        ge=0,
        description="Radius to search for chests to use resources from when crafting (0 = disabled)",
    )
    agent_cooldown: int = Field(
        default=0,
        ge=0,
        description="Per-agent cooldown ticks before the same agent can use this assembler again (0 = no cooldown)",
    )
