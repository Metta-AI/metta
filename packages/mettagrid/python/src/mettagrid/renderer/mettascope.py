...
class MettascopeRenderer(Renderer):
    ...
    def on_before_step(self, env: "MettaGridEnv", actions: np.ndarray) -> np.ndarray:
        """Render current state and get user input, then apply to actions."""
        # Generate replay data for current state BEFORE stepping
        grid_objects = []
        total_rewards = self._env.get_episode_rewards() if self._env else np.zeros(env.num_agents)

        for grid_object in self._env.grid_objects().values():
            # For before_step, we don't have rewards/actions from this step yet
            # Use zeros as placeholders since this is just for rendering
            placeholder_actions = np.zeros((env.num_agents, 2), dtype=np.int32)
            placeholder_rewards = np.zeros(env.num_agents)
            grid_objects.append(
                format_grid_object(
        ...
        # Apply all persisted user actions
        for agent_id, (action_id, action_param) in self._user_actions.items():
