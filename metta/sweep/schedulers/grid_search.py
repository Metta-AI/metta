...
class GridSearchSchedulerConfig(mettagrid.base_config.Config):
    ...
    max_concurrent_evals: typing.Optional[int] = None
...
class GridSearchScheduler:
    ...
    def __init__(self, config: GridSearchSchedulerConfig):
        ...
        self._grid: list[dict[str, typing.Any]] = self._cartesian_product(dims)
    ...
    def schedule(
        ...
        in_eval = [r for r in runs if r.status == metta.adaptive.models.JobStatus.IN_EVAL]
        eval_candidates = [r for r in runs if r.status == metta.adaptive.models.JobStatus.TRAINING_DONE_NO_EVAL]

        eval_capacity: typing.Optional[int] = None
    ...
    def _suggestion_key(self, suggestion: dict[str, typing.Any]) -> typing.Tuple[typing.Any, ...]:
        # Build a key tuple ordered by dimension names
        return tuple(suggestion.get(name) for name in self._dim_names)

    def _collect_used_suggestion_keys(
        self, runs: list[metta.adaptive.models.RunInfo]
    ) -> set[typing.Tuple[typing.Any, ...]]:
        keys: set[typing.Tuple[typing.Any, ...]] = set()
        for r in runs:
    ...
    def _flatten_dims(
        self, params: typing.Dict[str, typing.Any], prefix: str = ""
        ...
        """
        dims: dict[str, list[typing.Any]] = {}
        for key, value in params.items():
            full = f"{prefix}.{key}" if prefix else key
            if isinstance(value, metta.sweep.core.CategoricalParameterConfig):
    ...
    def _cartesian_product(
        self, dims: typing.Dict[str, typing.List[typing.Any]]
    ) -> typing.List[typing.Dict[str, typing.Any]]:
        """Build the Cartesian product of dimensions as a list of suggestion dicts."""
        names = list(dims.keys())
        values: typing.Iterable[typing.Tuple[typing.Any, ...]] = itertools.product(*(dims[name] for name in names))
