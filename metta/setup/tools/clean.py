...
def cmd_clean(
    ...
    for dirpath, _, _ in os.walk(repo_root, topdown=False):
        ...
        if is_dir_empty_or_pycache_only(dir_path):
            metta.setup.utils.success(f"Removing: {dir_path.relative_to(repo_root)}")
            shutil.rmtree(dir_path)
            removed_dirs.append(dir_path)
    metta.setup.utils.info(f"Removed {len(removed_dirs)} directories")


def is_dir_empty_or_pycache_only(dir_path: pathlib.Path) -> bool:
    try:
        ...
        metta.setup.utils.error(f"Permission denied: {dir_path}")
        return False
