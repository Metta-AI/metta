...
ALLOWED_SKIP_PACKAGES = {package.name.lower() for package in metta.setup.tools.test_runner.test_python.PACKAGES}
...
def _format_cmd_for_display(cmd: typing.Sequence[str]) -> str:
...
def _ensure_no_extra_args(stage_name: str, extra_args: typing.Sequence[str] | None) -> None:
    if extra_args:
        metta.setup.utils.error(f"Stage '{stage_name}' does not accept extra arguments.")
        raise typer.Exit(1)


def _normalize_python_stage_args(extra_args: typing.Sequence[str] | None) -> list[str]:
    if not extra_args:
        return []
    ...
    while idx < len(args):
        ...
        if token == "--skip-package":
            if idx + 1 >= len(args):
                metta.setup.utils.error("'--skip-package' requires a package name.")
                raise typer.Exit(1)
            package_name = args[idx + 1]
            if package_name.lower() not in ALLOWED_SKIP_PACKAGES:
                allowed = ", ".join(sorted(ALLOWED_SKIP_PACKAGES))
                metta.setup.utils.error(f"Unsupported package '{package_name}' for --skip-package.")
                metta.setup.utils.info(f"Allowed packages: {allowed}")
                raise typer.Exit(1)
            sanitized.extend([token, package_name])
            idx += 2
            continue

        metta.setup.utils.error(f"Argument '{token}' is not supported for python-tests stage.")
        metta.setup.utils.info("Allowed arguments: --skip-package <package>")
        raise typer.Exit(1)

    return sanitized


def _run_command(cmd: typing.Sequence[str], description: str, *, verbose: bool = False) -> bool:
    """Run a command and return True if successful."""
    display_cmd = _format_cmd_for_display(cmd)
    metta.setup.utils.info(f"Running: {display_cmd}")
    try:
        subprocess.run(
            cmd,
        ...
        metta.setup.utils.success(f"{description} passed")
        return True
    except subprocess.CalledProcessError as exc:  # pragma: no cover - invoked via subprocess
        metta.setup.utils.error(f"{description} failed")
        if not verbose:
            if exc.stdout:
...
def _run_lint(*, verbose: bool = False, extra_args: typing.Sequence[str] | None = None) -> CheckResult:
...
def _run_cpp_tests(*, verbose: bool = False, extra_args: typing.Sequence[str] | None = None) -> CheckResult:
...
def _run_cpp_benchmarks(*, verbose: bool = False, extra_args: typing.Sequence[str] | None = None) -> CheckResult:
    """Run C++ benchmarks."""
...
def _print_summary(results: list[CheckResult]) -> None:
    ...
    table = rich.table.Table(title="CI Check Summary", show_header=True, header_style="bold magenta")
...
StageRunner = typing.Callable[[bool, typing.Sequence[str] | None], CheckResult]
...
def cmd_ci(
    ctx: typer.Context,
    stage: typing.Annotated[
        str | None,
        typer.Option(help=f"Run specific stage: {', '.join(stages.keys())}"),
    ] = None,
    continue_on_error: typing.Annotated[
        bool, typer.Option("--continue-on-error", help="Don't stop on first failure")
    ] = False,
    verbose: typing.Annotated[bool, typer.Option("--verbose", "-v", help="Show detailed output")] = False,
):
    """Run CI checks locally to match remote CI behavior."""
    extra_args = list(getattr(ctx, "args", []))

    if extra_args and stage is None:
        metta.setup.utils.error("Extra arguments require specifying a --stage.")
        raise typer.Exit(1)

    # If specific stage requested, run only that stage
    if stage:
        if stage not in stages:
            metta.setup.utils.error(f"Unknown stage: {stage}")
            metta.setup.utils.info(f"Valid stages: {', '.join(stages.keys())}")
            raise typer.Exit(1)

        # Run the specific stage
        result = stages[stage](verbose, extra_args)
        if result.passed:
            metta.setup.utils.success(f"Stage '{stage}' passed!")
            sys.exit(0)
        else:
            metta.setup.utils.error(f"Stage '{stage}' failed.")
            sys.exit(1)

    # Otherwise run all stages (local development workflow)
    console.print(rich.panel.Panel.fit("[bold]Running All CI Checks[/bold]", border_style="cyan"))
    ...
    for stage_name, stage_func in stages.items():
        ...
        if not result.passed and not continue_on_error:
            ...
            metta.setup.utils.error(f"Stage '{stage_name}' failed. Fix errors and try again.")
            raise typer.Exit(1)
    ...
    if all_passed:
        metta.setup.utils.success("All CI checks passed!")
        sys.exit(0)
    else:
        metta.setup.utils.error("Some CI checks failed.")
        sys.exit(1)
