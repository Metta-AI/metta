...
def _is_staging_area_clean() -> bool:
    # This returns (status_code, filename) tuples.
    # Status codes are 2 chars: first is staged status, second is working tree status.
    # Space or '?' in first char means nothing staged, anything else means staged changes.
    files_by_status = gitta.get_uncommitted_files_and_statuses()
    for status, _filename in files_by_status:
        if status[0] not in (" ", "?"):
            return False
    return True
...
def _pin_dependency_version(*, package: str, dependency: str, version: str, dry_run: bool) -> None:
    ...
    if not _is_staging_area_clean():
        ...
        raise typer.Exit(1)
    gitta.run_git("add", f"{package}/pyproject.toml")

    if not typer.confirm(f"Okay to commit changes to {package}/pyproject.toml?"):
        error("Publishing aborted.")
        raise typer.Exit(1)

    commit_msg = f"chore: pin {package} dependency to {version} for {package} release"
...
def _push_git_history_to_child_repo(*, package: str, dry_run: bool) -> None:
    ...
    try:
        subprocess.run(
            [f"{get_repo_root()}/devops/gitta/push_child_repo.py", package, "-y"],
            check=True,
        )
...
def _publish(
    ...
    if create_and_push_tag_to_monorepo:
        ...
        if package == Package.COGAMES:
            if not typer.confirm("To publish cogames, we must publish mettagrid first. Okay to proceed?"):
                error("Publishing aborted.")
                raise typer.Exit(1)
...
def cmd_publish(
    ...
    tag_only: Annotated[bool, typer.Option("--tag-only", help="Only create/push tag (skip pushing to child repo)")] = False,
    repo_only: Annotated[bool, typer.Option("--repo-only", help="Only push to child repo (skip creating/pushing tag)")] = False,
