...
def get_job_details(
    job, job_descriptions, task_descriptions=None, container_instance_descriptions=None, ec2_instances=None
):
    ...
    if task_descriptions and task_arn and task_arn in task_descriptions:
        ...
        if (
            ...
            and container_instance_arn
            and container_instance_arn in container_instance_descriptions
        ):
    ...
    stop_command = (
        f"aws batch terminate-job --reason man_stop --job-id {job_id}"
        if job_status in ["RUNNING", "RUNNABLE", "STARTING"]
        else ""
    )
...
def get_batch_jobs(job_queue, max_jobs):
    ...
    with ThreadPoolExecutor() as executor:
        job_futures = [
            executor.submit(get_jobs_by_status, state)
            for state in ["RUNNING", "SUBMITTED", "PENDING", "RUNNABLE", "STARTING", "SUCCEEDED", "FAILED"]
        ]
    ...
    for cluster_name, container_instances in container_instances_by_cluster.items():
        for i in range(0, len(container_instances), 100):
            ...
            container_instance_descriptions.update(
                {instance["containerInstanceArn"]: instance for instance in response["containerInstances"]}
            )
    ...
    with ThreadPoolExecutor() as executor:
        job_details = list(
            executor.map(
                lambda job: get_job_details(
                    job, job_descriptions, task_descriptions, container_instance_descriptions, ec2_instances
                ),
                all_jobs,
            )
        )
...
def get_ecs_tasks(clusters, max_tasks):
    ...
    def get_task_details(cluster, task_arn):
        ...
        task_link = (
            f"https://console.aws.amazon.com/ecs/home?region=us-east-1#/clusters/{cluster}/tasks/{task_id}/details"
        )
...
def print_status(jobs_by_queue, tasks, use_color):
    for job_queue, jobs in jobs_by_queue.items():
        ...
        for job in jobs:
            ...
            print_row(
                "Status",
                f"{status_color}{job['status']}{Style.RESET_ALL} ({job['retries']})" if use_color else job["status"],
                use_color,
            )
    ...
    if tasks:
        ...
        for task in tasks:
            ...
            print_row(
                "Status", f"{status_color}{task['status']}{Style.RESET_ALL}" if use_color else task["status"], use_color
            )
