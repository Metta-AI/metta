...
def create_from_checkpoint(
    ...
    if (model_type == "MettaAgent"):
        return _create_metta_agent(metadata)
    elif (model_type == "PytorchAgent"):
        return _create_pytorch_agent(metadata)
    elif (model_type == "DistributedMettaAgent"):
        return _create_distributed_metta_agent(metadata)
    else:

    # Restore from checkpoint
    restore_agent(agent, checkpoint_name)

    return agent


def _create_metta_agent(self, metadata: Dict[str, Any]) -> MettaAgent:
    """Create MettaAgent from metadata"""
    ...
    agent = MettaAgent(
        config=config,
        **hyperparams,  # Unpack hyperparameters as constructor args
    )
    return agent


def _create_pytorch_agent(self, metadata: Dict[str, Any]) -> PytorchAgent:
    """Create PytorchAgent from metadata"""
    config = metadata.get("config", {})
    return PytorchAgent(config=config)


def _create_distributed_metta_agent(metadata: Dict[str, Any], device: torch.device) -> DistributedMettaAgent:
    """Create DistributedMettaAgent from metadata"""
    # First create the underlying MettaAgent
    base_agent = _create_metta_agent(metadata)
