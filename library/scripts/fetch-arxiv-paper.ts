...
export function extractArxivId(input: string): string {
  ...
  throw new Error(`Invalid arXiv identifier: ${input}. Expected format: 2204.11674 or https://arxiv.org/abs/2204.11674`);
}
 ...
 */
export async function fetchArxivPaper(arxivId: string): Promise<ArxivPaperData> {
  try {
    // Clean the arXiv ID
    ...
    const primaryCategory = parsedData.feed.entry['arxiv:primary_category']?.term || categories[0];
    ...
    console.error(`❌ Error fetching arXiv paper: ${error instanceof Error ? error.message : 'Unknown error'}`);
    throw error;
  }
}
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log('  tsx scripts/fetch-arxiv-paper.ts https://arxiv.org/abs/2204.11674');
  ...
  try {
    ...
    console.error(`❌ Failed to fetch paper: ${error instanceof Error ? error.message : 'Unknown error'}`);
    process.exit(1);
  }
}
