...
class TestGitHubCLI:
    ...
    def test_run_gh_failure(self, mock_run):
        ...
        with pytest.raises(gitta.GitError) as exc_info:
            gitta.run_gh("pr", "create")
...
class TestGitHubAPI:
    ...
    def test_get_matched_pr_success(self, mock_get):
        ...
        result = gitta.get_matched_pr("abc123", "Metta-AI/metta")
    ...
    def test_get_matched_pr_404_error(self, mock_get):
        ...
        mock_response.raise_for_status.side_effect = httpx.HTTPStatusError(
            message="Not found", request=unittest.mock.Mock(), response=mock_response
        )
        mock_get.return_value = mock_response
    ...
    @unittest.mock.patch("httpx.post")
    def test_post_commit_status_success(self, mock_post):
        ...
        with unittest.mock.patch.dict(os.environ, {"GITHUB_TOKEN": "test_token"}):
            result = gitta.post_commit_status(
    ...
    def test_post_commit_status_no_token(self):
        ...
        with unittest.mock.patch.dict(os.environ, {}, clear=True):
            with pytest.raises(ValueError) as exc_info:
                gitta.post_commit_status("abc123", "success", "Metta-AI/metta")
    ...
    def test_post_commit_status_no_repo(self):
        ...
        with unittest.mock.patch.dict(os.environ, {"GITHUB_TOKEN": "test_token"}):
            with pytest.raises(ValueError) as exc_info:
                gitta.post_commit_status("abc123", "success", "")
    ...
    def test_create_pr_no_token(self):
        ...
        with unittest.mock.patch.dict(os.environ, {}, clear=True):
            with pytest.raises(ValueError) as exc_info:
                gitta.create_pr(repo="Metta-AI/metta", title="Test", body="Test", head="feature", base="main")
            assert "secret not found" in str(exc_info.value).lower()
    ...
    def test_create_pr_api_error(self, mock_post):
        ...
        with unittest.mock.patch.dict(os.environ, {"GITHUB_TOKEN": "test_token"}):
            with pytest.raises(gitta.GitError) as exc_info:
                gitta.create_pr(repo="Metta-AI/metta", title="Test", body="Test", head="feature", base="invalid")
