...
class OptimizerConfig(BaseModelWithForbidExtra):
    ...
    learning_rate: float = Field(default=0.00509275, gt=0, le=1.0)  # 25% toward sweep: 0.000457 -> 0.019
    # Beta1: Standard Adam default from Kingma & Ba (2014) "Adam: A Method for Stochastic Optimization"
    beta1: float = Field(default=0.8975, ge=0, le=1.0)  # 25% toward sweep: 0.9 -> 0.89
    # Beta2: Standard Adam default from Kingma & Ba (2014)
    beta2: float = Field(default=0.98925, ge=0, le=1.0)  # 25% toward sweep: 0.999 -> 0.96
    # Epsilon: Type 2 default chosen arbitrarily
    eps: float = Field(default=7e-12, gt=0)  # CONSERVATIVE 5% toward sweep: 1e-12 -> 1.4e-7
...
class PrioritizedExperienceReplayConfig(BaseModelWithForbidExtra):
    ...
    prio_alpha: float = Field(default=0.1975, ge=0, le=1.0)  # 25% toward sweep: 0.0 -> 0.79
    # Beta0=0.6: From Schaul et al. (2016) "Prioritized Experience Replay" paper
    prio_beta0: float = Field(default=0.5975, ge=0, le=1.0)  # 25% toward sweep: 0.6 -> 0.59
...
class VTraceConfig(BaseModelWithForbidExtra):
    ...
    vtrace_rho_clip: float = Field(default=1.325, gt=0)  # 25% toward sweep: 1.0 -> 2.3
    # V-trace c clipping at 1.0: From IMPALA paper (Espeholt et al., 2018), standard for on-policy
    vtrace_c_clip: float = Field(default=1.275, gt=0)  # 25% toward sweep: 1.0 -> 2.1
...
class PPOConfig(BaseModelWithForbidExtra):
    ...
    clip_coef: float = Field(default=0.1125, gt=0, le=1.0)  # 25% toward sweep: 0.1 -> 0.15
    # Entropy coefficient: Type 2 default chosen from sweep
    ent_coef: float = Field(default=0.00447, ge=0)  # CONSERVATIVE 15% toward sweep: 0.0021 -> 0.017
    # GAE lambda: Type 2 default chosen from sweep, deviates from typical 0.95, bias/variance tradeoff
    gae_lambda: float = Field(default=0.897, ge=0, le=1.0)  # 25% toward sweep: 0.916 -> 0.84
    # Gamma: Type 2 default chosen from sweep, suggests shorter effective horizon for multi-agent
    gamma: float = Field(default=0.98025, ge=0, le=1.0)  # 25% toward sweep: 0.977 -> 0.99

    # Training parameters
    # Gradient clipping: 0.5 is standard PPO default to prevent instability
    max_grad_norm: float = Field(default=0.71, gt=0)  # CONSERVATIVE 10% toward sweep: 0.5 -> 2.6 (paired with high LR)
    # Value function clipping: Matches policy clip for consistency
    vf_clip_coef: float = Field(default=0.115, ge=0)  # 25% toward sweep: 0.1 -> 0.16
    # Value coefficient: Type 2 default chosen from sweep, balances policy vs value loss
    vf_coef: float = Field(default=1.13, ge=0)  # 25% toward sweep: 0.44 -> 3.2
...
class TrainerConfig(BaseModelWithForbidExtra):
    ...
    total_timesteps: int = Field(default=20_000_000_000, gt=0)  # 25% toward sweep: 10B -> 50B
