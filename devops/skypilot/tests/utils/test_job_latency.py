...
import devops.skypilot.utils.job_latency
...
class TestSkyPilotLatency:
    ...
    def test_parse_submission_timestamp_valid(self):
        ...
        ts = devops.skypilot.utils.job_latency.parse_submission_timestamp(task_id)
        assert ts.year == 2024
        assert ts.month == 1
        assert ts.day == 15
        assert ts.hour == 14
    ...
    def test_parse_submission_timestamp_invalid_format(self):
        ...
        for task_id in invalid_ids:
            with pytest.raises(ValueError):
                devops.skypilot.utils.job_latency.parse_submission_timestamp(task_id)
    ...
    def test_calculate_queue_latency_success(self):
        ...
        latency = devops.skypilot.utils.job_latency.calculate_queue_latency()
    ...
    def test_calculate_queue_latency_no_env(self):
        ...
        with pytest.raises(RuntimeError, match="SKYPILOT_TASK_ID environment variable not set"):
            devops.skypilot.utils.job_latency.calculate_queue_latency()
    ...
    def test_queue_latency_precision(self):
        ...
        for microseconds in [0, 123456, 999999]:
            ...
            parsed_time = devops.skypilot.utils.job_latency.parse_submission_timestamp(task_id)
