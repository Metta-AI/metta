...
import torch
from metta.agent.components.mamba_ssm.ops.triton.layer_norm import RMSNorm, layer_norm_fn
...
class Block(nn.Module):
    ...
    def forward(self, hidden_states: Tensor, residual: Optional[Tensor] = None, inference_params=None, **mixer_kwargs):
        ...
        """
        norm_bias = getattr(self.norm, "bias", None)
        ...
        if self.mlp is not None:
            norm2_bias = getattr(self.norm2, "bias", None)
            if not self.fused_add_norm:
