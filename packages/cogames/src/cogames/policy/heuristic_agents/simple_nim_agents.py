...
import heuristic_agents as ha  # noqa: E402  # type: ignore
import numpy as np  # noqa: E402

from mettagrid.policy.policy import AgentPolicy, MultiAgentPolicy  # noqa: E402
...
class HeuristicAgentPolicy(AgentPolicy):
    def __init__(self, policy_env_info: PolicyEnvInterface, agent_id: int, debug: bool = False):
...
class HeuristicAgentsPolicy(MultiAgentPolicy):
    def __init__(self, policy_env_info: PolicyEnvInterface, debug: bool = False):
        super().__init__(policy_env_info)
        self._debug = debug

    def agent_policy(self, agent_id: int) -> HeuristicAgentPolicy:
        return HeuristicAgentPolicy(self._policy_env_info, agent_id, debug=self._debug)
