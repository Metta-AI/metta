...
TimerReporter times are seconds (mean of 3 runs per variant unless noted). KSPS from progress logs hover ~66–70 for optimized runs. Env vars listed apply only to the optimized run; baseline always has `optimized_obs` off.
...
- Move-swap bugfix sanity check (single run, default toggles): baseline rollout 10.19 s (send 6.95), optimized rollout 8.27 s (send 4.94); confirms keeping dst clean preserves the swap benefit.
