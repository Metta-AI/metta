...
class SimTaskExecutor(AbstractTaskExecutor):
    ...
    def _upload_logs_to_s3(self, job_id: str, process: subprocess.CompletedProcess) -> None:
        log(f"Uploading logs to S3: {job_id}")
    ...
    def _setup_versioned_checkout(self, git_hash: str) -> None:
        ...
        if os.path.exists(self._versioned_path):
            log(f"Versioned checkout already exists at {self._versioned_path}")
            return

        log(f"Setting up versioned checkout at {self._versioned_path}")
        ...
        log("Installing dependencies in versioned checkout...")
        ...
        log(f"Successfully set up versioned checkout at {self._versioned_path}")
    ...
    async def execute_task(
        ...
        log(f"Simulation completed successfully: {result.stdout}")
...
class EvalTaskWorker:
    ...
    async def run(self) -> None:
        log("Starting eval worker")
        log(f"Worker id: {self._assignee}")
        while True:
            ...
            try:
                ...
                if claimed_tasks.tasks:
                    ...
                    try:
                        ...
                        log(f"Task {task.id} updated to {status}")
                    except Exception as e:
                        log(f"Task failed: {e}", exc_info=True, level=logging.ERROR)
                        await self._update_task_status(
                            task.id,
                            "error",
                            str(e),
                    ...
                    log("No tasks claimed", level=logging.DEBUG)
                ...
                log(f"Error in worker loop: {e}", exc_info=True, level=logging.ERROR)
