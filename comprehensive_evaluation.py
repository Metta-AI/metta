#!/usr/bin/env python3
"""
Comprehensive Evaluation Script for CoGames Scripted Agent

This script runs evaluations across:
- All evaluation missions (original, clipped, difficulty variants)
- All difficulty levels (easy, medium, hard, extreme, single_use, speed_run, energy_crisis)
...
def run_single_evaluation(
    ...
    try:
        ...
        num_cogs = 1
        env = MettaGridEnv(mission.instantiate(map_builder, num_cogs))
        
        # Create agent with hyperparameters
        agent = ScriptedAgentPolicy(env, hyperparams=hyperparams)
...
def run_comprehensive_evaluation(
    ...
    if difficulties is None:
        difficulties = ["default", "easy", "medium", "hard", "extreme"]
    if hyperparams is None:
        hyperparams = create_hyperparameter_sets()
    if seeds is None:
        seeds = [1, 2, 3, 4, 5]  # 5 seeds for statistical significance
