...
## COGNITIVE INTERFACE IMPLEMENTATION

### Basic Policy Interface

Your cognitive models should implement the `AgentPolicy.step()` method which takes an observation and returns an action:
...
> **Note**: `AgentPolicy` is appropriate for use in `play` and `evaluate`, but not `train`.

### Batch Command Processing

For training and efficient batch processing, your cognitive models should implement `MultiAgentPolicy.step_batch()`:
...
class MyMultiAgentPolicy(MultiAgentPolicy):
    def step_batch(self, raw_observations: np.ndarray, raw_actions: np.ndarray) -> None:
        ...
        Args:
            raw_observations: Observation array with shape (num_cogs, num_tokens, 3)
            raw_actions: Output buffer with shape (num_cogs,) - write actions here as int32
        """
        num_cogs = raw_observations.shape[0]

        for agent_id in range(num_cogs):
            # Extract observation tokens for this cog
...
- **`raw_observations`**: NumPy array with shape `(num_cogs, num_tokens, 3)` containing observation tokens
- **`raw_actions`**: NumPy array with shape `(num_cogs,)` - write action IDs here as `int32` values

Commands are written directly into the `raw_actions` buffer, which is then used by the environment. This allows for
