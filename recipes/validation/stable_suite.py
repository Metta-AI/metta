...
def get_stable_jobs(prefix: str) -> list[JobConfig]:
    ...
    cvc_all_maps_200ep_name = f"{prefix}.cvc_all_maps_mettabox_200ep"
    cvc_all_maps_200ep_timesteps = 200 * 524_288  # 200 epochs * default batch size
    cvc_all_maps_train_200ep = JobConfig(
        name=cvc_all_maps_200ep_name,
        module="recipes.prod.cvc.all_maps.train",
        args=[
            f"run={cvc_all_maps_200ep_name}",
            f"trainer.total_timesteps={cvc_all_maps_200ep_timesteps}",
            "num_cogs=4",
    ...
    # Multi-GPU training - 2B timesteps
    cvc_all_maps_train_name = f"{prefix}.cvc_all_maps_multi_gpu_2b"
    cvc_all_maps_train_2b = JobConfig(
        name=cvc_all_maps_train_name,
        module="recipes.prod.cvc.all_maps.train",
        args=[
            f"run={cvc_all_maps_train_name}",
            "trainer.total_timesteps=2000000000",
            "num_cogs=4",
            'variants=["lonely_heart","heart_chorus","pack_rat"]',
        ],
    ...
    return [
        ...
        arena_train_2b,
        arena_eval,
        cvc_all_maps_train_200ep,
        cvc_all_maps_train_2b,
    ]
