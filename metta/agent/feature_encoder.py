...
class FeatureSetEncoder(nn.Module):
    ...
    def forward(self, obs_dict):
        ...
        labeled_obs = torch.cat(
            [self._labels_emb.expand(batch_size, -1, -1), obs.view(batch_size, self._num_features, self._input_dim)],
            dim=-1,
        )
...
class MultiFeatureSetEncoder(nn.Module):
    def __init__(self, obs_space, grid_features, global_features, fc_cfg: OmegaConf, **cfg):
        super().__init__()
