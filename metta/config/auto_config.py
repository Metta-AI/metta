...
if TYPE_CHECKING:  # pragma: no cover - kept for type checkers only
...
AWSSetup: Callable[[], Any] | None = None
ObservatoryKeySetup: Callable[[], Any] | None = None
WandbSetup: Callable[[], Any] | None = None


@lru_cache(maxsize=None)
def _get_setup_class(module_path: str, class_name: str) -> Type[Any]:
    module = importlib.import_module(module_path)
    return getattr(module, class_name)


def _wandb_setup() -> "WandbSetup":
    if WandbSetup is not None:
        return WandbSetup()  # type: ignore[operator]
    cls = _get_setup_class("metta.setup.components.wandb", "WandbSetup")
    return cls()  # type: ignore[arg-type]


def _observatory_setup() -> "ObservatoryKeySetup":
    if ObservatoryKeySetup is not None:
        return ObservatoryKeySetup()  # type: ignore[operator]
    cls = _get_setup_class("metta.setup.components.observatory_key", "ObservatoryKeySetup")
    return cls()  # type: ignore[arg-type]


def _aws_setup() -> "AWSSetup":
    if AWSSetup is not None:
        return AWSSetup()  # type: ignore[operator]
    cls = _get_setup_class("metta.setup.components.aws", "AWSSetup")
    return cls()  # type: ignore[arg-type]
...
def auto_stats_server_uri() -> str | None:
    return {
        **_observatory_setup().to_config_settings(),  # type: ignore[arg-type]
...
def auto_replay_dir() -> str:
    ...
    return {
        **aws_setup_module.to_config_settings(),  # type: ignore[arg-type]
