...
class ObservatoryService:
    ...
    @staticmethod
    def get_policy_api_url(experiment_id: str, limit: int = 1) -> str:
        """Get Observatory API URL for fetching policy information.

        Args:
            experiment_id: Experiment/policy name to search for
            limit: Maximum number of results to return

        Returns:
            Full API URL
        """
        return f"{ServiceEndpoints.OBSERVATORY_API}/stats/policies?name_fuzzy={experiment_id}&limit={limit}"

    @staticmethod
    def get_policy_web_url(policy_name: str) -> str:
        """Get Observatory web URL for viewing a policy.
    ...
    def fetch_policy_version(experiment_id: str, timeout: int = 5) -> Optional[str]:
        ...
        """
        try:
            url = ObservatoryService.get_policy_api_url(experiment_id, limit=1)
            ...
            logger.debug(f"Could not fetch policy version for {experiment_id}: {e}")
