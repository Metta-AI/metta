...
class StatefulCurriculum(Curriculum):
    ...
    def get_env_cfg_by_bucket(self) -> dict[str, DictConfig]:
        return {
            "easy": OmegaConf.create({"game": {"difficulty": "easy", "width": 10, "height": 10}}),
            "hard": OmegaConf.create({"game": {"difficulty": "hard", "width": 20, "height": 20}})
        }
...
def test_random_curriculum_integration():
    ...
    with patch('metta.mettagrid.curriculum.random.curriculum_from_config_path') as mock_load:
        ...
        for task_id in tasks:
            ...
            mock_curr.get_env_cfg_by_bucket.return_value = {
                task_id: mock_task.env_cfg()
            }
