...
class FilterRepoSetup(SetupModule):
    ...
    def install(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if (
                user_bin.exists()
                and str(user_bin) in subprocess.run(["bash", "-c", "echo $PATH"], capture_output=True, text=True).stdout
            ):
            ...
            for location in install_locations:
                ...
                try:
                    ...
                    if location in [Path("/usr/local/bin"), Path("/usr/bin")]:
                        ...
                        subprocess.run(["sudo", "cp", str(script_path), str(dest_path)], check=True)
                    else:
                        # Direct copy
