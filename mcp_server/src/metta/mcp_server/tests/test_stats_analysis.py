...
class TestStatsExtractor:
    ...
    def test_extract_from_objects_format_replay(self, objects_format_replay):
        ...
        assert game_stats["total_agents"] == 2
        ...
        assert len(agent_stats) == 2  # All agents in test data
    ...
    def test_extract_from_grid_objects_format_replay(self, grid_objects_format_replay):
        ...
        agent_stats = result["agent"]
        assert isinstance(agent_stats, list)
        assert len(agent_stats) == 1  # All agents in test data
        ...
        # Verify actions have valid format (either named actions or action_N format)
        action_names = list(first_agent["action_counts"].keys())
        assert len(action_names) > 0, "Should have at least one action"
        # All action names should be valid (either named actions or action_N format)
        valid_actions = [
            name for name in action_names if name in ["attack", "get_items", "move"] or name.startswith("action_")
        ]
        assert len(valid_actions) == len(action_names), f"All actions should be valid, got: {action_names}"
...
class TestBehavioralAnalysisEngine:
    ...
    def test_analyze_agent_behaviors_real_data(self, replay_format, request):
        ...
        # Verify efficiency scores structure (may be zero if no reward data in replay)
        efficiency_scores = [ranking.get("efficiency_score", 0) for ranking in rankings]
        assert len(efficiency_scores) == len(agent_stats), "Should have efficiency score for each agent"
        # All scores should be numeric (may be zero for test data without rewards)
        assert all(isinstance(score, (int, float)) for score in efficiency_scores), "All scores should be numeric"
...
class TestResourceFlowAnalyzer:
    ...
    def test_analyze_resource_flows_real_data(self, replay_format, request):
        ...
        # Verify scarcity analysis structure (may be zero if no resource flow data)
        ...
        # All values should be non-negative (scarcity can't be negative)
        assert all(v >= 0 for v in scarcity_values), "Scarcity values should be non-negative"
