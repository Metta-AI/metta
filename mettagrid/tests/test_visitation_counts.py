...
def test_visitation_performance_impact(performance_config, simple_map):
    ...
    if enabled_time > 0:
        ...
        # Allow for small variance (10%) in timing measurements
        assert disabled_time <= enabled_time * 1.1, f"Disabling visitation counts should not slow down the environment by more than 10% (enabled: {enabled_time:.6f}s, disabled: {disabled_time:.6f}s)"
