...
def _find_object_anchor(env: MettaGridCore, type_name: str) -> tuple[int, tuple[int, int, int]]:
    env_objects = env.grid_objects()
    for object_id, obj in env_objects.items():
        if obj.get("type_name") == type_name:
            return object_id, (obj["r"], obj["c"], obj.get("layer", 0))
...
class TestAssemblerProperties:
    ...
    @staticmethod
    def _add_linear_cells(env: MettaGridCore, object_id: int, r: int, c: int, layer: int) -> tuple[tuple[int, int], tuple[int, int]]:
        directions = ((0, 1), (1, 0), (0, -1), (-1, 0))
        for dr, dc in directions:
            first = (r + dr, c + dc)
            ok_first = env.add_object_cell(object_id, first[0], first[1], layer)
            if not ok_first:
                continue
            second = (r + 2 * dr, c + 2 * dc)
            ok_second = env.add_object_cell(object_id, second[0], second[1], layer)
            if ok_second:
                return first, second
            removed = env.remove_object_cell(object_id, first[0], first[1], layer)
            assert removed, "Failed to remove temporary cell after unsuccessful attempt"
        raise AssertionError("Could not add two linear cells for assembler test")

    def test_assembler_multi_cell_add_remove_and_bbox(self, env_with_assembler):
        env_with_assembler.reset()

        assembler_id, (r, c, layer) = _find_object_anchor(env_with_assembler, "assembler")

        first_cell, second_cell = self._add_linear_cells(env_with_assembler, assembler_id, r, c, layer)

        objects = env_with_assembler.grid_objects()
        assembler = objects[assembler_id]
        cells = set((rr, cc, ll) for cc, rr, ll in assembler.get("cells", []))
        assert (r, c, layer) in cells
        assert (first_cell[0], first_cell[1], layer) in cells
        assert (second_cell[0], second_cell[1], layer) in cells

        removed_middle = env_with_assembler.remove_object_cell(assembler_id, first_cell[0], first_cell[1], layer)
        assert not removed_middle, "Removing middle cell should fail due to connectivity requirement"

        removed_far = env_with_assembler.remove_object_cell(assembler_id, second_cell[0], second_cell[1], layer)
        ...
        assert assembler_id not in objs_far, "Assembler should be excluded when only far cell area is queried after removal"
    ...
    def test_assembler_add_object_cell_rejects_diagonal_only_connection(self, env_with_assembler):
        ...
        for obj in objects.values():
            for c_loc, r_loc, layer_loc in obj.get("cells", []):
        ...
        ok = env_with_assembler.add_object_cell(assembler_id, diagonal_target[0], diagonal_target[1], layer)
        assert not ok, "Diagonal connections should be rejected by connectivity rule"
    ...
    def test_assembler_add_object_cell_rejects_other_layer(self, env_with_assembler):
        ...
        ok = env_with_assembler.add_object_cell(assembler_id, adjacent_target[0], adjacent_target[1], other_layer)
        assert not ok, "Adding cells on a different layer should be rejected"

    def test_assembler_remove_object_cell_rejects_anchor(self, env_with_assembler):
        env_with_assembler.reset()

        assembler_id, (r, c, layer) = _find_object_anchor(env_with_assembler, "assembler")

        removed = env_with_assembler.remove_object_cell(assembler_id, r, c, layer)
        assert not removed, "Removing the anchor cell should be rejected"

    def test_assembler_supports_large_connected_shapes(self, env_with_assembler):
        env_with_assembler.reset()
        ...
        target_total = 12  # Anchor + 11 extra cells exceed the old hard limit
        visited = {(r, c)}
        queue = [(r, c)]
        attempted = {(r, c)}
        ...
        while queue and len(visited) < target_total:
            ...
            for dr, dc in directions:
                ...
                if (nr, nc) in attempted:
                    continue
                attempted.add((nr, nc))
                ok = env_with_assembler.add_object_cell(assembler_id, nr, nc, layer)
                if ok:
                    visited.add((nr, nc))
                    queue.append((nr, nc))
        ...
        cells = assembler.get("cells", [])
        assert "max_cells" not in assembler, "max_cells should no longer be exposed"
        assert len(cells) == len(visited), "Reported cells should match cells tracked in the environment"
        assert len(cells) == len({(rr, cc) for cc, rr, _ in cells}), "Cells should remain unique"
        assert len(cells) >= 8, "Assembler should be able to occupy well beyond 5 cells"
