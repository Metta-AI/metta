...
export const PolicySelector: React.FC<PolicySelectorProps> = React.memo(({
  ...
  useEffect(() => {
    setAllTrainingRunsSelected(
      ...
      filteredTrainingRuns.every(run => selectedTrainingRunIds.includes(run.id))
    )
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.policiesContainer}>
        ...
        {!loading && filteredTrainingRuns.length === 0 && filteredRunFreePolicies.length === 0 && (
          <div className={styles.emptyMessage}>
            {debouncedSearchText ? 'No training runs or policies match your search.' : 'No training runs or policies available.'}
          </div>
        )}
        ...
        {!loading && filteredTrainingRuns.length > 0 && (
          <div className={styles.section}>
            <div className={styles.sectionHeader}>
              ...
              <button
                onClick={handleSelectAllTrainingRuns}
            ...
            {filteredTrainingRuns.map(run => (
              <div
                ...
                className={`${styles.policyItem} ${
                  selectedTrainingRunIds.includes(run.id) ? styles.selected : ''
                }`}
        ...
        {!loading && filteredRunFreePolicies.length > 0 && (
          <div className={styles.section}>
            <div className={styles.sectionHeader}>
              ...
              <button
                onClick={handleSelectAllRunFreePolicies}
                className={styles.selectAllButton}
              >
      ...
      <div className={styles.selectedCount}>
        <span>
          {selectedTrainingRunIds.length + selectedRunFreePolicyIds.length} selected
          ({selectedTrainingRunIds.length} training runs, {selectedRunFreePolicyIds.length} run-free policies)
          {policiesData && (
            <span> • Page {policiesData.page} of {Math.ceil(policiesData.total_count / policiesData.page_size)}</span>
          )}
        </span>
