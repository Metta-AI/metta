...
export class Context3d {
  ...
  private layoutText(
    ...
    quads: { x: number; y: number; w: number; h: number; u0: number; v0: number; u1: number; v1: number }[]
    width: number
    top: number
    bottom: number
  } {
    ...
    const quads: { x: number; y: number; w: number; h: number; u0: number; v0: number; u1: number; v1: number }[] = []
    let penX = 0
    let penY = 0
    ...
    for (const ch of text) {
      ...
      // Advance-only for spaces/tabs without drawing a glyph.
      if (ch === ' ') {
        if (glyph) {
          penX += glyph.advance
      ...
      const x = penX + glyph.bearingX - glyphInnerPadding - mBase
      const y = penY + glyph.bearingY - glyphInnerPadding - mBase
      const w = sw + 2 * mBase
      const h = sh + 2 * mBase

      const glyphTop = y
      const glyphBottom = y + h
    ...
    return { quads, width: maxWidth, top, bottom }
  }

  /** Draw text in screen space using sprites from the atlas. */
  drawTextScreen(fontName: string, text: string, x: number, y: number, color: number[] = [1, 1, 1, 1], scale = 1) {
    if (!this.ready) {
      throw new Error('Drawer not initialized')
    }
    this.ensureMeshSelected()
    ...
    const layout = this.layoutText(fontName, text)
    for (const q of layout.quads) {
      this.drawRect(q.x, q.y, q.w, q.h, q.u0, q.v0, q.u1, q.v1, color)
    }
  ...
  measureTextBounds(fontName: string, text: string): { width: number; top: number; bottom: number } {
    ...
    const layout = this.layoutText(fontName, text)
    return { width: layout.width, top: layout.top, bottom: layout.bottom }
  }

  /**
   * Draw text in using sprites from the atlas.
   * Uses the current transform to position the text.
   */
  drawText(fontName: string, text: string, color: number[] = [1, 1, 1, 1]) {
    if (!this.ready) {
      throw new Error('Drawer not initialized')
    }
    this.ensureMeshSelected()
