...
class JobConfig(Config):
    ...
    def migrate_legacy_fields(cls, data: Any) -> Any:
        """Backwards compatibility: migrate old field names to new API."""
        if isinstance(data, dict):
            # Migrate 'module' to 'tool'
            if "module" in data:
                data["tool"] = data.pop("module")

            # Migrate 'is_training_job' to 'metrics_source'
            if "is_training_job" in data:
                is_training = data.pop("is_training_job")
                if is_training and "metrics_source" not in data:
                    data["metrics_source"] = MetricsSource.WANDB
