...
from softmax.cli.components.base import SetupModuleStatus
from softmax.cli.local_commands import app as local_app
from softmax.cli.symlink_setup import app as symlink_app
from softmax.cli.tools.book import app as book_app
from softmax.cli.utils import debug, error, info, success, warning
...
if TYPE_CHECKING:
    from softmax.cli.registry import SetupModule
...
class MettaCLI:
    ...
    def _init_all(self):
        ...
        from softmax.cli.utils import import_all_modules_from_subpackage

        import_all_modules_from_subpackage("softmax.cli", "components")
    ...
    def setup_wizard(self, non_interactive: bool = False):
        ...
        from softmax.cli.utils import header, info, prompt_choice, success
    ...
    def _custom_setup(self, non_interactive: bool = False):
        ...
        from softmax.cli.registry import get_all_modules
        from softmax.cli.saved_settings import get_saved_settings
        from softmax.cli.utils import prompt_choice
...
def cmd_shell():
    cmd = ["uv", "run", "--active", "python", "-m", "softmax.cli.shell"]
    try:
        subprocess.run(cmd, cwd=cli.repo_root, check=True)
    except subprocess.CalledProcessError as e:
        raise typer.Exit(e.returncode) from e
