...
class ActionProbs(nn.Module):
    ...
    def forward_inference(self, td: TensorDict) -> TensorDict:
        ...
        action = action.reshape(*original_shape, action.shape[-1])
        selected_log_probs = selected_log_probs.reshape(*original_shape)
        full_log_probs = full_log_probs.reshape(*original_shape, num_actions)
