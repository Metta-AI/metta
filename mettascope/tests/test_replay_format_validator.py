...
def _validate_building_object(obj: dict, obj_index: int) -> None:
    ...
    for field in ["is_converting", "is_cooling_down"]:
        if field in obj:
            _assert(isinstance(obj[field], bool), f"Building object {obj_index} '{field}' must be boolean")
...
def _find_generated_replay_files() -> List[Path]:
    """Find replay files generated by the training/replay process.

    Returns:
        List of paths to .json.z replay files found in the expected directories.
    """
    replay_paths = []

    # Check common replay directories based on the CI configuration
    potential_dirs = [
        Path("./train_dir/smoke_test/replays"),  # CI run directory
        Path("./train_dir/github_test/replays"),  # Alternative CI run name
        Path("./replays"),  # Default local directory
        Path("./outputs"),  # Alternative output directory
    ]

    for base_dir in potential_dirs:
        if base_dir.exists():
            # Look for .json.z files recursively
            for replay_file in base_dir.glob("**/*.json.z"):
                replay_paths.append(replay_file)

    return replay_paths


def test_validate_real_generated_replay() -> None:
    """Test validation against actual generated replay files when available.

    This test attempts to find and validate real replay files generated by the
    training or replay process. If no real replay files are found, it falls back
    to testing with synthetic data to ensure the test doesn't fail in environments
    where no replays have been generated.
    """
    generated_replays = _find_generated_replay_files()

    version_2_replays = []
    if generated_replays:
        # Filter for version 2 replays only
        for replay_path in generated_replays[:10]:  # Check first 10 files
            try:
                loaded_replay = load_replay(replay_path)
                if loaded_replay.get("version") == 2:
                    version_2_replays.append(replay_path)
            except Exception as e:
                print(f"⚠ Could not load replay {replay_path}: {e}")
                continue

    if version_2_replays:
        # Test with real version 2 replay files
        for replay_path in version_2_replays[:3]:  # Limit to first 3 files for speed
            try:
                loaded_replay = load_replay(replay_path)
                # Should not raise an exception
                validate_replay_schema(loaded_replay)
                print(f"✓ Successfully validated real replay: {replay_path}")
            except Exception as e:
                pytest.fail(f"Failed to validate real v2 replay {replay_path}: {e}")
    else:
        # Fall back to synthetic data if no real v2 replays found
        print("No version 2 replay files found, testing with synthetic data")
        synthetic_replay = _make_valid_replay("fallback_test.json.z")
        validate_replay_schema(synthetic_replay)
