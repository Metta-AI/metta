...
void MettaGrid::_init_buffers(unsigned int num_agents) {
  ...
  auto zero_actions = py::array_t<ActionType, py::array::c_style>({static_cast<ssize_t>(_agents.size())});
...
void MettaGrid::init_action_handlers(const GameConfig& game_config) {
  ...
  // ChangeGlyph
  auto change_glyph_config = std::static_pointer_cast<const ChangeGlyphActionConfig>(game_config.actions.at("change_glyph"));
  auto change_glyph = std::make_unique<ChangeGlyph>(*change_glyph_config, &game_config);
  change_glyph->init(_grid.get(), &_rng);
  if (change_glyph->priority > _max_action_priority) _max_action_priority = change_glyph->priority;
  for (const auto& action : change_glyph->actions()) {
    _action_handlers.push_back(action);
  }
  _action_handler_impl.push_back(std::move(change_glyph));
...
void MettaGrid::set_actions(const py::array_t<ActionType, py::array::c_style>& actions) {
  _actions = actions;
}
...
PYBIND11_MODULE(mettagrid_c, m) {
  ...
  py::class_<MettaGrid>(m, "MettaGrid")
      ...
      .def("set_actions", &MettaGrid::set_actions, py::arg("actions").noconvert())
      .def("set_buffers",
