...
def attach_train_command(
    ...
    *,
    ...
    def train_tribal_cmd(  # noqa: PLR0913 - CLI surface mirrors cogames train
        ...
        checkpoints_path: Path = typer.Option(
            DEFAULTS["checkpoints_path"],
            "--checkpoints",
            help="Path to save training data",
        ),
        steps: int = typer.Option(DEFAULTS["steps"], "--steps", "-s", help="Number of training steps", min=1),
        device: str = typer.Option(
            "auto",
            "--device",
            help="Device to train on (e.g. 'auto', 'cpu', 'cuda')",
        ),
        seed: int = typer.Option(DEFAULTS["seed"], "--seed", help="Seed for training", min=0),
        batch_size: int = typer.Option(DEFAULTS["batch_size"], "--batch-size", help="Batch size for training", min=1),
        minibatch_size: int = typer.Option(
            DEFAULTS["minibatch_size"],
            "--minibatch-size",
        ...
        log_outputs: bool = typer.Option(DEFAULTS["log_outputs"], "--log-outputs", help="Log training outputs"),
    ) -> None:
