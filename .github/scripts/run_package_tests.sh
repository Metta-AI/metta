...
run_package_tests() {
    ...
    if [ "$package" == "core" ]; then
        (pytest $PYTEST_BASE_ARGS \
            ...
            2>&1; echo $? > test-results/${package_name}.exit) | tee "$raw_output" | while IFS= read -r line; do
        ...
        (cd "$package" && pytest $PYTEST_BASE_ARGS \
            ...
            2>&1; echo $? > ${path_prefix}test-results/${package_name}.exit) | tee "$raw_output" | while IFS= read -r line; do
              echo -e "${color}[${package_name}]${NC} $line"
            done
    fi
