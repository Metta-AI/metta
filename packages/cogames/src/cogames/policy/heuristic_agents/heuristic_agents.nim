...
proc newHeuristicAgent(agentId: int, environmentConfig: string): HeuristicAgent {.raises: [].} =
  ...
  try:
    ...
    result.planner = newPriorityAgent()
    result.chestInventory = initTable[ResourceKind, int]()
    result.chestTargets = initTable[ResourceKind, int]()
    for resource in HeartResources:
      result.chestTargets[resource] = defaultChestTarget
    result.assemblerLocation = none(Location)
    result.chestLocation = none(Location)
    result.homeLocation = none(Location)
...
proc step(
  ...
  try:
    ...
    let snapshot = agent.buildSnapshot(map)
