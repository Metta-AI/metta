...
type
  ...
  SettingsState* = object
    ...
    lockFocus*: bool
    aoeEnabledCommons*: seq[int]  ## List of commons IDs with AOE enabled
  ...
  if fileExists(configPath):
    try:
      ...
      if json.hasKey("settings"):
        ...
        if s.hasKey("aoeEnabledCommons"):
          for id in s["aoeEnabledCommons"]:
            result.settings.aoeEnabledCommons.add(id.getInt())
    except:
