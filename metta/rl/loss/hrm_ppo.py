...
class HRMPPOConfig(PPOConfig):
    ...
    def create(
        ...
        """Points to the HRMPPO class for initialization."""
        return HRMPPO(
            policy,
            trainer_cfg,
            env,
...
class HRMPPO(PPO):
    """PPO loss with ACT Q-learning for HRM policies."""

    def compute_act_loss(self, policy_td: TensorDict, advantages: Tensor) -> tuple[Tensor, dict[str, Tensor]]:
        """Compute ACT Q-learning loss for HRM.

        Q-halt should predict: "Should I stop reasoning?"
        Target: Based on whether advantage is positive (good action taken).
        """
        cfg = self.loss_cfg

        # Get Q-logits from HRM forward pass
        if "q_halt_logits" not in policy_td:
            # HRM not in use, return zero loss
            return torch.tensor(0.0, device=self.device), {}

        q_halt_logits = policy_td["q_halt_logits"]  # Shape varies

        # Flatten to match advantages shape
        q_halt_logits_flat = q_halt_logits.reshape(-1)
        advantages_flat = advantages.reshape(-1)

        # Target: Did reasoning lead to good outcome?
        # Use advantage as signal: positive advantage = good, negative = bad
        with torch.no_grad():
            if cfg.use_advantage_target:
                # Advantage-based target (recommended)
                target = (advantages_flat > 0).float()
            else:
                # Alternative: use value improvement
                # For now, just use constant target for stability
        ...
        # Metrics
        with torch.no_grad():
            q_halt_pred = (q_halt_logits_flat > 0).float()
            q_halt_accuracy = (q_halt_pred == target).float().mean()

        metrics = {
            "q_halt_loss": q_halt_loss.detach(),
            "q_halt_accuracy": q_halt_accuracy,
            "q_halt_mean": q_halt_logits_flat.mean().detach(),
        }

        return q_halt_loss, metrics
    ...
    def _process_minibatch_update(
        ...
        # Compute ACT Q-learning loss
        act_loss, act_metrics = self.compute_act_loss(policy_td, adv)

        # Combine losses: PPO + ACT
        loss = pg_loss - cfg.ent_coef * entropy_loss + v_loss * cfg.vf_coef + cfg.act_loss_coef * act_loss
