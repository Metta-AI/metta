...
def test_get_replay_urls(tmp_path: Path):
    ...
    policy1_v1_urls = db.get_replay_urls(policy_uri=CheckpointManager.normalize_uri("policy1/checkpoints/v1.pt"))
    assert len(policy1_v1_urls) == 1
    assert replay_urls[0] in policy1_v1_urls

    # Test filtering by policy URI (policy1 version 2)
    policy1_v2_urls = db.get_replay_urls(policy_uri=CheckpointManager.normalize_uri("policy1/checkpoints/v2.pt"))
    assert len(policy1_v2_urls) == 1
    assert replay_urls[1] in policy1_v2_urls

    # Test filtering by policy URI (policy2 version 1)
    policy2_urls = db.get_replay_urls(policy_uri=CheckpointManager.normalize_uri("policy2/checkpoints/v1.pt"))
...
def test_from_shards_and_context(tmp_path: Path):
    ...
    agent_map = {0: CheckpointManager.normalize_uri("test_policy/checkpoints/v1.pt")}
    ...
    merged_db = SimulationStatsDB.from_shards_and_context(
        ...
        sim_name="test_sim",
        sim_env="test_env",
        policy_uri=CheckpointManager.normalize_uri("test_policy/checkpoints/v1.pt"),
    )
