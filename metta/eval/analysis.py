...
def analyze(checkpoint_info: CheckpointInfo, config: AnalysisConfig) -> None:
    logger = logging.getLogger(__name__)
    logger.info(f"Analyzing policy: {checkpoint_info.uri}")
    ...
    with local_copy(config.eval_db_uri) as local_path:
        ...
        sample_count = stats_db.sample_count(checkpoint_info, sim_name=config.sim_name)
        if sample_count == 0:
            pk, pv = stats_db.key_and_version(checkpoint_info)
        ...
        available_metrics = get_available_metrics(stats_db, checkpoint_info)
        ...
        metrics_data = get_metrics_data(stats_db, checkpoint_info, selected_metrics, sim_name=config.sim_name)
        print_metrics_table(stats_db, metrics_data, checkpoint_info)


# --------------------------------------------------------------------------- #
#   helpers                                                                   #
# --------------------------------------------------------------------------- #
def get_available_metrics(stats_db: EvalStatsDB, checkpoint_info: CheckpointInfo) -> List[str]:
    pk, pv = stats_db.key_and_version(checkpoint_info)
...
def get_metrics_data(
    ...
    for m in metrics:
        mean = stats_db.get_average_metric_by_filter(m, checkpoint_info, filter_condition)
        if mean is None:
            continue
        std = stats_db.get_std_metric_by_filter(m, checkpoint_info, filter_condition) or 0.0
...
def print_metrics_table(
    stats_db: EvalStatsDB, metrics_data: Dict[str, Dict[str, float]], checkpoint_info: CheckpointInfo
) -> None:
    logger = logging.getLogger(__name__)
    if not metrics_data:
        pk, pv = stats_db.key_and_version(checkpoint_info)
    ...
    logger.info(f"\nMetrics for policy: {checkpoint_info.uri}\n")
