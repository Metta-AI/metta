...
class Simulation:
    ...
    def __init__(
        ...
        if pre_built_config is not None:
            ...
            curriculum = single_task_tree(config.env, pre_built_config)
        else:
            env_config = config_from_path(config.env, env_overrides)
            curriculum = single_task_tree(config.env, env_config)
