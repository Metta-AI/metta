            ...
            if trainer_cfg.evaluation and should_run(epoch, trainer_cfg.evaluation.evaluate_interval):
                if latest_saved_policy_record:
                    ...
                    if trainer_cfg.evaluation.evaluate_remote:
                        try:
                            # Get checkpoint URI from the latest saved policy record
                            if hasattr(latest_saved_policy_record, "uri"):
                                checkpoint_uri = latest_saved_policy_record.uri
                            elif hasattr(latest_saved_policy_record, "checkpoint_uri"):
                                checkpoint_uri = latest_saved_policy_record.checkpoint_uri
                            else:
                                # Fallback to constructing URI from checkpoint directory
                                checkpoint_uri = f"file://{trainer_cfg.checkpoint.checkpoint_dir}/{run}__e{epoch}__s{agent_step}__t{int(timer.get_total_elapsed())}.pt"
