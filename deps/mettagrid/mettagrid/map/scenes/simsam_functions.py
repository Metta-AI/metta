...
def xy_noise(
    ...
    width: int,
    height: int,
    x_zoom: float = 0.1,  # x scale
    y_zoom: float = 0.1,  # y scale
) -> tuple[float, float]:
    """
    simple function to sample noise almost directly, scaled simply along x and y axes

    used almost everywhere as an additional layer
    """
...
def squeezed_noise(
    ...
    width: int,
    height: int,
    zoom: float = 0.1,  # global scale
    squeeze: float = 1.5,  # squeeze or ellipticity factor
    angle_theta: float = 0.25,  # angle of rotation. Full circle from 0 to 1
) -> tuple[float, float]:
    """
    scaled globally, rotated arbitrary at angle theta and squeezed

    function used in "noise" generator config
    """
...
def spiral(
    ...
    """
    Samples noise in a spiral pattern around an arbitrary point at (xc,yc)

    used in "spiral" generator
    """
    alpha = 2 * math.pi * angle_theta
    cs = math.cos(alpha)
    sn = math.sin(alpha)
...
def arbitrary_tilted_lattice(
    ...
    """
    Places wall sections randomly in a global lattice pattern of potential spots.
    The background (not a global lattice) is set to be sampled from the same spot everywhere.

    used in "arbitrary_tilted_lattice" generator
    """
    alpha = 2 * math.pi * angle_theta
...
def arbitrary_tilted_napkin(
    ...
    """
    During my attempts to modify "the lattice" sampler I've made a few mistakes.
    As a result, it produced an interesting pattern. I don't know why exactly, but it works.
    It generates a lot of repeating or similar sections of walls and intersections.
    Infinite backrooms, but for agents.

    used in "arbitrary_tilted_napkin" generator
    """
    alpha = 2 * math.pi * angle_theta
    alpha_tangent = math.tan(alpha)
    line1 = math.floor(x + alpha_tangent * y)
...
def the_sphere(
    ...
    used in "the_sphere" and "the_what" generator
    """
...
def radial_symmetry(
    ...
    """
    Generates radially symmetric pattern.
    If viewed in polar coordinates: N identical sections will be sampled from the same points in noise.
    This is done by working in polar coordinates and sampling noise N times faster than terrain itself.

    For example with N=6 symmetry:
    After first 60deg is done in terrain, noise would be sampled from full 360deg circle.

    Could be used to generate ~symmetric maps for N teams of agents.

    used in "radial_symmetry" generator
    """
    alpha = 2 * math.pi * angle_theta
    cs = math.cos(alpha)
    sn = math.sin(alpha)
