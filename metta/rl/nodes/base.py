...
def analyze_loss_alignment(
    ...
    """Compute alignment metrics between two loss vectors stored in shared_data."""
...
class NodeBase:
    """Base class coordinating rollout and training behaviour for node implementations."""
    ...
    def policy_output_keys(self, policy_td: Optional[TensorDict] = None) -> set[str]:
        raise NotImplementedError("Nodes must implement policy_output_keys")
    ...
    # Scheduling helpers
    def node_gate_allows(self, phase: str, context: ComponentContext) -> bool:
        gates = getattr(context, "node_run_gates", None)
        if not gates:
            return True
        entry = gates.get(self.node_name)
        if not entry:
            return True
    ...
    def attach_replay_buffer(self, experience: Experience) -> None:
        """Attach the replay buffer to the node."""
    ...
    def register_state_attr(self, *names: str) -> None:
        """Register attributes that should be persisted in the node state."""

        for name in names:
            if not hasattr(self, name):
                raise AttributeError(f"Node has no attribute '{name}' to register for state tracking")
