...
_POLICY_CLASS_SHORTHAND: dict[str, str] = {
    ...
    "nim-scripted": "cogames.policy.nim_agent.NimScriptedPolicy",
}
...
def initialize_or_load_policy(
    ...
    if env is not None:
        if hasattr(policy, "attach_environment"):
            policy.attach_environment(env)

        simulation = None
        env_candidates = [env]
        driver_env = getattr(env, "driver_env", None)
        if driver_env is not None:
            env_candidates.append(driver_env)
        ...
        if simulation is not None and hasattr(policy, "attach_simulation"):
            policy.attach_simulation(simulation)
