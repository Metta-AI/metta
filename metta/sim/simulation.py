...
from mettagrid.simulator import Simulation as MettaGridSimulation
from mettagrid.simulator.interface import AgentObservation, ObservationFeature
...
class MettaPolicyAdapter(AgentPolicy):
    """Adapter to wrap MettaAgent policy for use with Rollout."""

    def __init__(self, policy: Policy, agent_id: int, device: torch.device, actions_config):
    ...
    def step(self, obs: AgentObservation):
        """Get action from MettaAgent policy."""
        # Convert observation to tensor dict format expected by MettaAgent
        obs_array = np.array([obs])  # Add batch dimension
        td = obs_to_td(obs_array, self._device)
...
class Simulation:
    ...
    def __init__(
        ...
        # We need to create a temporary environment to get GameRules
    ...
    def _create_game_rules(self) -> GameRules:
        """Create GameRules from environment config by creating a temporary simulation."""
        # Create a temporary simulation to extract properties
        temp_sim = MettaGridSimulation(self._env_cfg, seed=0)

        # Extract observation features
        obs_features: dict[str, ObservationFeature] = {feat.name: feat for feat in temp_sim.features}

        # Create observation and action spaces
        observation_space = Box(
            low=0,
            high=255,
            shape=temp_sim.observation_shape,
            dtype=np.uint8,
        )

        action_space = Discrete(len(temp_sim.action_names))

        # Feature normalizations
        feature_normalizations = {feat.id: feat.normalization for feat in temp_sim.features}

        game_rules = GameRules(
            obs_width=1,  # Not used in new API
            obs_height=temp_sim.num_observation_tokens,
            obs_features=obs_features,
            action_names=temp_sim.action_names,
    ...
    def start_simulation(self) -> None:
        ...
        # Create game rules and instantiate policies
        game_rules = self._create_game_rules()
    ...
    def _create_agent_policies(self, seed: int) -> list[AgentPolicy]:
        ...
        for agent_id in range(self._agents_per_env):
            ...
            if agent_id < self._policy_agents_per_env:
                policy_adapter = MettaPolicyAdapter(self._policy, agent_id, self._device, self._env_cfg.game.actions)
            else:
                if self._npc_policy is not None:
                    policy_adapter = MettaPolicyAdapter(
                        self._npc_policy, agent_id, self._device, self._env_cfg.game.actions
                    )
                else:
                    # Fallback to main policy if no NPC policy
                    policy_adapter = MettaPolicyAdapter(
                        self._policy, agent_id, self._device, self._env_cfg.game.actions
                    )

            agent_policies.append(policy_adapter)
