...
run_test() {
    ...
    echo ""
    echo "Running: $name"
    echo "Command: timeout ${timeout}s uv run ./tools/run.py experiment.cogs_v_clips use_lp=True run=$name $extra_args"
    echo "Press Ctrl+C within 5 seconds to skip..."
    sleep 5

    timeout ${timeout}s uv run ./tools/run.py experiment.cogs_v_clips use_lp=True run=$name $extra_args || true

    echo ""
    echo "Test complete: $name"
    echo "Check logs: outputs/*${name}*/logs/train.log"
    echo ""
}
