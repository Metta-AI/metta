...
cdef class MettaActionHandler(ActionHandler):
    ...
    cdef bint handle_action(
        ...
        if actor.shield:
            ...
            self.env._stats.agent_add(actor_id, "." + actor.species + ".shield_upkeep", actor.shield_upkeep)
            self.env._stats.agent_incr(actor_id, "status.shield.ticks")
            self.env._stats.agent_incr(actor_id, "." + actor.species + ".status.shield.ticks")
            if actor.energy == 0:
                actor.shield = False

        if actor.frozen > 0:
            self.env._stats.agent_incr(actor_id, "status.frozen.ticks")
            self.env._stats.agent_incr(actor_id, "." + actor.species + ".status.frozen.ticks")
            actor.frozen -= 1
            return False
        ...
        self.env._stats.agent_add(actor_id, "." + actor.species + "." + self._stats.action_energy.c_str(), self.action_cost)
