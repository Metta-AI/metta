...
def print_summary(jobs: dict[str, Job], tail_lines: int = 50) -> bool:
    ...
    succeeded = [j for j in jobs.values() if j.status == JobStatus.SUCCEEDED and j.acceptance_passed in (None, True)]
    failed = [
        j
        for j in jobs.values()
        if j.status == JobStatus.FAILED or (j.status == JobStatus.SUCCEEDED and j.acceptance_passed is False)
    ]
    ...
    for job in jobs.values():
        ...
        if job.status == JobStatus.SUCCEEDED and job.acceptance_passed is False:
            status_key = "failed"
        print(f"  {status_key.upper():<9} {job.name}{duration}")
        if job.error:
