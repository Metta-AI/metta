#!/usr/bin/env python3
"""
Comprehensive Evaluation Script using Available Maps

This script runs evaluations on missions that use available maps:
- extractor_hub maps (30x30, 50x50, 70x70, 80x80, 100x100)
- eval_collect_resources maps (easy, medium)
- eval_clip_oxygen.map
- machina_eval_template.map (for clipped missions)
"""
...
def create_extractor_hub_missions():
    """Create missions using the available extractor_hub maps."""
    from cogames.cogs_vs_clips.mission import Mission, Site
    
    # Create a site for extractor hub missions
    extractor_hub_site = Site(
        name="extractor_hub",
        description="Extractor hub missions with different map sizes.",
        map_builder=get_map("extractor_hub_50x50.map"),  # Default to 50x50
        min_cogs=1,
        max_cogs=8,
    )
    ...
    # Create missions for different extractor hub sizes
    for size in [30, 50, 70, 80, 100]:
        class_name = f"ExtractorHub{size}x{size}"
        
        class ExtractorHubMission(Mission):
            name: str = f"extractor_hub_{size}x{size}"
            description: str = f"Extractor hub mission on {size}x{size} map"
            site: Site = extractor_hub_site
            map_name: str = f"extractor_hub_{size}x{size}.map"
            
            # Standard configuration for extractor hub
            carbon_eff: int = 100
            oxygen_eff: int = 100
            germanium_eff: int = 1
            silicon_eff: int = 100
        ...
        ExtractorHubMission.__name__ = class_name
        missions.append(ExtractorHubMission)
    
    return missions


def get_available_missions():
    """Get all available missions including extractor hub missions."""
    available_missions = []
    
    # Add extractor hub missions
    extractor_hub_missions = create_extractor_hub_missions()
    available_missions.extend(extractor_hub_missions)
    
    # Add existing missions that have available maps
...
def run_single_evaluation(
    ...
    try:
        ...
        if hasattr(total_reward, '__len__') and len(total_reward) > 0:
...
def print_summary(results: List[Dict[str, Any]]):
    ...
    # Success rate by mission type (original vs clipped vs extractor_hub)
    print(f"\nSuccess rate by mission type:")
    original_success = 0
    original_total = 0
    clipped_success = 0
    clipped_total = 0
    extractor_hub_success = 0
    extractor_hub_total = 0
    
    for result in results:
        if result['mission_name'].startswith('extractor_hub_'):
            extractor_hub_total += 1
            if result['success']:
                extractor_hub_success += 1
        elif result['mission_name'].startswith('clip_'):
            clipped_total += 1
            if result['success']:
                clipped_success += 1
        else:
    ...
    if extractor_hub_total > 0:
        print(f"  Extractor hub missions: {extractor_hub_success/extractor_hub_total:.2%} ({extractor_hub_success}/{extractor_hub_total})")


if __name__ == "__main__":
    print("Starting comprehensive evaluation with available maps...")
