...
def _require_merged_at(entry: typing.Dict[str, typing.Any]) -> str:
...
def _coerce_min_authored_at(value: datetime.datetime | str | None) -> datetime.datetime | None:
    if value is None:
        return None
...
def resolve_cache_paths(path: pathlib.Path) -> typing.Tuple[pathlib.Path, pathlib.Path]:
...
def _load_legacy_cache(path: pathlib.Path) -> typing.Tuple[CacheMetadata, typing.List[EmbeddingRecord]]:
    ...
    entries: typing.List[EmbeddingRecord] = []
...
def load_cache(path: pathlib.Path) -> typing.Tuple[CacheMetadata, typing.List[EmbeddingRecord]]:
...
def create_client(api_key: str) -> google.genai.Client:
...
def embed_text(
    ...
    return list(typing.cast(typing.Sequence[float], values))


def cosine_similarity(vector_a: typing.Sequence[float], vector_b: typing.Sequence[float]) -> float:
...
def score_records(
    query_vector: typing.Sequence[float],
    records: typing.Iterable[EmbeddingRecord],
) -> typing.List[typing.Tuple[float, EmbeddingRecord]]:
    scored: typing.List[typing.Tuple[float, EmbeddingRecord]] = []
...
def find_similar_prs(
    ...
    *,
    ...
    min_authored_at: datetime.datetime | str | None = None,
) -> typing.Tuple[CacheMetadata, typing.List[typing.Tuple[float, EmbeddingRecord]]]:
    if top_k <= 0:
    ...
    if min_date is not None:
        filtered_records: typing.List[EmbeddingRecord] = []
        for record in records:
