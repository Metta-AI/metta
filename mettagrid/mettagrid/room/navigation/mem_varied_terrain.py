...
class MemVariedTerrain(Room):
    ...
    def _build(self) -> np.ndarray:
        ...
        if self._teams is None:
            if isinstance(self._agents, int):
                agents_list = ["agent.agent"] * self._agents
            else:
                agents_list = []
        else:
            agents_list = []
            if isinstance(self._agents, int) and self._teams:
                per_team = self._agents // len(self._teams) if self._teams else 0
                for team in self._teams:
                    agents_list += [f"agent.{team}"] * per_team
        ...
        # Scatter objects randomly (altar, mine, generator counts)
        for obj_name, obj_cfg in self._objects.items():
            try:
                target_count = int(obj_cfg)
        ...
        # Place agents.
        for agent_symbol in agents_list:
            pos = self._choose_random_empty()
            if pos is None:
                break
            r, c = pos
    ...
    def _place_blocks(self, grid: np.ndarray) -> np.ndarray:
        ...
        for _ in range(block_count):
            block_w_raw = self._rng.integers(2, 15)
            block_h_raw = self._rng.integers(2, 15)
            block_w = int(block_w_raw)
            block_h = int(block_h_raw)
    ...
    def _generate_labyrinth_pattern(self) -> np.ndarray:
        ...
        # Removed scattering altars in labyrinths for mem variant
...
# End of MemVariedTerrain class implementation
