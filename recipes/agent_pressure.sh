...
for seed in 0; do
  ./devops/skypilot/launch.py train \
    run=$USER.navigation.ffa_8xupdate_epoch_MASSIVE_${agent_cluster_type}_${num_rooms}rooms_of_${num_agents_per_room}_seed${seed}.$(date +%m-%d) \
    ...
    ++trainer.env_overrides.game.map_builder.room.agensts=${num_agents_per_room} \
