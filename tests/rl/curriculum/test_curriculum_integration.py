...
def test_multiple_clients():
    ...
    try:
        ...
        clients = []
        for _i in range(3):
            client = CurriculumClient(server_url="http://127.0.0.1:15558", batch_size=2)
...
def test_learning_progress_curriculum():
    ...
    class LearningProgressMock(Curriculum):
        ...
        def get_task(self) -> Task:
            ...
            task_name = random.choices(list(self.task_weights.keys()), weights=list(self.task_weights.values()))[0]
            self.task_count += 1
    ...
    try:
        client = CurriculumClient(server_url="http://127.0.0.1:15560", batch_size=10)
