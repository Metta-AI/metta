...
def compute_contrastive_loss(
    ...
    current_indices = (segment_indices.unsqueeze(1) * bptt_horizon + torch.arange(bptt_horizon, device=device)).view(-1)

    # sample future indices using geometric distribution
    u = torch.rand(batch_size, device=device)
    gamma = torch.tensor(trainer_cfg.contrastive.gamma, device=device)
    ...
    for i in range(batch_size):
        ...
        if len(done_indices) > 0:
            ...
            delta[i] = min(delta[i], max_steps_available)
    ...
    print(f"lstm_hidden shape: {lstm_hidden.shape}")  # should be [minibatch_segments, bptt_horizon, hidden_size]
    print(f"all_lstm_hidden shape: {all_lstm_hidden.shape}")  # should be [segments, bptt_horizon, hidden_size]
