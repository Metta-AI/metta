...
TaskStatus = Literal["unprocessed", "running", "canceled", "done", "error"]
FinishedTaskStatus = Literal["done", "error", "canceled"]
    ...
    async def create_eval_task(
        ...
        async with self.connect() as con:
            async with con.cursor(row_factory=class_row(EvalTaskRow)) as cur:
                await cur.execute(
                    """
                    INSERT INTO eval_tasks (command, git_hash, attributes, user_id)
                    VALUES (%s, %s, %s, %s)
                    RETURNING *
                    """,
                    (command, git_hash, Jsonb(attributes), user_id),
                )
        ...
        async with self.connect() as con:
            async with con.cursor(row_factory=class_row(EvalTaskRow)) as cur:
                await cur.execute(
                    ...
                    SELECT *
                    FROM eval_tasks
                    WHERE status = 'unprocessed' AND assignee IS NULL
    ...
    async def get_git_hashes_for_workers(self, assignees: list[str]) -> dict[str, list[str]]:
        async with self.connect() as con:
            ...
            queryRes = await con.execute(
                "SELECT DISTINCT assignee, git_hash FROM eval_tasks WHERE assignee = ANY(%s)",
                (assignees,),
            )
