...
class Rollout:
    ...
    def step(self) -> None:
        ...
        for i in range(len(self._policies)):
            if (
                getattr(self._policies[i], "uses_raw_numpy", False)
                or type(self._policies[i]).__name__ == "HeuristicAgentPolicy"
            ):
                # These policies operate on the raw buffers directly.
                raw_observations = self._sim.raw_observations()
                raw_actions = self._sim.raw_actions()
                self._policies[i].step(raw_observations, raw_actions)
            else:
