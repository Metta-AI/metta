...
class SlidingTransformerWithGRUConfig(ComponentConfig):
    in_key: str
    out_key: str
    name: str = "sliding_transformer_with_gru"
    output_dim: int = 16
    input_dim: int = 64
    num_heads: int = 1
    ff_mult: int = 4
    ...
    def make_component(self, env=None):
        return SlidingTransformerWithGRU(config=self, env=env)


class SlidingTransformerWithGRU(nn.Module):
    """A sliding window transformer with a KV cache and a GRU.

    This component processes sequences of observations, actions, and rewards.
    It maintains a KV cache for the transformer and a hidden state for the GRU
    to support efficient rollouts and training with BPTT.
    It has two modes of operation, determined by the time dimension (TT) of the input TensorDict:
    - Rollout (TT=1): Processes a single timestep for a batch of environments, updating the KV cache and GRU state.
    - Training (TT>1): Processes a batch of sequences, using a causal mask for self-attention and propagating GRU state.
    """

    def __init__(self, config: SlidingTransformerWithGRUConfig, env):
        super().__init__()
        self.config = config
        ...
        gru_state_shape = (0, self.config.gru_num_layers, self.config.gru_hidden_dim)
        self.register_buffer("gru_state_rollout", torch.empty(gru_state_shape))
        self.register_buffer("gru_state_training", torch.empty(gru_state_shape))
    ...
    def _forward_rollout(self, td: TensorDict, x: torch.Tensor, B: int, S: int, training_env_ids: torch.Tensor):
        ...
        if max_num_envs > self.position_counter.size(0):
            ...
            gru_filler = torch.zeros(
                num_new_envs, *self.gru_state_rollout.shape[1:], device=self.gru_state_rollout.device
            )
            self.gru_state_rollout = torch.cat([self.gru_state_rollout, gru_filler])
            self.gru_state_training = self.gru_state_rollout.clone()
        ...
        # GRU processing
        hidden_state = self.gru_state_rollout[training_env_ids].transpose(0, 1)
        gru_output, new_hidden_state = self.gru(pooled_output.unsqueeze(1), hidden_state)
        self.gru_state_rollout[training_env_ids] = new_hidden_state.transpose(0, 1).detach()

        td.set(self.out_key, gru_output.squeeze(1))
        td.set("gru_hidden_state", hidden_state.transpose(0, 1).detach())
        ...
        self.gru_state_rollout[training_env_ids] *= (~resets.bool()).unsqueeze(-1).unsqueeze(-1).float()
    ...
    def _forward_training(
        ...
        # GRU processing
        initial_hidden_state = td["gru_hidden_state"].view(
            B, TT, self.config.gru_num_layers, self.config.gru_hidden_dim
        )
        initial_hidden_state = initial_hidden_state[:, 0].transpose(0, 1)  # L, B, H
        gru_output, last_hidden = self.gru(pooled_output, initial_hidden_state.detach())
        self.gru_state_training[training_env_ids] = last_hidden.transpose(0, 1).detach()
        self.gru_state_rollout = self.gru_state_training.clone()

        gru_output = rearrange(gru_output, "b tt ... -> (b tt) ...")
        td.set(self.out_key, gru_output)
        ...
        return Composite(
            {
                ...
                "gru_hidden_state": UnboundedContinuous(shape=(self.config.gru_num_layers, self.config.gru_hidden_dim)),
            }
        )

    def get_memory(self):
        return self.k_cache_training, self.v_cache_training, self.gru_state_training

    def set_memory(self, memory):
        """Cannot be called at the Policy level - use policy.<path_to_this_layer>.set_memory()"""
        self.k_cache_training, self.v_cache_training, self.gru_state_training = memory[0], memory[1], memory[2]
