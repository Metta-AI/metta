...
from metta.sweep.protein_utils import convert_suggestion_to_cli_args
...
def run_single_rollout(cfg: DictConfig, original_args: list[str] | None = None) -> int:
    ...
    run_name, protein_suggestion = run_once(
...
def launch_training_subprocess(
    ...
    if config_sections:
        config_args = convert_suggestion_to_cli_args(config_sections)
        cmd.extend(config_args)
        logger.info(f"Config args: {config_args}")

    # Convert protein suggestion to command-line arguments
    suggestion_args = convert_suggestion_to_cli_args(protein_suggestion)
