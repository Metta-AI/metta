...
class TestEvalTaskRoutes:
    ...
    def eval_task_client(self, test_client: TestClient, test_app: FastAPI) -> EvalTaskClient:
        ...
        client._machine_token = None

        # Override the _make_request method to add X-Auth-Request-Email header
        original_http_request = client._http_client.request
        ...
        async def request_with_auth(method, url, **kwargs):
            ...
            return await original_http_request(method, url, **kwargs)
    ...
    @pytest.mark.asyncio
    async def test_create_eval_task(self, eval_task_client: EvalTaskClient):
    ...
    @pytest.mark.asyncio
    async def test_get_available_tasks(self, eval_task_client: EvalTaskClient):
    ...
    @pytest.mark.asyncio
    async def test_claim_and_update_tasks(self, eval_task_client: EvalTaskClient):
        ...
        # Update to done
        await eval_task_client.finish_task(task_id, TaskFinishRequest(task_id=task_id, status="done"))

        # Verify status updated
        claimed_tasks = await eval_task_client.get_claimed_tasks(assignee="worker_1")
    ...
    @pytest.mark.asyncio
    async def test_get_claimed_tasks_without_assignee(self, eval_task_client: EvalTaskClient):
    ...
    @pytest.mark.asyncio
    async def test_task_assignment_expiry(self, eval_task_client: EvalTaskClient):
    ...
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_multiple_workers_claiming_same_task(self, eval_task_client: EvalTaskClient):
    ...
    @pytest.mark.asyncio
    async def test_update_task_with_error_reason(self, eval_task_client: EvalTaskClient):
        ...
        await eval_task_client.finish_task(
    ...
    @pytest.mark.asyncio
    async def test_update_task_mixed_formats(self, eval_task_client: EvalTaskClient):
        ...
        await eval_task_client.finish_task(task_ids[0], TaskFinishRequest(task_id=task_ids[0], status="done"))
        await eval_task_client.finish_task(
            task_ids[1],
            TaskFinishRequest(
        ...
        await eval_task_client.finish_task(task_ids[2], TaskFinishRequest(task_id=task_ids[2], status="canceled"))

        # Verify all updated
        all_tasks = await eval_task_client.get_all_tasks(TaskFilterParams(limit=100))
    ...
    async def test_error_reason_stored_in_db(self, eval_task_client: EvalTaskClient, stats_repo: MettaRepo):
        ...
        async with stats_repo.connect() as con:
            result = await con.execute("SELECT status, status_details FROM eval_tasks WHERE id = %s", (task_id,))
    ...
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_get_all_tasks_with_filters(self, eval_task_client: EvalTaskClient):
        ...
        await eval_task_client.finish_task(task4.id, TaskFinishRequest(task_id=task4.id, status="done"))
    ...
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_get_all_tasks_with_multiple_statuses(self, eval_task_client: EvalTaskClient):
        ...
        await eval_task_client.finish_task(done_task.id, TaskFinishRequest(task_id=done_task.id, status="done"))
