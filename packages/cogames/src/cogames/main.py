...
def play_cmd(
    ...
    resolved_mission, env_cfg = get_mission_name_and_config(ctx, mission)
    warning_msg, env_cfg = _apply_num_cogs_override(resolved_mission, env_cfg, num_cogs)
    if warning_msg:
        console.print(f"[yellow]Warning: {warning_msg}. Using default configuration for '{resolved_mission}'.[/yellow]")
