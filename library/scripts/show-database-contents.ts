...
async function showDatabaseContents() {
  ...
  try {
    ...
    console.log(
      `${"ID".padEnd(25)} | ${"Name".padEnd(25)} | ${"ORCID".padEnd(20)} | ${"Papers".padEnd(10)}`
    );
    console.log("-".repeat(80));
    ...
    console.log(
      `${"ID".padEnd(25)} | ${"Title".padEnd(50)} | ${"arXiv ID".padEnd(15)} | ${"Authors".padEnd(20)}`
    );
    console.log("-".repeat(120));
    ...
    papers.forEach((paper) => {
      ...
      const title =
        paper.title.length > 47
          ? paper.title.substring(0, 47) + "..."
          : paper.title;
      const arxivId = (paper.externalId || "N/A").padEnd(15);
      const authorNames = paper.paperAuthors
        .map((pa) => pa.author.name)
        .join(", ");
      const authors =
        authorNames.length > 17
          ? authorNames.substring(0, 17) + "..."
          : authorNames;

      console.log(
        `${id} | ${title.padEnd(50)} | ${arxivId} | ${authors.padEnd(20)}`
      );
    });
    ...
    console.log(
      `${"Paper ID".padEnd(25)} | ${"Author ID".padEnd(25)} | ${"Paper Title".padEnd(40)}`
    );
    console.log("-".repeat(100));
    ...
    relationships.forEach((rel) => {
      ...
      const title =
        rel.paper.title.length > 37
          ? rel.paper.title.substring(0, 37) + "..."
          : rel.paper.title;
    ...
    console.log(
      `Papers with Authors: ${papers.filter((p) => p.paperAuthors.length > 0).length}`
    );
    console.log(
      `Authors with Papers: ${authors.filter((a) => a.paperAuthors.length > 0).length}`
    );
    console.log(`Authors with ORCID: ${authors.filter((a) => a.orcid).length}`);
    console.log(
      `Authors with Institution: ${authors.filter((a) => a.institution).length}`
    );
    ...
    if (authors.length > 0) {
      ...
      console.log(
        `Papers: ${sampleAuthor.paperAuthors.map((pa) => pa.paper.title).join(", ")}`
      );
    }
    ...
    if (papers.length > 0) {
      ...
      console.log(
        `Abstract: ${samplePaper.abstract ? samplePaper.abstract.substring(0, 100) + "..." : "N/A"}`
      );
      console.log(
        `Authors: ${samplePaper.paperAuthors.map((pa) => pa.author.name).join(", ")}`
      );
    }
  } catch (error) {
