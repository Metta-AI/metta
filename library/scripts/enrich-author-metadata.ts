...
async function updateAuthorMetadata(
  ...
  const primaryInstitution =
    stats.institutions.length > 0 ? stats.institutions[0] : null;
...
async function processAuthorMetadata(
  ...
  try {
    ...
    console.log(
      `  📊 Found ${stats.paperCount} papers with ${stats.totalStars} total stars`
    );
    console.log(
      `  📈 Categories: ${stats.categories.slice(0, 3).join(", ")}...`
    );
    console.log(
      `  🏢 Institutions: ${stats.institutions.slice(0, 2).join(", ")}...`
    );
    ...
    progress.errors.push(
      `Author ${author.name}: ${error instanceof Error ? error.message : "Unknown error"}`
    );
