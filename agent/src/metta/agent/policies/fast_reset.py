...
class FastResetConfig(PolicyArchitecture):
    """
    ...
    class_path: str = "metta.agent.policies.fast_reset.FastResetPolicy"
    ...
    lstm_config: LSTMConfig = LSTMResetConfig(
...
class FastResetPolicy(Policy):
    def __init__(self, env, config: Optional[FastResetConfig] = None):
        super().__init__()
        self.config = config or FastResetConfig()
        self.env = env
        ...
        self.lstm = LSTMReset(config=self.config.lstm_config)
    ...
    def get_agent_experience_spec(self) -> Composite:
        return Composite(
            ...
            lstm_h=UnboundedDiscrete(
                shape=torch.Size([self.config.lstm_config.num_layers, self.config.lstm_config.hidden_size]),
                dtype=torch.float32,
            ),
