...
def parse_variants(variants_arg: typing.Optional[list[str]]) -> list[cogames.cogs_vs_clips.mission.MissionVariant]:
    ...
    for name in variants_arg:
        ...
        variant: cogames.cogs_vs_clips.mission.MissionVariant | None = None
        for v in cogames.cogs_vs_clips.variants.VARIANTS:
            if v.name == name:
                variant = v
                break
        ...
        if variant is None:
            ...
            available = ", ".join(v.name for v in cogames.cogs_vs_clips.variants.VARIANTS)
...
def get_all_missions() -> list[str]:
    ...
    return [mission.full_name() for mission in cogames.cogs_vs_clips.missions.MISSIONS]


def get_site_by_name(site_name: str) -> cogames.cogs_vs_clips.mission.Site:
    """Get a site by name.

    Raises:
        ValueError: If site not found
    """
    matching_sites = [site for site in cogames.cogs_vs_clips.sites.SITES if site.name == site_name]
...
def get_mission_name_and_config(
    ctx: typer.Context,
    mission_arg: typing.Optional[str],
    variants_arg: typing.Optional[list[str]] = None,
    cogs: typing.Optional[int] = None,
) -> tuple[str, mettagrid.MettaGridConfig, typing.Optional[cogames.cogs_vs_clips.mission.Mission]]:
    if not mission_arg:
        cogames.cli.base.console.print(ctx.get_help())
        cogames.cli.base.console.print("[yellow]Missing: --mission / -m[/yellow]\n")
    else:
        try:
            return get_mission(mission_arg, variants_arg, cogs)
        except ValueError as e:
            cogames.cli.base.console.print(f"[yellow]{e}[/yellow]\n")
    list_missions()
...
def get_mission_names_and_configs(
    ctx: typer.Context,
    missions_arg: typing.Optional[list[str]],
    *,
    variants_arg: typing.Optional[list[str]] = None,
    cogs: typing.Optional[int] = None,
    steps: typing.Optional[int] = None,
) -> list[tuple[str, mettagrid.MettaGridConfig]]:
    if not missions_arg:
        cogames.cli.base.console.print(ctx.get_help())
        cogames.cli.base.console.print("[yellow]Supply at least one: --mission / -m[/yellow]\n")
    else:
        try:
            not_deduped = [
                mission
...
def _get_missions_by_possible_wildcard(
    mission_arg: str,
    variants_arg: typing.Optional[list[str]],
    cogs: typing.Optional[int],
) -> list[tuple[str, mettagrid.MettaGridConfig]]:
    if "*" in mission_arg:
...
def find_mission(
...
) -> cogames.cogs_vs_clips.mission.Mission:
    for mission in cogames.cogs_vs_clips.missions.MISSIONS:
    ...
    if mission_name is not None:
        ...
        if site_name in [site.name for site in cogames.cogs_vs_clips.sites.SITES]:
...
def get_mission(
    mission_arg: str, variants_arg: typing.Optional[list[str]] = None, cogs: typing.Optional[int] = None
) -> tuple[str, mettagrid.MettaGridConfig, typing.Optional[cogames.cogs_vs_clips.mission.Mission]]:
    ...
    if any(mission_arg.endswith(ext) for ext in [".yaml", ".yml", ".json", ".py"]):
        ...
        if path.suffix == ".py":
            return mission_arg, cogames.game.load_mission_config_from_python(path), None
        elif path.suffix in [".yaml", ".yml", ".json"]:
            return mission_arg, cogames.game.load_mission_config(path), None
        else:
    ...
    if (delim_count := mission_arg.count(cogames.cogs_vs_clips.mission.MAP_MISSION_DELIMITER)) == 0:
        site_name, mission_name = mission_arg, None
    elif delim_count > 1:
        raise ValueError(
            f"Mission name can contain at most one `{cogames.cogs_vs_clips.mission.MAP_MISSION_DELIMITER}` delimiter"
        )
    else:
        site_name, mission_name = mission_arg.split(cogames.cogs_vs_clips.mission.MAP_MISSION_DELIMITER)
    ...
    if cogs is not None:
        mission = mission.with_variants([cogames.cogs_vs_clips.mission.NumCogsVariant(num_cogs=cogs)])
...
def list_variants() -> None:
    ...
    if not cogames.cogs_vs_clips.variants.VARIANTS:
        return

    cogames.cli.base.console.print("\n")
    variant_table = rich.table.Table(
        title="Available Variants", show_header=True, header_style="bold magenta", box=rich.box.ROUNDED, padding=(0, 1)
    )
    variant_table.add_column("Variant", style="yellow", no_wrap=True)
    variant_table.add_column("Description", style="white")

    for variant in cogames.cogs_vs_clips.variants.VARIANTS:
        variant_table.add_row(variant.name, variant.description)

    cogames.cli.base.console.print(variant_table)


def list_missions() -> None:
    """Print a table listing all available missions."""

    if not cogames.cogs_vs_clips.sites.SITES:
        cogames.cli.base.console.print("No missions found")
        return

    # Create a single table for all missions
    table = rich.table.Table(show_header=True, header_style="bold magenta", box=rich.box.ROUNDED, padding=(0, 1))
    ...
    for idx, site in enumerate(cogames.cogs_vs_clips.sites.SITES):
        # Get missions for this site
        site_missions = [
            mission for mission in cogames.cogs_vs_clips.missions.MISSIONS if mission.site.name == site.name
        ]
        ...
        for mission_idx, mission in enumerate(site_missions):
            ...
            is_last_site = idx == len(cogames.cogs_vs_clips.sites.SITES) - 1
    ...
    cogames.cli.base.console.print("  • Use a mission name from above (e.g., [blue]training_facility.harvest[/blue])")
    cogames.cli.base.console.print("  • Use a path to a mission configuration file, e.g. path/to/mission.yaml")
    cogames.cli.base.console.print("\nTo specify [bold yellow] -v [VARIANT][/bold yellow] modifiers:")
    cogames.cli.base.console.print(
        "  • Use multiple --variant flags: [yellow]--variant solar_flare --variant dark_side[/yellow]"
    )
    cogames.cli.base.console.print("  • Or use the short form: [yellow]-v solar_flare -v rough_terrain[/yellow]")
    cogames.cli.base.console.print("\nTo specify number of cogs:")
    cogames.cli.base.console.print("  • Use [green]--cogs N[/green] or [green]-c N[/green] (e.g., [green]-c 4[/green])")
    cogames.cli.base.console.print("\n[bold green]Examples:[/bold green]")
    cogames.cli.base.console.print("  [bold]cogames play[/bold] --mission [blue]training_facility.harvest[/blue]")
    cogames.cli.base.console.print(
...
def describe_mission(mission_name: str, game_config: mettagrid.MettaGridConfig) -> None:
    ...
    cogames.cli.base.console.print(f"\n[bold cyan]{mission_name}[/bold cyan]\n")

    # Display mission configuration
    cogames.cli.base.console.print("[bold]Mission Configuration:[/bold]")
    cogames.cli.base.console.print(f"  • Number of agents: {game_config.game.num_agents}")
    if isinstance(game_config.game.map_builder, mettagrid.mapgen.mapgen.MapGen.Config):
        cogames.cli.base.console.print(
            f"  • Map size: {game_config.game.map_builder.width}x{game_config.game.map_builder.height}"
        )
    ...
    for n, a in game_config.game.actions.model_dump().items():
        if a["enabled"]:
            cogames.cli.base.console.print(f"  • {n}: {a['consumed_resources']}")

    # Display objects
    cogames.cli.base.console.print("\n[bold]Stations:[/bold]")
    for obj_name, obj_config in game_config.game.objects.items():
        cogames.cli.base.console.print(f"  • {obj_name}")
        if isinstance(obj_config, mettagrid.config.mettagrid_config.AssemblerConfig):
            for protocol in obj_config.protocols:
                if protocol.input_resources:
                    ...
                    cogames.cli.base.console.print(f"    {inputs} → {outputs} (cooldown: {protocol.cooldown})")

    # Display agent configuration
    cogames.cli.base.console.print("\n[bold]Agent Configuration:[/bold]")
    cogames.cli.base.console.print(f"  • Default resource limit: {game_config.game.agent.default_resource_limit}")
    if game_config.game.agent.resource_limits:
        cogames.cli.base.console.print(f"  • Resource limits: {game_config.game.agent.resource_limits}")
