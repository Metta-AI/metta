...
class TestFloatDistributionInstancesSupport:
    def test_constant_distribution_instance(self):
        from pydantic import BaseModel

        class TestModel(BaseModel):
            dist: FloatDistribution

        dist_instance = FloatConstantDistribution(value=3.14)
        model = TestModel(dist=dist_instance)

        assert isinstance(model.dist, FloatConstantDistribution)
        assert model.dist.value == 3.14

        rng = np.random.default_rng(seed=123)
        assert model.dist.sample(rng) == 3.14

    def test_uniform_distribution_instance(self):
        from pydantic import BaseModel

        class TestModel(BaseModel):
            dist: FloatDistribution

        dist_instance = FloatUniformDistribution(low=1.0, high=5.0)
        model = TestModel(dist=dist_instance)

        assert isinstance(model.dist, FloatUniformDistribution)
        assert model.dist.low == 1.0
        assert model.dist.high == 5.0

        rng = np.random.default_rng(seed=123)
        samples = [model.dist.sample(rng) for _ in range(50)]
        assert all(1.0 <= sample <= 5.0 for sample in samples)

    def test_lognormal_distribution_instance(self):
        from pydantic import BaseModel

        class TestModel(BaseModel):
            dist: FloatDistribution

        dist_instance = FloatLognormalDistribution(low=1.0, high=10.0)
        model = TestModel(dist=dist_instance)
    ...
    def test_lognormal_distribution_instance_with_max(self):
        from pydantic import BaseModel

        class TestModel(BaseModel):
            dist: FloatDistribution

        dist_instance = FloatLognormalDistribution(low=1.0, high=10.0, max=15.0)
        model = TestModel(dist=dist_instance)
        ...
        rng = np.random.default_rng(seed=123)
        samples = [model.dist.sample(rng) for _ in range(100)]
        assert all(0 < sample <= 15.0 for sample in samples)
