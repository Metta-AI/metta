...
def create_real_curriculum_configs():
    ...
    return {
        ...
        "random": {
            "path": "/env/mettagrid/curriculum/navigation/random",
            "description": "Random Curriculum",
            "color": "red",
        },
        "sequential": {
            "path": "/env/mettagrid/curriculum/navigation/sequential",
            "description": "Sequential Curriculum",
            "color": "green",
        },
        "all": {
...
def create_analysis_config(curriculum_name: str, curriculum_path: str) -> DictConfig:
    """Create analysis configuration for a curriculum."""
    config_dict = {
        "analysis_mode": True,
        "analysis_epochs": 5,  # Small number for demo
        "analysis_tasks_per_epoch": 3,
        "oracle_curriculum": None,
        "analysis_output_dir": f"real_curriculum_analysis/{curriculum_name}",
        "analysis_metrics": [
            "efficiency_regret",
            "time_regret",
...
def run_curriculum_analysis_real(curriculum_name: str, curriculum_path: str) -> Dict:
    """Run curriculum analysis on a real curriculum."""
    logger.info(f"Analyzing {curriculum_name} at {curriculum_path}")

    try:
        # Create analysis config
        trainer_cfg = create_analysis_config(curriculum_name, curriculum_path)

        # Initialize Hydra for curriculum loading
        from omegaconf import OmegaConf

        # Create a simple Hydra config for curriculum loading
        OmegaConf.create(
            {"config_path": "configs", "config_name": "trainer/test_curriculum_analysis", "version_base": None}
        )

        # Load curriculum with proper Hydra setup
...
def generate_real_curriculum_tables(all_results: Dict[str, Dict]):
...
def generate_real_curriculum_plots(comparison_df: pd.DataFrame, all_results: Dict[str, Dict]):
    ...
    for _curriculum_name, results in all_results.items():
        ...
        row = [epoch_counts.get(epoch, 0) for epoch in range(5)]  # 5 epochs
    ...
    if heatmap_data:
        heatmap_df = pd.DataFrame(
            ...
            columns=[f"Epoch {i}" for i in range(5)],
        )
...
def main():
    ...
    # Get real curriculum configurations
    curriculum_configs = create_real_curriculum_configs()
    ...
    for curriculum_name, config in curriculum_configs.items():
        ...
        try:
            results = run_curriculum_analysis_real(curriculum_name, config["path"])
    ...
    # Generate tables
    comparison_df = generate_real_curriculum_tables(all_results)

    # Generate plots
    generate_real_curriculum_plots(comparison_df, all_results)
