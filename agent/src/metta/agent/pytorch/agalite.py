...
class AGaLiTeCore(nn.Module):
    ...
    def __init__(
        ...
        for layer in range(n_layers):
            if use_fast_mode and FAST_MODE_AVAILABLE:
                # Use fast implementation EXACTLY as in the working commit
...
class AGaLiTePolicy(nn.Module):
    ...
    def decode_actions(self, hidden: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if torch.isnan(hidden).any() or torch.isinf(hidden).any():
            ...
            print("WARNING: Invalid hidden representation in decode_actions", file=sys.stderr)
        ...
        if torch.isnan(logits).any() or torch.isinf(logits).any():
            ...
            print("WARNING: Invalid logits produced in decode_actions", file=sys.stderr)
