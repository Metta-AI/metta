        ...
        if isinstance(map_data, list):
            for row in map_data:
                if isinstance(row, list):
                    map_rows.append("".join(str(c) for c in row))
                else:
                    map_rows.append(str(row))
        spawn_count = sum(r.count("%") for r in map_rows)
        agent_count = sum(r.count("@") for r in map_rows)
        if spawn_count > 0 and agent_count == 0:
            if hasattr(map_builder_config, "target_agents") and isinstance(game.num_agents, int):
                try:
                    map_builder_config.target_agents = game.num_agents  # type: ignore[attr-defined]
                except Exception:  # pragma: no cover - defensive
                    pass
        for key, value in map_builder_args.items():
            if hasattr(map_builder_config, key):
                try:
                    setattr(map_builder_config, key, value)
                except Exception:  # pragma: no cover - defensive
                    pass
        game.map_builder = map_builder_config
        cfg = MettaGridConfig(game=game)
        mission_modifications.apply_modifications(
            cfg, {"easy": easy, "shaped": shaped, "extend_max_steps": easy or shaped}
        )
