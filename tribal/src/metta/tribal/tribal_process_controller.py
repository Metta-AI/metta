...
Uses file-based IPC to eliminate SIGSEGV issues.
"""
...
class TribalProcessController:
    """Controls a tribal environment running in a separate Nim process via JSON files."""
        ...
        try:
            ...
            if not viewer_exe.exists():
                ...
                result = subprocess.run(
                    ["bash", str(build_script)], cwd=self.tribal_dir, capture_output=True, text=True
                )
            ...
            self.nim_process = subprocess.Popen(
                [str(viewer_exe)], cwd=self.tribal_dir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
            )
    ...
    def stop_nim_process(self):
        ...
        if self.nim_process:
            try:
                ...
                shutdown_data = {"active": False, "shutdown": True, "timestamp": time.time()}
    ...
    def activate_communication(self):
        ...
        try:
            control_data = {"active": True, "shutdown": False, "timestamp": time.time()}
    ...
    def step(self, actions: List[List[int]]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict[str, Any]]:
        ...
        try:
            ...
            actions_data = {"actions": actions, "timestamp": time.time()}
        ...
        if self.nim_process and self.nim_process.stdout:
            try:
                ...
                if select.select([self.nim_process.stdout], [], [], 0)[0]:
                    output = self.nim_process.stdout.read(1024)
                    if output:
                        pass
            except Exception as e:
...
def test_process_communication():
    ...
    with TribalProcessController(tribal_dir) as controller:
        ...
        for step in range(5):
            ...
            actions = []
            for _ in range(controller.num_agents):
        ...
        print("✅ Process communication test completed!")
