...
class Mirror(Scene[MirrorConfig]):
    def get_children(self) -> list[ChildrenAction]:
        ...
        return [
            ChildrenAction(
                scene=symmetry_to_child_class[self.config.symmetry].Config(
...
class HorizontalMirror(Scene[HorizontalMirrorConfig]):
    ...
    def render(self):
        ...
        original_area = _make_area_if_positive(
            self, 0, 0, left_width, self.height, ["original"]
        )
        self._original_mirror_area = original_area or self.area

        mirrored_width = self.width - left_width
        _make_area_if_positive(
            self, left_width, 0, mirrored_width, self.height, ["mirrored"]
        )
...
class VerticalMirror(Scene[VerticalMirrorConfig]):
    ...
    def render(self):
        ...
        original_area = _make_area_if_positive(
            self, 0, 0, self.width, top_height, ["original"]
        )
        self._original_mirror_area = original_area or self.area

        mirrored_height = self.height - top_height
        _make_area_if_positive(
            self, 0, top_height, self.width, mirrored_height, ["mirrored"]
        )
...
class X4Mirror(Scene[X4MirrorConfig]):
    ...
    def render(self):
        ...
        original_area = _make_area_if_positive(
            self, 0, 0, sub_width, sub_height, ["original"]
        )
        self._original_mirror_area = original_area or self.area
        ...
        _make_area_if_positive(
            self, sub_width, 0, mirrored_width, sub_height, ["mirrored_x"]
        )
        _make_area_if_positive(
            self, 0, sub_height, sub_width, mirrored_height, ["mirrored_y"]
        )
        _make_area_if_positive(
            self,
            sub_width,
...
class Mirrored(Scene[MirroredConfig]):
    def render(self):
        ...
        slice_x = (
            slice(self.width - 1, None, -1) if self.config.flip_x else slice(self.width)
        )
        slice_y = (
            slice(self.height - 1, None, -1)
            if self.config.flip_y
            else slice(self.height)
        )
