...
# Note: Memory budgets vary based on empirically tested batch sizes
# Original target was ~528M tokens, but empirical testing showed larger sizes work
...
CONFIGS: List[Tuple[int, int, str]] = [
    ...
    (512, 4128768, "h512"),  # Empirically tested batch size
    (1024, 8257536, "h1024"),  # Empirically tested batch size
]
...
# Base experiment name with date
DATE_STR = datetime.now().strftime("%Y%m%d_%H%M%S")
BASE_NAME = "icl_bptt_sweep"
GROUP_NAME = f"{BASE_NAME}_{DATE_STR}"
...
def launch_job(
    ...
    job_name = f"{BASE_NAME}_{DATE_STR}_{suffix}_rep{replicate}"
    ...
    # Add module path and arguments after all launch.py flags
    cmd.extend(
        [
...
def main():
    ...
    print("Starting BPTT horizon sweep")
    print(f"Date/Time: {DATE_STR}")
    print(f"WandB Group: {GROUP_NAME}")
    ...
    print("\nJob naming pattern:")
    print(f"  {BASE_NAME}_{DATE_STR}_<horizon>_rep<N>")
    print(f"  Example: {BASE_NAME}_{DATE_STR}_h256_rep1")
    print("\nTo monitor jobs:")
    print("  sky status")
    print("\nTo view logs:")
    print("  sky logs <job_name>")
    print("\nTo cancel all jobs from this sweep:")
    print(f"  sky cancel {BASE_NAME}_{DATE_STR}_*")
