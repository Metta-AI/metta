...
class EnvConfigBuilder:
    ...
    def build(self) -> EnvConfig:
        ...
        if self._level_map is None:
            self._level_map = make_test_level_map(
...
def make_test_level_map(width: int = 25, height: int = 25, num_agents: int = 1) -> Level:
    ...
    Returns:
        Level with walls around perimeter and agents placed
    """
    ...
    return Level(grid=grid, labels=["test"])
