...
class CheckpointManager:
    ...
    def load_trainer_state(self) -> Optional[Dict[str, Any]]:
        ...
        if "optimizer" not in state and "optimizer_state" in state:
            state["optimizer"] = state["optimizer_state"]
        if "node_states" not in state and "loss_states" in state:
            state["node_states"] = state["loss_states"]
        return state
