...
def write_local_map_preview(env: MettaGridEnv):
    with tempfile.NamedTemporaryFile(
        delete=False, dir="./mettascope/local/", suffix=".json"
    ) as temp_file:
...
def upload_map_preview(
    ...
    try:
        write_file(
            path=s3_path, local_file=preview_path, content_type="application/x-compress"
        )
        ...
        if wandb_run:
            ...
            link_summary = {
                "replays/link": wandb.Html(
                    f'<a href="{player_url}">MetaScope Map Preview</a>'
                )
            }
