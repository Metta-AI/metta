...
class TestMazePrimMapBuilderConfig:
    def test_create(self):
        config = mettagrid.map_builder.maze.MazePrimMapBuilder.Config(
            width=11, height=11, start_pos=(1, 1), end_pos=(9, 9), seed=42
        )
        builder = config.create()
        assert isinstance(builder, mettagrid.map_builder.maze.MazePrimMapBuilder)


class TestMazeKruskalMapBuilderConfig:
    def test_create(self):
        config = mettagrid.map_builder.maze.MazeKruskalMapBuilder.Config(
            width=11, height=11, start_pos=(1, 1), end_pos=(9, 9), seed=42
        )
        builder = config.create()
        assert isinstance(builder, mettagrid.map_builder.maze.MazeKruskalMapBuilder)
...
class TestMazePrimMapBuilder:
    def test_build_deterministic_with_seed(self):
        ...
        builder1 = mettagrid.map_builder.maze.MazePrimMapBuilder(config)
        map1 = builder1.build()

        builder2 = mettagrid.map_builder.maze.MazePrimMapBuilder(config)
        map2 = builder2.build()

        assert isinstance(map1, mettagrid.map_builder.map_builder.GameMap)
        assert isinstance(map2, mettagrid.map_builder.map_builder.GameMap)
        assert np.array_equal(map1.grid, map2.grid)

    def test_build_different_seeds_different_results(self):
        config1 = mettagrid.map_builder.maze.MazePrimMapBuilder.Config(
            width=11, height=11, start_pos=(1, 1), end_pos=(9, 9), seed=42
        )
        config2 = mettagrid.map_builder.maze.MazePrimMapBuilder.Config(
            width=11, height=11, start_pos=(1, 1), end_pos=(9, 9), seed=123
        )

        builder1 = mettagrid.map_builder.maze.MazePrimMapBuilder(config1)
        builder2 = mettagrid.map_builder.maze.MazePrimMapBuilder(config2)
        map1 = builder1.build()
        map2 = builder2.build()
    ...
    def test_build_start_and_end_positions(self):
        ...
        builder = mettagrid.map_builder.maze.MazePrimMapBuilder(config)
    ...
    def test_build_odd_dimensions_preserved(self):
        config = mettagrid.map_builder.maze.MazePrimMapBuilder.Config(
            width=11, height=13, start_pos=(1, 1), end_pos=(9, 11), seed=42
        )
        builder = mettagrid.map_builder.maze.MazePrimMapBuilder(config)
    ...
    def test_build_even_dimensions_made_odd(self):
        config = mettagrid.map_builder.maze.MazePrimMapBuilder.Config(
            width=10, height=12, start_pos=(1, 1), end_pos=(7, 9), seed=42
        )
        builder = mettagrid.map_builder.maze.MazePrimMapBuilder(config)
    ...
    def test_build_position_adjustment(self):
        ...
        config = mettagrid.map_builder.maze.MazePrimMapBuilder.Config(
            width=11, height=11, start_pos=(0, 0), end_pos=(10, 10), seed=42
        )
        builder = mettagrid.map_builder.maze.MazePrimMapBuilder(config)
    ...
    def test_build_small_maze(self):
        config = mettagrid.map_builder.maze.MazePrimMapBuilder.Config(
            width=5, height=5, start_pos=(1, 1), end_pos=(3, 3), seed=42
        )
        builder = mettagrid.map_builder.maze.MazePrimMapBuilder(config)
        game_map = builder.build()
    ...
    def test_build_returns_game_map(self):
        config = mettagrid.map_builder.maze.MazePrimMapBuilder.Config(
            width=7, height=7, start_pos=(1, 1), end_pos=(5, 5), seed=42
        )
        builder = mettagrid.map_builder.maze.MazePrimMapBuilder(config)
        result = builder.build()

        assert isinstance(result, mettagrid.map_builder.map_builder.GameMap)
...
class TestMazeKruskalMapBuilder:
    def test_build_deterministic_with_seed(self):
        ...
        builder1 = mettagrid.map_builder.maze.MazeKruskalMapBuilder(config)
        map1 = builder1.build()

        builder2 = mettagrid.map_builder.maze.MazeKruskalMapBuilder(config)
        map2 = builder2.build()
    ...
    def test_build_different_seeds_different_results(self):
        config1 = mettagrid.map_builder.maze.MazeKruskalMapBuilder.Config(
            width=11, height=11, start_pos=(1, 1), end_pos=(9, 9), seed=42
        )
        config2 = mettagrid.map_builder.maze.MazeKruskalMapBuilder.Config(
            width=11, height=11, start_pos=(1, 1), end_pos=(9, 9), seed=123
        )

        builder1 = mettagrid.map_builder.maze.MazeKruskalMapBuilder(config1)
        builder2 = mettagrid.map_builder.maze.MazeKruskalMapBuilder(config2)
        map1 = builder1.build()
        map2 = builder2.build()
    ...
    def test_build_start_and_end_positions(self):
        ...
        builder = mettagrid.map_builder.maze.MazeKruskalMapBuilder(config)
        game_map = builder.build()
    ...
    def test_union_find_algorithm_integrity(self):
        ...
        config = mettagrid.map_builder.maze.MazeKruskalMapBuilder.Config(
            width=15, height=15, start_pos=(1, 1), end_pos=(13, 13), seed=42
        )
        builder = mettagrid.map_builder.maze.MazeKruskalMapBuilder(config)
        game_map = builder.build()
...
class TestMazeAlgorithmComparison:
    def test_both_algorithms_produce_valid_mazes(self):
        ...
        prim_config = mettagrid.map_builder.maze.MazePrimMapBuilder.Config(
            width=11, height=11, start_pos=(1, 1), end_pos=(9, 9), seed=42
        )
        kruskal_config = mettagrid.map_builder.maze.MazeKruskalMapBuilder.Config(
            width=11, height=11, start_pos=(1, 1), end_pos=(9, 9), seed=42
        )

        prim_builder = mettagrid.map_builder.maze.MazePrimMapBuilder(prim_config)
        kruskal_builder = mettagrid.map_builder.maze.MazeKruskalMapBuilder(kruskal_config)

        prim_map = prim_builder.build()
        kruskal_map = kruskal_builder.build()

        # Both should produce GameMap instances
        assert isinstance(prim_map, mettagrid.map_builder.map_builder.GameMap)
        assert isinstance(kruskal_map, mettagrid.map_builder.map_builder.GameMap)
