...
class MettaGridEnv(PufferEnv, GymEnv):
    ...
    def _initialize_c_env(self) -> None:
        ...
        config_dict = cast(Dict[str, Any], OmegaConf.to_container(task.env_cfg().game))
        ...
        if self._num_episodes == 0 and task.env_cfg().desync_episodes:
            ...
            config_dict["max_steps"] = int(np.random.randint(1, max_steps + 1))
            logger.info(f"Desync episode with max_steps {config_dict['max_steps']}")
