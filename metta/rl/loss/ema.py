...
class EMA(metta.rl.loss.Loss):
    ...
    def run_train(
        ...
        metta.rl.utils.ensure_sequence_metadata(policy_td, batch_size=B, time_steps=TT)

        pred_flat: torch.Tensor = policy_td["EMA_pred_output_2"].to(dtype=torch.float32)

        target_td = policy_td.select(*self.policy_experience_spec.keys(include_nested=True)).clone()
        metta.rl.utils.ensure_sequence_metadata(target_td, batch_size=B, time_steps=TT)

        with torch.no_grad():
            self.target_model(target_td)
            target_pred_flat: torch.Tensor = target_td["EMA_pred_output_2"].to(dtype=torch.float32)

        shared_loss_data["EMA"]["pred"] = pred_flat.reshape(B, TT, -1)
        shared_loss_data["EMA"]["target_pred"] = target_pred_flat.reshape(B, TT, -1)

        loss = torch.nn.functional.mse_loss(pred_flat, target_pred_flat) * self.ema_coef
