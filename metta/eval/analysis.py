...
def analyze(policy_record: SimplePolicyRecord, config: AnalysisConfig) -> None:
...
#   helpers                                                                   #
# --------------------------------------------------------------------------- #
def get_available_metrics(stats_db: EvalStatsDB, policy_record: CheckpointInfo) -> List[str]:
    pk, pv = stats_db.key_and_version(policy_record)
...
def print_metrics_table(
    stats_db: EvalStatsDB, metrics_data: Dict[str, Dict[str, float]], policy_record: CheckpointInfo
) -> None:
    logger = logging.getLogger(__name__)
    if not metrics_data:
        pk, pv = stats_db.key_and_version(policy_record)
