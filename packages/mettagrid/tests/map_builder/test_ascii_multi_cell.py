...
def test_ascii_multi_cell_grouping():
    ...
    assert game_map.grid[0, 0] == "wall"  # anchor
    # Other walls in the component should be empty
    empty_count = np.count_nonzero(game_map.grid == "empty")
    assert empty_count > 1  # At least the extra wall cells + the original empty cell
    
    # Check multi_cell_groups metadata
    assert len(game_map.multi_cell_groups) == 1  # One connected component
    anchor_r, anchor_c, layer, extra_cells = game_map.multi_cell_groups[0]
    assert layer == 1  # Walls are on ObjectLayer (1)
    assert len(extra_cells) >= 7  # 8 walls - 1 anchor = 7 extra cells
...
,
    ...
    # Each component should have 1 extra cell (2 walls - 1 anchor)
    for _, _, _, extra_cells in game_map.multi_cell_groups:
...
def test_ascii_layer_override():
    ...
    _, _, layer, extra_cells = game_map.multi_cell_groups[0]
    assert layer == 1
    assert len(extra_cells) == 2
