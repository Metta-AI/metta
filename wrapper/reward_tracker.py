...
class RewardTracker(gym.Wrapper):
    ...
    def _augment_observations(self, obs):
        return [
            {"last_reward": np.array(self._last_rewards[agent]), **agent_obs} for agent, agent_obs in enumerate(obs)
        ]
    ...
    def observation_space(self):
        return gym.spaces.Dict(
            {
                "last_reward": gym.spaces.Box(-np.inf, high=np.inf, shape=(1,), dtype=np.float32),
